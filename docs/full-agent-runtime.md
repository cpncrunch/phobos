...
- **Native tool-calling status contract:** `/status` now exposes read-only native autonomy safety metadata plus a milestone acceptance matrix: model-planning enablement, the bounded step budget, terminal no-tool/no-dispatch markers, execution-summary claim-count support, and max-step/model-error/invalid-plan/duplicate/same-step-duplicate stop enforcement, plan-only and explicit-`execute=true` defaults, one-shot and per-step planner traces, per-step execution-ledger delta support, supported provider-native tool-call variants, provider `tool_result`/`function_result` echo suppression, model-hidden approval controls, execution-capable and target-affecting tool classes, local native-loop contract completion flags, and local transcript counts without reading target systems or raw transcript contents.
