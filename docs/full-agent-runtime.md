...
`native_tool_call_milestone_contract_ok=True` is an aggregate gate over every native tool-call safety/translation smoke above, including single/top-level content-block calls, flat/nested/message-level/message-content/parts Responses output calls, Responses direct or nested `output[].message` `tool_calls`/`toolCalls`/`tool_call`/`toolCall` aliases, root/message `functionCall` / `functionCalls` / `function_calls` aliases, provider call-ID provenance, result-echo suppression, runtime policy, ROE preview, approval stops, and ledger claim semantics.
