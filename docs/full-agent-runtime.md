...
- **Evidence workspace:** all target-affecting decisions and outputs are written under the engagement evidence directory, with secret redaction applied to logged commands/tool args and audit event payloads before storage/display; redaction covers common Authorization bearer/basic headers, authorization assignments, cookie headers, quoted password/token/API-key values, cloud/OAuth secret fields such as client secrets and AWS secret access keys, and pasted PEM private-key blocks.
