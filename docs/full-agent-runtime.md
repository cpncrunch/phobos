...
- `/auto` has deterministic planning plus optional model-returned JSON plans, pre-dispatch name/schema validation, read-only guardrail preview metadata for target-affecting calls, redacted rejected-call transcript entries, and a bounded result-feedback `/auto-loop` with redacted transcript artifacts, but it is not yet Hermes' full native function-calling autonomy or a general-purpose task computer;
