...
- `/auto` has deterministic planning plus optional model-returned JSON/provider-native tool-call plans, pre-dispatch name/schema validation, read-only guardrail preview metadata for target-affecting calls, explicit allowed-execution ledger proof, redacted rejected-call transcript entries, redacted one-shot applied-plan transcripts under `agent/auto-plans`, redacted execution ledgers, and a bounded result-feedback `/auto-loop` with redacted transcript artifacts, but it is not yet Hermes' full native function-calling autonomy or a general-purpose task computer;
