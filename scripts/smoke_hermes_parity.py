...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        checks["native_tool_call_status_contract_ok"] = (
            ...
            and native_status_milestone_contract.get("tool_calls_nested_tool_object_translation") is True
            ...
            and native_status_milestone_contract.get("candidate_direct_function_call_translation") is True
            ...
            and native_status_milestone_contract.get("root_output_item_singular_wrapper_translation") is True
            and native_status_milestone_contract.get("root_items_wrapper_translation") is True
            and native_status_milestone_contract.get("root_item_singular_wrapper_translation") is True
            ...
            and native_status_milestone_contract.get("root_tool_use_alias_translation") is True
            and native_status_milestone_contract.get("root_neutral_tool_alias_translation") is True
            and native_status_milestone_contract.get("message_tool_use_alias_translation") is True
            and native_status_milestone_contract.get("message_neutral_tool_alias_translation") is True
            ...
            and "tool_calls_nested_tool" in native_status_data.get("provider_native_tool_call_variants", [])
            ...
            and "root_output_item_function_call" in native_status_data.get("provider_native_tool_call_variants", [])
            and "root_items_function_call" in native_status_data.get("provider_native_tool_call_variants", [])
            and "root_items_function" in native_status_data.get("provider_native_tool_call_variants", [])
            and "root_item_function_call" in native_status_data.get("provider_native_tool_call_variants", [])
            and "root_item_function" in native_status_data.get("provider_native_tool_call_variants", [])
            ...
            and "candidate_direct_function_call" in native_status_data.get("provider_native_tool_call_variants", [])
            ...
            and "root_tool_use" in native_status_data.get("provider_native_tool_call_variants", [])
            and "root_tool" in native_status_data.get("provider_native_tool_call_variants", [])
            and "root_toolUse" in native_status_data.get("provider_native_tool_call_variants", [])
            and "root_tool_uses" in native_status_data.get("provider_native_tool_call_variants", [])
            and "root_toolUses" in native_status_data.get("provider_native_tool_call_variants", [])
            and "message_tool_use" in native_status_data.get("provider_native_tool_call_variants", [])
            and "message_tool" in native_status_data.get("provider_native_tool_call_variants", [])
            and "message_toolUse" in native_status_data.get("provider_native_tool_call_variants", [])
            and "message_tool_uses" in native_status_data.get("provider_native_tool_call_variants", [])
            and "message_toolUses" in native_status_data.get("provider_native_tool_call_variants", [])
        ...
        native_choice_delta_functioncall_marker = root / "native-choice-delta-functioncall-should-not-run.txt"
        native_choice_delta_functioncall_captured: dict[str, object] = {}

        class NativeOpenAIChoiceDeltaFunctionCallFragmentSmokeResponse:
            def __enter__(self):
                return self
            ...
            def read(self) -> bytes:
                memory_args = json.dumps({"key": "native-choice-delta-functioncall-smoke", "value": "choice delta functionCall fragments translated"})
                run_args = json.dumps({
                    "target": "app.example.test",
                    "purpose": "choice delta functionCall fragment dry-run smoke",
                    "command": f"printf native-choice-delta-functioncall > {native_choice_delta_functioncall_marker}",
                    "execute": True,
                })
                memory_chunks = [memory_args[:24], memory_args[24:77], memory_args[77:]]
                run_chunks = [run_args[:39], run_args[39:119], run_args[119:]]
                return json.dumps({
                    "choices": [
                        {
                            "delta": {
                                "content": "native choice delta functionCall smoke token=native-choice-delta-functioncall-secret",
                                "tool_calls": [
                                    {
                                        "index": 0,
                                        "type": "functionCall",
                                        "functionCall": {"functionName": "remember", "callId": "choice_delta_functioncall_memory", "argsJson": memory_chunks[0]},
                                    },
                                    {
                                        "index": 1,
                                        "type": "function_call",
                                        "function_call": {"toolName": "run_command", "toolCallId": "choice_delta_functioncall_dry", "argumentsJson": run_chunks[0]},
                                    },
                                ],
                            }
                        },
                        {"delta": {"tool_calls": [{"index": 0, "functionCall": {"argsJson": memory_chunks[1]}}, {"index": 1, "function_call": {"argumentsJson": run_chunks[1]}}]}},
                        {"delta": {"tool_calls": [{"index": 0, "functionCall": {"argsJson": memory_chunks[2]}}, {"index": 1, "function_call": {"argumentsJson": run_chunks[2]}}], "content": [{"type": "tool_result", "content": "FUNCTIONCALL_RESULT_SHOULD_NOT_SURFACE"}]}},
                    ]
                }).encode("utf-8")

        def fake_native_choice_delta_functioncall_urlopen(request, timeout=0):
            payload = json.loads(request.data.decode("utf-8"))
            native_choice_delta_functioncall_captured["tool_count"] = len(payload.get("tools", [])) if isinstance(payload.get("tools"), list) else 0
            native_choice_delta_functioncall_captured["tool_choice"] = payload.get("tool_choice")
            return NativeOpenAIChoiceDeltaFunctionCallFragmentSmokeResponse()

        native_choice_delta_functioncall_runtime = PhobosAgentRuntime(
            AgentRuntimeConfig(
                engagement_path=str(engagement_path),
                db_path=str(data / "native-provider-choice-delta-functioncall.db"),
                session_name="native-provider-choice-delta-functioncall-smoke",
                auto_model_planning=True,
            ),
            adapter=OpenAICompatibleAdapter(model="fake-native-choice-delta-functioncall-smoke", base_url="http://127.0.0.1:9/v1"),
        )
        native_choice_delta_functioncall_original_urlopen = model_adapters.urllib.request.urlopen
        try:
            model_adapters.urllib.request.urlopen = fake_native_choice_delta_functioncall_urlopen
            native_choice_delta_functioncall_plan = native_choice_delta_functioncall_runtime.handle_message('/auto model=true prompt="native choice delta functionCall smoke token=native-choice-delta-functioncall-secret"')
            native_choice_delta_functioncall_plan_payload = json.loads(native_choice_delta_functioncall_plan.split("\n", 1)[1])
            native_choice_delta_functioncall_apply = native_choice_delta_functioncall_runtime.handle_message('/auto apply=true model=true prompt="native choice delta functionCall smoke token=native-choice-delta-functioncall-secret"')
            native_choice_delta_functioncall_apply_payload = json.loads(native_choice_delta_functioncall_apply.split("\n", 1)[1])
            native_choice_delta_functioncall_recall = native_choice_delta_functioncall_runtime.handle_message('/recall query=native-choice-delta-functioncall-smoke')
            write("native-provider-choice-delta-functioncall-fragments.json", json.dumps({
                "plan": native_choice_delta_functioncall_plan_payload,
                "apply": native_choice_delta_functioncall_apply_payload,
                "captured": native_choice_delta_functioncall_captured,
                "recall": native_choice_delta_functioncall_recall,
                "marker_exists": native_choice_delta_functioncall_marker.exists(),
            }, indent=2, sort_keys=True))
        finally:
            model_adapters.urllib.request.urlopen = native_choice_delta_functioncall_original_urlopen
            native_choice_delta_functioncall_runtime.close()
        native_choice_delta_functioncall_calls = native_choice_delta_functioncall_plan_payload.get("tool_calls", []) if isinstance(native_choice_delta_functioncall_plan_payload.get("tool_calls"), list) else []
        native_choice_delta_functioncall_metadata = native_choice_delta_functioncall_plan_payload.get("metadata", {}) if isinstance(native_choice_delta_functioncall_plan_payload.get("metadata"), dict) else {}
        native_choice_delta_functioncall_call_metadata = [call.get("metadata", {}) if isinstance(call, dict) else {} for call in native_choice_delta_functioncall_calls]
        native_choice_delta_functioncall_ledger = native_choice_delta_functioncall_apply_payload.get("execution_ledger", []) if isinstance(native_choice_delta_functioncall_apply_payload.get("execution_ledger"), list) else []
        checks["native_provider_choice_delta_function_call_fragment_ok"] = (
            native_choice_delta_functioncall_plan_payload.get("mode") == "plan_only"
            and [call.get("tool") for call in native_choice_delta_functioncall_calls] == ["remember", "run_command"]
            and native_choice_delta_functioncall_calls[0].get("args", {}).get("key") == "native-choice-delta-functioncall-smoke"
            and native_choice_delta_functioncall_calls[0].get("args", {}).get("value") == "choice delta functionCall fragments translated"
            and native_choice_delta_functioncall_calls[1].get("args", {}).get("purpose") == "choice delta functionCall fragment dry-run smoke"
            and native_choice_delta_functioncall_calls[1].get("args", {}).get("execute") is False
            and native_choice_delta_functioncall_metadata.get("native_tool_calls") is True
            and native_choice_delta_functioncall_metadata.get("native_tool_call_count") == 2
            and [item.get("provider_tool_call_id") for item in native_choice_delta_functioncall_call_metadata] == ["choice_delta_functioncall_memory", "choice_delta_functioncall_dry"]
            and [item.get("native_tool_call_source") for item in native_choice_delta_functioncall_call_metadata] == ["native provider choice delta tool_calls nested functionCall", "native provider choice delta tool_calls nested function_call"]
            and [item.get("result", {}).get("status") for item in native_choice_delta_functioncall_apply_payload.get("results", [])] == ["ok", "dry_run"]
            and [item.get("provider_tool_call_id") for item in native_choice_delta_functioncall_ledger] == ["choice_delta_functioncall_memory", "choice_delta_functioncall_dry"]
            and native_choice_delta_functioncall_ledger[1].get("actual_command_or_process_activity") is False
            and native_status_milestone_contract.get("choice_delta_function_call_fragment_assembly") is True
            and "choice_delta_function_call_fragments" in native_status_data.get("provider_native_tool_call_variants", [])
            and "choice delta functionCall fragments translated" in native_choice_delta_functioncall_recall
            and native_choice_delta_functioncall_captured.get("tool_choice") == "auto"
            and int(native_choice_delta_functioncall_captured.get("tool_count", 0) or 0) > 0
            and not native_choice_delta_functioncall_marker.exists()
            and "FUNCTIONCALL_RESULT_SHOULD_NOT_SURFACE" not in native_choice_delta_functioncall_plan + native_choice_delta_functioncall_apply
            and "native-choice-delta-functioncall-secret" not in native_choice_delta_functioncall_plan + native_choice_delta_functioncall_apply + native_choice_delta_functioncall_recall + json.dumps(native_choice_delta_functioncall_plan_payload) + json.dumps(native_choice_delta_functioncall_apply_payload)
        )
                            ...
                            "message": {
                                ...
                                "tool_calls": [
                                    ...
                                    },
                                    {
                                        ...
                                        "tool": {
                                            "name": "remember",
                                            "argumentsJson": {"key": "native-nested-tool-object-smoke", "value": "nested tool object tool_calls translated"},
                                        },
                                    },
                                ],
                            }
        ...
        try:
            ...
            native_nested_tool_recall = native_nested_runtime.handle_message('/recall query=native-nested-tool-object-smoke')
            write("native-provider-nested-tool-call-aliases.json", json.dumps({
                ...
                "tool_recall": native_nested_tool_recall,
        ...
        checks["native_provider_tool_calls_nested_aliases_ok"] = (
            ...
            and [call.get("tool") for call in native_nested_calls] == ["remember", "list_tasks", "run_command", "remember"]
            and "native provider tool_call nested functionCall" in native_nested_calls[0].get("reason", "")
            and "native provider tool_call nested toolUse" in native_nested_calls[1].get("reason", "")
            and "native provider tool_call nested toolUse" in native_nested_calls[2].get("reason", "")
            and "native provider tool_call nested tool" in native_nested_calls[3].get("reason", "")
            and native_nested_calls[2].get("args", {}).get("execute") is False
            and native_nested_metadata.get("native_tool_calls") is True
            and native_nested_metadata.get("native_tool_call_count") == 4
            and [item.get("provider_tool_call_id") for item in native_nested_call_metadata] == ["nested_functioncall_memory", "nested_tooluse_tasks", "nested_tooluse_dry", "nested_tool_object_memory"]
            and [item.get("native_tool_call_source") for item in native_nested_call_metadata] == ["native provider tool_call nested functionCall", "native provider tool_call nested toolUse", "native provider tool_call nested toolUse", "native provider tool_call nested tool"]
            and [item.get("result", {}).get("status") for item in native_nested_apply_payload.get("results", [])] == ["ok", "ok", "dry_run", "ok"]
            and [item.get("provider_tool_call_id") for item in native_nested_ledger] == ["nested_functioncall_memory", "nested_tooluse_tasks", "nested_tooluse_dry", "nested_tool_object_memory"]
            ...
            and "nested tool object tool_calls translated" in native_nested_tool_recall
            ...
            and "native-nested-secret" not in native_nested_plan + native_nested_apply + native_nested_recall + native_nested_tool_recall + json.dumps(native_nested_plan_payload) + json.dumps(native_nested_apply_payload)
        )
        checks["native_provider_tool_call_nested_tool_object_ok"] = (
            checks["native_provider_tool_calls_nested_aliases_ok"]
            and native_nested_calls[3].get("tool") == "remember"
            and native_nested_calls[3].get("args", {}).get("key") == "native-nested-tool-object-smoke"
            and native_nested_call_metadata[3].get("provider_tool_call_id") == "nested_tool_object_memory"
            and native_nested_call_metadata[3].get("native_tool_call_source") == "native provider tool_call nested tool"
            and native_nested_ledger[3].get("actual_command_or_process_activity") is False
            and native_nested_ledger[3].get("execution_state") == "completed_without_command_execution"
        )
        ...
        native_result_envelope_captured = {}
        native_result_envelope_marker = root / "native-result-envelope-should-not-run.txt"
        native_result_envelope_result_marker = "RESULT_ENVELOPE_RESULT_SHOULD_NOT_SURFACE_SMOKE"

        class NativeOpenAIResultEnvelopeSmokeResponse:
            def __enter__(self):
                return self
                ...
                return json.dumps({
                    "result": {
                        "message": {
                            "role": "assistant",
                            "content": [
                                {"type": "text", "text": "native result envelope smoke token=native-result-envelope-secret"},
                                {"type": "tool_result", "content": native_result_envelope_result_marker + " token=native-result-envelope-secret"},
                            ],
                            "tool_calls": [
                                {
                                    "id": "result_envelope_memory",
                                    "type": "function",
                                    "function": {
                                        "name": "remember",
                                        "arguments": json.dumps({"key": "native-result-envelope-smoke", "value": "result envelope wrapper native tool call translated"}),
                                    },
                                },
                                {
                                    "toolCallId": "result_envelope_dry",
                                    "type": "function",
                                    "function": {
                                        "name": "run_command",
                                        "arguments": json.dumps({
                                            "target": "app.example.test",
                                            "purpose": "result envelope native dry-run smoke",
                                            "command": f"printf native-result-envelope > {native_result_envelope_marker}",
                                            "execute": True,
                                        }),
                                    },
                                },
                    ...
                    "metadata": {"note": "outer envelope token=native-result-envelope-secret should not surface"},
                }).encode("utf-8")

        def fake_native_result_envelope_urlopen(request, timeout=0):
            payload = json.loads(request.data.decode("utf-8"))
            native_result_envelope_captured["tool_count"] = len(payload.get("tools", [])) if isinstance(payload.get("tools"), list) else 0
            native_result_envelope_captured["tool_choice"] = payload.get("tool_choice")
            return NativeOpenAIResultEnvelopeSmokeResponse()

        native_result_envelope_runtime = PhobosAgentRuntime(
            AgentRuntimeConfig(
                engagement_path=str(engagement_path),
                db_path=str(data / "native-provider-result-envelope.db"),
                session_name="native-provider-result-envelope-smoke",
                auto_model_planning=True,
            ),
            adapter=OpenAICompatibleAdapter(model="fake-native-result-envelope-smoke", base_url="http://127.0.0.1:9/v1"),
        )
        native_result_envelope_original_urlopen = model_adapters.urllib.request.urlopen
        try:
            model_adapters.urllib.request.urlopen = fake_native_result_envelope_urlopen
            native_result_envelope_plan = native_result_envelope_runtime.handle_message('/auto model=true prompt="native result envelope smoke token=native-result-envelope-secret"')
            native_result_envelope_plan_payload = json.loads(native_result_envelope_plan.split("\n", 1)[1])
            native_result_envelope_apply = native_result_envelope_runtime.handle_message('/auto apply=true model=true prompt="native result envelope smoke token=native-result-envelope-secret"')
            native_result_envelope_apply_payload = json.loads(native_result_envelope_apply.split("\n", 1)[1])
            native_result_envelope_recall = native_result_envelope_runtime.handle_message('/recall query=native-result-envelope-smoke')
            write("native-provider-result-envelope-wrapper.json", json.dumps({
                "plan": native_result_envelope_plan_payload,
                "apply": native_result_envelope_apply_payload,
                "captured": native_result_envelope_captured,
                "recall": native_result_envelope_recall,
                "marker_exists": native_result_envelope_marker.exists(),
            }, indent=2, sort_keys=True))
        finally:
            model_adapters.urllib.request.urlopen = native_result_envelope_original_urlopen
            native_result_envelope_runtime.close()
        native_result_envelope_calls = native_result_envelope_plan_payload.get("tool_calls", []) if isinstance(native_result_envelope_plan_payload.get("tool_calls"), list) else []
        native_result_envelope_metadata = native_result_envelope_plan_payload.get("metadata", {}) if isinstance(native_result_envelope_plan_payload.get("metadata"), dict) else {}
        native_result_envelope_call_metadata = [call.get("metadata", {}) if isinstance(call, dict) else {} for call in native_result_envelope_calls]
        native_result_envelope_ledger = native_result_envelope_apply_payload.get("execution_ledger", []) if isinstance(native_result_envelope_apply_payload.get("execution_ledger"), list) else []
        native_result_envelope_blob = native_result_envelope_plan + native_result_envelope_apply + native_result_envelope_recall + json.dumps(native_result_envelope_plan_payload) + json.dumps(native_result_envelope_apply_payload)
        checks["native_provider_result_envelope_wrapper_ok"] = (
            native_result_envelope_plan_payload.get("mode") == "plan_only"
            and [call.get("tool") for call in native_result_envelope_calls] == ["remember", "run_command"]
            and all("native provider root message tool_calls" in call.get("reason", "") for call in native_result_envelope_calls)
            and native_result_envelope_calls[1].get("args", {}).get("execute") is False
            and native_result_envelope_metadata.get("native_tool_calls") is True
            and native_result_envelope_metadata.get("native_tool_call_count") == 2
            and [item.get("provider_tool_call_id") for item in native_result_envelope_call_metadata] == ["result_envelope_memory", "result_envelope_dry"]
            and [item.get("native_tool_call_source") for item in native_result_envelope_call_metadata] == ["native provider root message tool_calls", "native provider root message tool_calls"]
            and [item.get("result", {}).get("status") for item in native_result_envelope_apply_payload.get("results", [])] == ["ok", "dry_run"]
            and [item.get("provider_tool_call_id") for item in native_result_envelope_ledger] == ["result_envelope_memory", "result_envelope_dry"]
            and native_result_envelope_ledger[1].get("actual_command_or_process_activity") is False
            and native_status_milestone_contract.get("provider_result_envelope_wrapper_translation") is True
            and "provider_result_envelope_wrapper" in native_status_data.get("provider_native_tool_call_variants", [])
            and "tool_result" in json.dumps(native_result_envelope_plan_payload.get("warnings", [])).lower()
            and "result envelope wrapper native tool call translated" in native_result_envelope_recall
            and native_result_envelope_captured.get("tool_choice") == "auto"
            and native_result_envelope_captured.get("tool_count", 0) > 0
            and not native_result_envelope_marker.exists()
            and native_result_envelope_result_marker not in native_result_envelope_blob
            and "native-result-envelope-secret" not in native_result_envelope_blob
        )

        native_data_envelope_captured = {}
        native_data_envelope_marker = root / "native-data-envelope-should-not-run.txt"
        native_data_envelope_result_marker = "DATA_ENVELOPE_RESULT_SHOULD_NOT_SURFACE_SMOKE"

        class NativeOpenAIDataEnvelopeSmokeResponse:
            def __enter__(self):
                return self
                ...
                return json.dumps({
                    "data": {
                        "message": {
                            "role": "assistant",
                            "content": [
                                {"type": "text", "text": "native data envelope smoke token=native-data-envelope-secret"},
                                {"type": "tool_result", "content": native_data_envelope_result_marker + " token=native-data-envelope-secret"},
                            ],
                            "tool_calls": [
                                {
                                    "id": "data_envelope_memory",
                                    "type": "function",
                                    "function": {
                                        "name": "remember",
                                        "arguments": json.dumps({"key": "native-data-envelope-smoke", "value": "data envelope wrapper native tool call translated"}),
                                    },
                                },
                                {
                                    "toolCallId": "data_envelope_dry",
                                    "type": "function",
                                    "function": {
                                        "name": "run_command",
                                        "arguments": json.dumps({
                                            "target": "app.example.test",
                                            "purpose": "data envelope native dry-run smoke",
                                            "command": f"printf native-data-envelope > {native_data_envelope_marker}",
                                            "execute": True,
                                        }),
                                    },
                                },
                    ...
                    "metadata": {"note": "outer envelope token=native-data-envelope-secret should not surface"},
                }).encode("utf-8")

        def fake_native_data_envelope_urlopen(request, timeout=0):
            payload = json.loads(request.data.decode("utf-8"))
            native_data_envelope_captured["tool_count"] = len(payload.get("tools", [])) if isinstance(payload.get("tools"), list) else 0
            native_data_envelope_captured["tool_choice"] = payload.get("tool_choice")
            return NativeOpenAIDataEnvelopeSmokeResponse()

        native_data_envelope_runtime = PhobosAgentRuntime(
            AgentRuntimeConfig(
                engagement_path=str(engagement_path),
                db_path=str(data / "native-provider-data-envelope.db"),
                session_name="native-provider-data-envelope-smoke",
                auto_model_planning=True,
            ),
            adapter=OpenAICompatibleAdapter(model="fake-native-data-envelope-smoke", base_url="http://127.0.0.1:9/v1"),
        )
        native_data_envelope_original_urlopen = model_adapters.urllib.request.urlopen
        try:
            model_adapters.urllib.request.urlopen = fake_native_data_envelope_urlopen
            native_data_envelope_plan = native_data_envelope_runtime.handle_message('/auto model=true prompt="native data envelope smoke token=native-data-envelope-secret"')
            native_data_envelope_plan_payload = json.loads(native_data_envelope_plan.split("\n", 1)[1])
            native_data_envelope_apply = native_data_envelope_runtime.handle_message('/auto apply=true model=true prompt="native data envelope smoke token=native-data-envelope-secret"')
            native_data_envelope_apply_payload = json.loads(native_data_envelope_apply.split("\n", 1)[1])
            native_data_envelope_recall = native_data_envelope_runtime.handle_message('/recall query=native-data-envelope-smoke')
            write("native-provider-data-envelope-wrapper.json", json.dumps({
                "plan": native_data_envelope_plan_payload,
                "apply": native_data_envelope_apply_payload,
                "captured": native_data_envelope_captured,
                "recall": native_data_envelope_recall,
                "marker_exists": native_data_envelope_marker.exists(),
            }, indent=2, sort_keys=True))
        finally:
            model_adapters.urllib.request.urlopen = native_data_envelope_original_urlopen
            native_data_envelope_runtime.close()
        native_data_envelope_calls = native_data_envelope_plan_payload.get("tool_calls", []) if isinstance(native_data_envelope_plan_payload.get("tool_calls"), list) else []
        native_data_envelope_metadata = native_data_envelope_plan_payload.get("metadata", {}) if isinstance(native_data_envelope_plan_payload.get("metadata"), dict) else {}
        native_data_envelope_call_metadata = [call.get("metadata", {}) if isinstance(call, dict) else {} for call in native_data_envelope_calls]
        native_data_envelope_ledger = native_data_envelope_apply_payload.get("execution_ledger", []) if isinstance(native_data_envelope_apply_payload.get("execution_ledger"), list) else []
        native_data_envelope_blob = native_data_envelope_plan + native_data_envelope_apply + native_data_envelope_recall + json.dumps(native_data_envelope_plan_payload) + json.dumps(native_data_envelope_apply_payload)
        checks["native_provider_data_envelope_wrapper_ok"] = (
            native_data_envelope_plan_payload.get("mode") == "plan_only"
            and [call.get("tool") for call in native_data_envelope_calls] == ["remember", "run_command"]
            and all("native provider root message tool_calls" in call.get("reason", "") for call in native_data_envelope_calls)
            and native_data_envelope_calls[1].get("args", {}).get("execute") is False
            and native_data_envelope_metadata.get("native_tool_calls") is True
            and native_data_envelope_metadata.get("native_tool_call_count") == 2
            and [item.get("provider_tool_call_id") for item in native_data_envelope_call_metadata] == ["data_envelope_memory", "data_envelope_dry"]
            and [item.get("native_tool_call_source") for item in native_data_envelope_call_metadata] == ["native provider root message tool_calls", "native provider root message tool_calls"]
            and [item.get("result", {}).get("status") for item in native_data_envelope_apply_payload.get("results", [])] == ["ok", "dry_run"]
            and [item.get("provider_tool_call_id") for item in native_data_envelope_ledger] == ["data_envelope_memory", "data_envelope_dry"]
            and native_data_envelope_ledger[1].get("actual_command_or_process_activity") is False
            and native_status_milestone_contract.get("provider_data_envelope_wrapper_translation") is True
            and "provider_data_envelope_wrapper" in native_status_data.get("provider_native_tool_call_variants", [])
            and "tool_result" in json.dumps(native_data_envelope_plan_payload.get("warnings", [])).lower()
            and "data envelope wrapper native tool call translated" in native_data_envelope_recall
            and native_data_envelope_captured.get("tool_choice") == "auto"
            and native_data_envelope_captured.get("tool_count", 0) > 0
        ...
        native_root_predictions_captured = {}
        native_root_predictions_marker = root / "native-root-predictions-should-not-run.txt"
        native_root_predictions_result_marker = "ROOT_PREDICTIONS_RESULT_SHOULD_NOT_SURFACE_SMOKE"

        class NativeOpenAIRootPredictionsSmokeResponse:
            def __enter__(self):
                return self
                    ...
                    "predictions": [
                        ...
                        {"role": "tool", "content": native_root_predictions_result_marker + " token=native-root-predictions-secret"},
                        {
                            "message": {
                                "role": "assistant",
                                "content": [
                                    {"type": "text", "text": "native root predictions smoke token=native-root-predictions-secret"},
                                    {"functionResponse": {"name": "remember", "response": {"content": native_root_predictions_result_marker + " trailing token=native-root-predictions-secret"}}},
                                ],
                                "tool_calls": [
                                    {
                                        "id": "root_predictions_memory",
                                        "type": "function",
                                        "function": {
                                            "name": "remember",
                                            "arguments": json.dumps({"key": "native-root-predictions-smoke", "value": "root predictions wrapper native tool call translated"}),
                                        },
                                    },
                                    {
                                        "toolCallId": "root_predictions_dry",
                                        "type": "function",
                                        "function": {
                                            "name": "run_command",
                                            "arguments": json.dumps({
                                                "target": "app.example.test",
                                                "purpose": "root predictions native dry-run smoke",
                                                "command": f"printf native-root-predictions > {native_root_predictions_marker}",
                                                "execute": True,
                                            }),
                                        },
                                    },
        ...
        def fake_native_root_predictions_urlopen(request, timeout=0):
            payload = json.loads(request.data.decode("utf-8"))
            native_root_predictions_captured["tool_count"] = len(payload.get("tools", [])) if isinstance(payload.get("tools"), list) else 0
            native_root_predictions_captured["tool_choice"] = payload.get("tool_choice")
            return NativeOpenAIRootPredictionsSmokeResponse()

        native_root_predictions_runtime = PhobosAgentRuntime(
            AgentRuntimeConfig(
                engagement_path=str(engagement_path),
                db_path=str(data / "native-provider-root-predictions.db"),
                session_name="native-provider-root-predictions-smoke",
                auto_model_planning=True,
            ),
            adapter=OpenAICompatibleAdapter(model="fake-native-root-predictions-smoke", base_url="http://127.0.0.1:9/v1"),
        )
        native_root_predictions_original_urlopen = model_adapters.urllib.request.urlopen
        try:
            model_adapters.urllib.request.urlopen = fake_native_root_predictions_urlopen
            native_root_predictions_plan = native_root_predictions_runtime.handle_message('/auto model=true prompt="native root predictions smoke token=native-root-predictions-secret"')
            native_root_predictions_plan_payload = json.loads(native_root_predictions_plan.split("\n", 1)[1])
            native_root_predictions_apply = native_root_predictions_runtime.handle_message('/auto apply=true model=true prompt="native root predictions smoke token=native-root-predictions-secret"')
            native_root_predictions_apply_payload = json.loads(native_root_predictions_apply.split("\n", 1)[1])
            native_root_predictions_recall = native_root_predictions_runtime.handle_message('/recall query=native-root-predictions-smoke')
            write("native-provider-root-predictions-tool-calls.json", json.dumps({
                "plan": native_root_predictions_plan_payload,
                "apply": native_root_predictions_apply_payload,
                "captured": native_root_predictions_captured,
                "recall": native_root_predictions_recall,
                "marker_exists": native_root_predictions_marker.exists(),
            }, indent=2, sort_keys=True))
        finally:
            model_adapters.urllib.request.urlopen = native_root_predictions_original_urlopen
            native_root_predictions_runtime.close()
        native_root_predictions_calls = native_root_predictions_plan_payload.get("tool_calls", []) if isinstance(native_root_predictions_plan_payload.get("tool_calls"), list) else []
        native_root_predictions_metadata = native_root_predictions_plan_payload.get("metadata", {}) if isinstance(native_root_predictions_plan_payload.get("metadata"), dict) else {}
        native_root_predictions_call_metadata = [call.get("metadata", {}) if isinstance(call, dict) else {} for call in native_root_predictions_calls]
        native_root_predictions_ledger = native_root_predictions_apply_payload.get("execution_ledger", []) if isinstance(native_root_predictions_apply_payload.get("execution_ledger"), list) else []
        native_root_predictions_blob = native_root_predictions_plan + native_root_predictions_apply + native_root_predictions_recall + json.dumps(native_root_predictions_plan_payload) + json.dumps(native_root_predictions_apply_payload)
        checks["native_provider_root_predictions_wrapper_ok"] = (
            native_root_predictions_plan_payload.get("mode") == "plan_only"
            and [call.get("tool") for call in native_root_predictions_calls] == ["remember", "run_command"]
            and all("native provider root predictions tool_calls" in call.get("reason", "") for call in native_root_predictions_calls)
            and native_root_predictions_calls[0].get("args", {}).get("key") == "native-root-predictions-smoke"
            and native_root_predictions_calls[1].get("args", {}).get("execute") is False
            and native_root_predictions_metadata.get("native_tool_calls") is True
            and native_root_predictions_metadata.get("native_tool_call_count") == 2
            and [item.get("provider_tool_call_id") for item in native_root_predictions_call_metadata] == ["root_predictions_memory", "root_predictions_dry"]
            and [item.get("native_tool_call_source") for item in native_root_predictions_call_metadata] == ["native provider root predictions tool_calls", "native provider root predictions tool_calls"]
            and [item.get("result", {}).get("status") for item in native_root_predictions_apply_payload.get("results", [])] == ["ok", "dry_run"]
            and [item.get("provider_tool_call_id") for item in native_root_predictions_ledger] == ["root_predictions_memory", "root_predictions_dry"]
            and native_root_predictions_ledger[1].get("actual_command_or_process_activity") is False
            and native_status_milestone_contract.get("root_predictions_wrapper_translation") is True
            and "root_predictions_tool_calls" in native_status_data.get("provider_native_tool_call_variants", [])
        ...
        native_root_outputs_marker = root / "native-root-outputs-should-not-run.txt"
        native_root_outputs_result_marker = "ROOT_OUTPUTS_RESULT_SHOULD_NOT_SURFACE_SMOKE"

        class NativeOpenAIRootOutputsSmokeResponse:
            def __enter__(self):
                return self
                    ...
                    "outputs": [
                        {
                            ...
                            "tool_calls": [
                                {
                                    "id": "root_outputs_old_memory",
                                    "type": "function",
                                    "function": {
                                        "name": "remember",
                                        "arguments": json.dumps({"key": "native-root-outputs-old-smoke", "value": "old root outputs call should not dispatch"}),
                                    },
                                }
                            ],
                        },
                        {"role": "tool", "content": native_root_outputs_result_marker + " token=native-root-outputs-secret"},
                        {
                            "message": {
                                "role": "assistant",
                                "content": [
                                    {"type": "text", "text": "native root outputs smoke token=native-root-outputs-secret"},
                                    {"functionResponse": {"name": "remember", "response": {"content": native_root_outputs_result_marker + " trailing token=native-root-outputs-secret"}}},
                                ],
                                "tool_calls": [
                                    {
                                        "id": "root_outputs_memory",
                                        "type": "function",
                                        "function": {
                                            "name": "remember",
                                            "arguments": json.dumps({"key": "native-root-outputs-smoke", "value": "root outputs wrapper native tool call translated"}),
                                        },
                                    },
                                    {
                                        "toolCallId": "root_outputs_dry",
                                        "type": "function",
                                        "function": {
                                            "name": "run_command",
                                            "arguments": json.dumps({
                                                "target": "app.example.test",
                                                "purpose": "root outputs native dry-run smoke",
                                                "command": f"printf native-root-outputs > {native_root_outputs_marker}",
                                                "execute": True,
                                            }),
                                        },
                                    },
        ...
        def fake_native_root_outputs_urlopen(request, timeout=0):
            payload = json.loads(request.data.decode("utf-8"))
            native_root_outputs_captured["tool_count"] = len(payload.get("tools", [])) if isinstance(payload.get("tools"), list) else 0
            native_root_outputs_captured["tool_choice"] = payload.get("tool_choice")
            return NativeOpenAIRootOutputsSmokeResponse()

        native_root_outputs_runtime = PhobosAgentRuntime(
            AgentRuntimeConfig(
                engagement_path=str(engagement_path),
                db_path=str(data / "native-provider-root-outputs.db"),
                session_name="native-provider-root-outputs-smoke",
                auto_model_planning=True,
            ),
            adapter=OpenAICompatibleAdapter(model="fake-native-root-outputs-smoke", base_url="http://127.0.0.1:9/v1"),
        )
        native_root_outputs_original_urlopen = model_adapters.urllib.request.urlopen
        try:
            model_adapters.urllib.request.urlopen = fake_native_root_outputs_urlopen
            native_root_outputs_plan = native_root_outputs_runtime.handle_message('/auto model=true prompt="native root outputs smoke token=native-root-outputs-secret"')
            native_root_outputs_plan_payload = json.loads(native_root_outputs_plan.split("\n", 1)[1])
            native_root_outputs_apply = native_root_outputs_runtime.handle_message('/auto apply=true model=true prompt="native root outputs smoke token=native-root-outputs-secret"')
            native_root_outputs_apply_payload = json.loads(native_root_outputs_apply.split("\n", 1)[1])
            native_root_outputs_recall = native_root_outputs_runtime.handle_message('/recall query=native-root-outputs-smoke')
            write("native-provider-root-outputs-tool-calls.json", json.dumps({
                "plan": native_root_outputs_plan_payload,
                "apply": native_root_outputs_apply_payload,
                "captured": native_root_outputs_captured,
                "recall": native_root_outputs_recall,
                "marker_exists": native_root_outputs_marker.exists(),
            }, indent=2, sort_keys=True))
        finally:
            model_adapters.urllib.request.urlopen = native_root_outputs_original_urlopen
            native_root_outputs_runtime.close()
        native_root_outputs_calls = native_root_outputs_plan_payload.get("tool_calls", []) if isinstance(native_root_outputs_plan_payload.get("tool_calls"), list) else []
        native_root_outputs_metadata = native_root_outputs_plan_payload.get("metadata", {}) if isinstance(native_root_outputs_plan_payload.get("metadata"), dict) else {}
        native_root_outputs_call_metadata = [call.get("metadata", {}) if isinstance(call, dict) else {} for call in native_root_outputs_calls]
        native_root_outputs_ledger = native_root_outputs_apply_payload.get("execution_ledger", []) if isinstance(native_root_outputs_apply_payload.get("execution_ledger"), list) else []
        native_root_outputs_blob = native_root_outputs_plan + native_root_outputs_apply + native_root_outputs_recall + json.dumps(native_root_outputs_plan_payload) + json.dumps(native_root_outputs_apply_payload)
        checks["native_provider_root_outputs_wrapper_ok"] = (
            native_root_outputs_plan_payload.get("mode") == "plan_only"
            and [call.get("tool") for call in native_root_outputs_calls] == ["remember", "run_command"]
            and all("native provider root outputs tool_calls" in call.get("reason", "") for call in native_root_outputs_calls)
            and native_root_outputs_calls[0].get("args", {}).get("key") == "native-root-outputs-smoke"
            and native_root_outputs_calls[1].get("args", {}).get("execute") is False
            and native_root_outputs_metadata.get("native_tool_calls") is True
            and native_root_outputs_metadata.get("native_tool_call_count") == 2
            and [item.get("provider_tool_call_id") for item in native_root_outputs_call_metadata] == ["root_outputs_memory", "root_outputs_dry"]
            and [item.get("native_tool_call_source") for item in native_root_outputs_call_metadata] == ["native provider root outputs tool_calls", "native provider root outputs tool_calls"]
            and [item.get("result", {}).get("status") for item in native_root_outputs_apply_payload.get("results", [])] == ["ok", "dry_run"]
            and [item.get("provider_tool_call_id") for item in native_root_outputs_ledger] == ["root_outputs_memory", "root_outputs_dry"]
            and native_root_outputs_ledger[1].get("actual_command_or_process_activity") is False
            and native_status_milestone_contract.get("root_outputs_wrapper_translation") is True
            and "root_outputs_tool_calls" in native_status_data.get("provider_native_tool_call_variants", [])
            and "functionResponse" in json.dumps(native_root_outputs_plan_payload.get("warnings", []))
            and "root outputs wrapper native tool call translated" in native_root_outputs_recall
            and "old root outputs call should not dispatch" not in native_root_outputs_recall
            and native_root_outputs_captured.get("tool_choice") == "auto"
            and native_root_outputs_captured.get("tool_count", 0) > 0
        ...
        native_root_outputs_direct_captured = {}
        native_root_outputs_direct_marker = root / "native-root-outputs-direct-should-not-run.txt"
        native_root_outputs_direct_result_marker = "ROOT_OUTPUTS_DIRECT_RESULT_SHOULD_NOT_SURFACE_SMOKE"

        class NativeOpenAIRootOutputsDirectSmokeResponse:
            def __enter__(self):
                return self
                            ...
                            "tool_calls": [
                                {
                                    ...
                                    "function": {
                                        "name": "remember",
                                        "arguments": json.dumps({"key": "native-root-outputs-direct-old-smoke", "value": "old root outputs direct call should not dispatch"}),
                                    },
                                }
                            ],
                        },
                        {"role": "tool", "content": native_root_outputs_direct_result_marker + " token=native-root-outputs-direct-secret"},
                        {
                            "type": "function_call",
                            "call_id": "root_outputs_direct_memory",
                            "name": "remember",
                            "arguments": json.dumps({"key": "native-root-outputs-direct-smoke", "value": "root outputs direct function_call native tool call translated"}),
                        },
                        {
                            "type": "function",
                            ...
                            "function": {
                                ...
                                "arguments": json.dumps({
                                    "target": "app.example.test",
                                    "purpose": "root outputs direct native dry-run smoke",
                                    "command": f"printf native-root-outputs-direct > {native_root_outputs_direct_marker}",
                                    "execute": True,
                                }),
                            },
                        },
        ...
        def fake_native_root_outputs_direct_urlopen(request, timeout=0):
            payload = json.loads(request.data.decode("utf-8"))
            native_root_outputs_direct_captured["tool_count"] = len(payload.get("tools", [])) if isinstance(payload.get("tools"), list) else 0
            native_root_outputs_direct_captured["tool_choice"] = payload.get("tool_choice")
            return NativeOpenAIRootOutputsDirectSmokeResponse()

        native_root_outputs_direct_runtime = PhobosAgentRuntime(
            AgentRuntimeConfig(
                engagement_path=str(engagement_path),
                db_path=str(data / "native-provider-root-outputs-direct.db"),
                session_name="native-provider-root-outputs-direct-smoke",
                auto_model_planning=True,
            ),
            adapter=OpenAICompatibleAdapter(model="fake-native-root-outputs-direct-smoke", base_url="http://127.0.0.1:9/v1"),
        )
        native_root_outputs_direct_original_urlopen = model_adapters.urllib.request.urlopen
        try:
            model_adapters.urllib.request.urlopen = fake_native_root_outputs_direct_urlopen
            native_root_outputs_direct_plan = native_root_outputs_direct_runtime.handle_message('/auto model=true prompt="native root outputs direct smoke token=native-root-outputs-direct-secret"')
            native_root_outputs_direct_plan_payload = json.loads(native_root_outputs_direct_plan.split("\n", 1)[1])
            native_root_outputs_direct_apply = native_root_outputs_direct_runtime.handle_message('/auto apply=true model=true prompt="native root outputs direct smoke token=native-root-outputs-direct-secret"')
            native_root_outputs_direct_apply_payload = json.loads(native_root_outputs_direct_apply.split("\n", 1)[1])
            native_root_outputs_direct_recall = native_root_outputs_direct_runtime.handle_message('/recall query=native-root-outputs-direct-smoke')
            write("native-provider-root-outputs-direct-function-calls.json", json.dumps({
                "plan": native_root_outputs_direct_plan_payload,
                "apply": native_root_outputs_direct_apply_payload,
                "captured": native_root_outputs_direct_captured,
                "recall": native_root_outputs_direct_recall,
                "marker_exists": native_root_outputs_direct_marker.exists(),
            }, indent=2, sort_keys=True))
        finally:
            model_adapters.urllib.request.urlopen = native_root_outputs_direct_original_urlopen
            native_root_outputs_direct_runtime.close()
        native_root_outputs_direct_calls = native_root_outputs_direct_plan_payload.get("tool_calls", []) if isinstance(native_root_outputs_direct_plan_payload.get("tool_calls"), list) else []
        native_root_outputs_direct_call_metadata = [call.get("metadata", {}) if isinstance(call, dict) else {} for call in native_root_outputs_direct_calls]
        native_root_outputs_direct_ledger = native_root_outputs_direct_apply_payload.get("execution_ledger", []) if isinstance(native_root_outputs_direct_apply_payload.get("execution_ledger"), list) else []
        native_root_outputs_direct_blob = native_root_outputs_direct_plan + native_root_outputs_direct_apply + native_root_outputs_direct_recall + json.dumps(native_root_outputs_direct_plan_payload) + json.dumps(native_root_outputs_direct_apply_payload)
        checks["native_provider_root_outputs_direct_function_call_ok"] = (
            native_root_outputs_direct_plan_payload.get("mode") == "plan_only"
            and [call.get("tool") for call in native_root_outputs_direct_calls] == ["remember", "run_command"]
            and [item.get("provider_tool_call_id") for item in native_root_outputs_direct_call_metadata] == ["root_outputs_direct_memory", "root_outputs_direct_dry"]
            and [item.get("native_tool_call_source") for item in native_root_outputs_direct_call_metadata] == ["native provider root outputs function_call", "native provider root outputs function"]
            and native_root_outputs_direct_calls[0].get("args", {}).get("key") == "native-root-outputs-direct-smoke"
            and native_root_outputs_direct_calls[1].get("args", {}).get("execute") is False
            and [item.get("result", {}).get("status") for item in native_root_outputs_direct_apply_payload.get("results", [])] == ["ok", "dry_run"]
            and [item.get("provider_tool_call_id") for item in native_root_outputs_direct_ledger] == ["root_outputs_direct_memory", "root_outputs_direct_dry"]
            and native_root_outputs_direct_ledger[1].get("actual_command_or_process_activity") is False
            and native_status_milestone_contract.get("root_outputs_direct_tool_call_translation") is True
            and "root_outputs_function_call" in native_status_data.get("provider_native_tool_call_variants", [])
            and "root_outputs_function" in native_status_data.get("provider_native_tool_call_variants", [])
            and "root outputs direct function_call native tool call translated" in native_root_outputs_direct_recall
            and "old root outputs direct call should not dispatch" not in native_root_outputs_direct_recall
            and native_root_outputs_direct_captured.get("tool_choice") == "auto"
            and native_root_outputs_direct_captured.get("tool_count", 0) > 0
            and not native_root_outputs_direct_marker.exists()
            and native_root_outputs_direct_result_marker not in native_root_outputs_direct_blob
            and "native-root-outputs-direct-secret" not in native_root_outputs_direct_blob
        )

        native_root_output_items_captured = {}
        native_root_output_items_marker = root / "native-root-output-items-should-not-run.txt"
        native_root_output_items_result_marker = "ROOT_OUTPUT_ITEMS_RESULT_SHOULD_NOT_SURFACE_SMOKE"

        class NativeOpenAIRootOutputItemsSmokeResponse:
            def __enter__(self):
                return self
                    ...
                    "output_items": [
                        {
                            ...
                            "tool_calls": [
                                {
                                    "id": "root_output_items_old_memory",
                                    "type": "function",
                                    "function": {
                                        "name": "remember",
                                        "arguments": json.dumps({"key": "native-root-output-items-old-smoke", "value": "old root output_items call should not dispatch"}),
                                    },
                                }
                            ],
                        },
                        {"role": "tool", "content": native_root_output_items_result_marker + " token=native-root-output-items-secret"},
                        {
                            "type": "function_call",
                            "call_id": "root_output_items_memory",
                            "name": "remember",
                            "arguments": json.dumps({"key": "native-root-output-items-smoke", "value": "root output_items function_call native tool call translated"}),
                        },
                        {
                            "type": "function",
                            "id": "root_output_items_dry",
                            "function": {
                                "name": "run_command",
                                "arguments": json.dumps({
                                    "target": "app.example.test",
                                    "purpose": "root output_items native dry-run smoke",
                                    "command": f"printf native-root-output-items > {native_root_output_items_marker}",
                                    "execute": True,
                                }),
                            },
                        },
        ...
        def fake_native_root_output_items_urlopen(request, timeout=0):
            payload = json.loads(request.data.decode("utf-8"))
            native_root_output_items_captured["tool_count"] = len(payload.get("tools", [])) if isinstance(payload.get("tools"), list) else 0
            native_root_output_items_captured["tool_choice"] = payload.get("tool_choice")
            return NativeOpenAIRootOutputItemsSmokeResponse()

        native_root_output_items_runtime = PhobosAgentRuntime(
            AgentRuntimeConfig(
                engagement_path=str(engagement_path),
                db_path=str(data / "native-provider-root-output-items.db"),
                session_name="native-provider-root-output-items-smoke",
                auto_model_planning=True,
            ),
            adapter=OpenAICompatibleAdapter(model="fake-native-root-output-items-smoke", base_url="http://127.0.0.1:9/v1"),
        )
        native_root_output_items_original_urlopen = model_adapters.urllib.request.urlopen
        try:
            model_adapters.urllib.request.urlopen = fake_native_root_output_items_urlopen
            native_root_output_items_plan = native_root_output_items_runtime.handle_message('/auto model=true prompt="native root output_items smoke token=native-root-output-items-secret"')
            native_root_output_items_plan_payload = json.loads(native_root_output_items_plan.split("\n", 1)[1])
            native_root_output_items_apply = native_root_output_items_runtime.handle_message('/auto apply=true model=true prompt="native root output_items smoke token=native-root-output-items-secret"')
            native_root_output_items_apply_payload = json.loads(native_root_output_items_apply.split("\n", 1)[1])
            native_root_output_items_recall = native_root_output_items_runtime.handle_message('/recall query=native-root-output-items-smoke')
            write("native-provider-root-output-items-function-calls.json", json.dumps({
                "plan": native_root_output_items_plan_payload,
                "apply": native_root_output_items_apply_payload,
                "captured": native_root_output_items_captured,
                "recall": native_root_output_items_recall,
                "marker_exists": native_root_output_items_marker.exists(),
            }, indent=2, sort_keys=True))
        finally:
            model_adapters.urllib.request.urlopen = native_root_output_items_original_urlopen
            native_root_output_items_runtime.close()
        native_root_output_items_calls = native_root_output_items_plan_payload.get("tool_calls", []) if isinstance(native_root_output_items_plan_payload.get("tool_calls"), list) else []
        native_root_output_items_call_metadata = [call.get("metadata", {}) if isinstance(call, dict) else {} for call in native_root_output_items_calls]
        native_root_output_items_ledger = native_root_output_items_apply_payload.get("execution_ledger", []) if isinstance(native_root_output_items_apply_payload.get("execution_ledger"), list) else []
        native_root_output_items_blob = native_root_output_items_plan + native_root_output_items_apply + native_root_output_items_recall + json.dumps(native_root_output_items_plan_payload) + json.dumps(native_root_output_items_apply_payload)
        checks["native_provider_root_output_items_wrapper_ok"] = (
            native_root_output_items_plan_payload.get("mode") == "plan_only"
            and [call.get("tool") for call in native_root_output_items_calls] == ["remember", "run_command"]
            and [item.get("provider_tool_call_id") for item in native_root_output_items_call_metadata] == ["root_output_items_memory", "root_output_items_dry"]
            and [item.get("native_tool_call_source") for item in native_root_output_items_call_metadata] == ["native provider root output_items function_call", "native provider root output_items function"]
            and native_root_output_items_calls[0].get("args", {}).get("key") == "native-root-output-items-smoke"
            and native_root_output_items_calls[1].get("args", {}).get("execute") is False
            and [item.get("result", {}).get("status") for item in native_root_output_items_apply_payload.get("results", [])] == ["ok", "dry_run"]
            and [item.get("provider_tool_call_id") for item in native_root_output_items_ledger] == ["root_output_items_memory", "root_output_items_dry"]
            and native_root_output_items_ledger[1].get("actual_command_or_process_activity") is False
            and native_status_milestone_contract.get("root_output_items_wrapper_translation") is True
            and "root_output_items_function_call" in native_status_data.get("provider_native_tool_call_variants", [])
            and "root_output_items_function" in native_status_data.get("provider_native_tool_call_variants", [])
            and "root output_items function_call native tool call translated" in native_root_output_items_recall
            and "old root output_items call should not dispatch" not in native_root_output_items_recall
            and native_root_output_items_captured.get("tool_choice") == "auto"
            and native_root_output_items_captured.get("tool_count", 0) > 0
            and not native_root_output_items_marker.exists()
            and native_root_output_items_result_marker not in native_root_output_items_blob
        ...
        class NativeOpenAIRootOutputItemSingularSmokeResponse:
            def __init__(self, wrapper_key: str):
                self.wrapper_key = wrapper_key
            ...
            def read(self) -> bytes:
                if self.wrapper_key == "outputItem":
                    return json.dumps({
                        "outputItem": {
                            "type": "function",
                            ...
                            "function": {
                                ...
                                "arguments": json.dumps({
                                    "target": "app.example.test",
                                    "purpose": "root outputItem singular native dry-run smoke",
                                    "command": f"printf native-root-output-item-singular > {native_root_output_item_singular_marker}",
                                    "execute": True,
                                }),
                            },
                        }
                ...
                return json.dumps({
                    "output_item": {
                        ...
                        "name": "remember",
                        "arguments": json.dumps({"key": "native-root-output-item-singular-smoke", "value": "root output_item singular function_call native tool call translated"}),
                    }
                }).encode("utf-8")

        def fake_native_root_output_item_singular_urlopen(request, timeout=0):
            payload = json.loads(request.data.decode("utf-8"))
            payload_blob = json.dumps(payload)
            wrapper_key = "outputItem" if "native root outputItem singular dry" in payload_blob else "output_item"
            bucket = "dry" if wrapper_key == "outputItem" else "memory"
            native_root_output_item_singular_captured[bucket] = {
                "wrapper": wrapper_key,
                "tool_count": len(payload.get("tools", [])) if isinstance(payload.get("tools"), list) else 0,
                "tool_choice": payload.get("tool_choice"),
            }
            return NativeOpenAIRootOutputItemSingularSmokeResponse(wrapper_key)

        native_root_output_item_singular_runtime = PhobosAgentRuntime(
            AgentRuntimeConfig(
                engagement_path=str(engagement_path),
                db_path=str(data / "native-provider-root-output-item-singular.db"),
                session_name="native-provider-root-output-item-singular-smoke",
                auto_model_planning=True,
            ),
            adapter=OpenAICompatibleAdapter(model="fake-native-root-output-item-singular-smoke", base_url="http://127.0.0.1:9/v1"),
        )
        native_root_output_item_singular_original_urlopen = model_adapters.urllib.request.urlopen
        try:
            model_adapters.urllib.request.urlopen = fake_native_root_output_item_singular_urlopen
            native_root_output_item_singular_plan = native_root_output_item_singular_runtime.handle_message('/auto model=true prompt="native root output_item singular smoke token=native-root-output-item-singular-secret"')
            native_root_output_item_singular_plan_payload = json.loads(native_root_output_item_singular_plan.split("\n", 1)[1])
            native_root_output_item_singular_apply = native_root_output_item_singular_runtime.handle_message('/auto apply=true model=true prompt="native root output_item singular smoke token=native-root-output-item-singular-secret"')
            native_root_output_item_singular_apply_payload = json.loads(native_root_output_item_singular_apply.split("\n", 1)[1])
            native_root_output_item_singular_dry_plan = native_root_output_item_singular_runtime.handle_message('/auto model=true prompt="native root outputItem singular dry token=native-root-output-item-singular-secret"')
            native_root_output_item_singular_dry_plan_payload = json.loads(native_root_output_item_singular_dry_plan.split("\n", 1)[1])
            native_root_output_item_singular_dry_apply = native_root_output_item_singular_runtime.handle_message('/auto apply=true model=true prompt="native root outputItem singular dry token=native-root-output-item-singular-secret"')
            native_root_output_item_singular_dry_apply_payload = json.loads(native_root_output_item_singular_dry_apply.split("\n", 1)[1])
            native_root_output_item_singular_recall = native_root_output_item_singular_runtime.handle_message('/recall query=native-root-output-item-singular-smoke')
            write("native-provider-root-output-item-singular-wrapper.json", json.dumps({
                "plan": native_root_output_item_singular_plan_payload,
                "apply": native_root_output_item_singular_apply_payload,
                "dry_plan": native_root_output_item_singular_dry_plan_payload,
                "dry_apply": native_root_output_item_singular_dry_apply_payload,
                "captured": native_root_output_item_singular_captured,
                "recall": native_root_output_item_singular_recall,
                "marker_exists": native_root_output_item_singular_marker.exists(),
            }, indent=2, sort_keys=True))
        finally:
            model_adapters.urllib.request.urlopen = native_root_output_item_singular_original_urlopen
            native_root_output_item_singular_runtime.close()
        native_root_output_item_singular_calls = native_root_output_item_singular_plan_payload.get("tool_calls", []) if isinstance(native_root_output_item_singular_plan_payload.get("tool_calls"), list) else []
        native_root_output_item_singular_dry_calls = native_root_output_item_singular_dry_plan_payload.get("tool_calls", []) if isinstance(native_root_output_item_singular_dry_plan_payload.get("tool_calls"), list) else []
        native_root_output_item_singular_call_metadata = [call.get("metadata", {}) if isinstance(call, dict) else {} for call in native_root_output_item_singular_calls]
        native_root_output_item_singular_dry_call_metadata = [call.get("metadata", {}) if isinstance(call, dict) else {} for call in native_root_output_item_singular_dry_calls]
        native_root_output_item_singular_ledger = native_root_output_item_singular_apply_payload.get("execution_ledger", []) if isinstance(native_root_output_item_singular_apply_payload.get("execution_ledger"), list) else []
        native_root_output_item_singular_dry_ledger = native_root_output_item_singular_dry_apply_payload.get("execution_ledger", []) if isinstance(native_root_output_item_singular_dry_apply_payload.get("execution_ledger"), list) else []
        native_root_output_item_singular_blob = native_root_output_item_singular_plan + native_root_output_item_singular_apply + native_root_output_item_singular_dry_plan + native_root_output_item_singular_dry_apply + native_root_output_item_singular_recall + json.dumps(native_root_output_item_singular_plan_payload) + json.dumps(native_root_output_item_singular_dry_plan_payload)
        checks["native_provider_root_output_item_singular_wrapper_ok"] = (
            native_root_output_item_singular_plan_payload.get("mode") == "plan_only"
            and native_root_output_item_singular_dry_plan_payload.get("mode") == "plan_only"
            and [call.get("tool") for call in native_root_output_item_singular_calls] == ["remember"]
            and [call.get("tool") for call in native_root_output_item_singular_dry_calls] == ["run_command"]
            and [item.get("provider_tool_call_id") for item in native_root_output_item_singular_call_metadata] == ["root_output_item_memory"]
            and [item.get("provider_tool_call_id") for item in native_root_output_item_singular_dry_call_metadata] == ["root_outputItem_dry"]
            and [item.get("native_tool_call_source") for item in native_root_output_item_singular_call_metadata] == ["native provider root output_item function_call"]
            and [item.get("native_tool_call_source") for item in native_root_output_item_singular_dry_call_metadata] == ["native provider root outputItem function"]
            and native_root_output_item_singular_calls[0].get("args", {}).get("key") == "native-root-output-item-singular-smoke"
            and native_root_output_item_singular_dry_calls[0].get("args", {}).get("execute") is False
            and [item.get("result", {}).get("status") for item in native_root_output_item_singular_apply_payload.get("results", [])] == ["ok"]
            and [item.get("result", {}).get("status") for item in native_root_output_item_singular_dry_apply_payload.get("results", [])] == ["dry_run"]
            and [item.get("provider_tool_call_id") for item in native_root_output_item_singular_ledger] == ["root_output_item_memory"]
            and [item.get("provider_tool_call_id") for item in native_root_output_item_singular_dry_ledger] == ["root_outputItem_dry"]
            and native_root_output_item_singular_dry_ledger[0].get("actual_command_or_process_activity") is False
            and native_status_milestone_contract.get("root_output_item_singular_wrapper_translation") is True
            and "root_output_item_function_call" in native_status_data.get("provider_native_tool_call_variants", [])
            and "root_outputItem_function" in native_status_data.get("provider_native_tool_call_variants", [])
            and "root output_item singular function_call native tool call translated" in native_root_output_item_singular_recall
            and native_root_output_item_singular_captured.get("memory", {}).get("tool_choice") == "auto"
            and native_root_output_item_singular_captured.get("dry", {}).get("tool_choice") == "auto"
            and native_root_output_item_singular_captured.get("memory", {}).get("tool_count", 0) > 0
            and native_root_output_item_singular_captured.get("dry", {}).get("tool_count", 0) > 0
            and not native_root_output_item_singular_marker.exists()
            and "native-root-output-item-singular-secret" not in native_root_output_item_singular_blob
        )

        native_root_items_captured = {}
        native_root_items_marker = root / "native-root-items-should-not-run.txt"
        native_root_items_result_marker = "ROOT_ITEMS_RESULT_SHOULD_NOT_SURFACE_SMOKE"

        class NativeOpenAIRootItemsSmokeResponse:
            def __enter__(self):
                return self
                    ...
                    "items": [
                        {
                            ...
                            "tool_calls": [
                                {
                                    "id": "root_items_old_memory",
                                    "type": "function",
                                    "function": {
                                        "name": "remember",
                                        "arguments": json.dumps({"key": "native-root-items-old-smoke", "value": "old root items call should not dispatch"}),
                                    },
                                }
                            ],
                        },
                        {"role": "tool", "content": native_root_items_result_marker + " token=native-root-items-secret"},
                        {
                            "type": "function_call",
                            "call_id": "root_items_memory",
                            "name": "remember",
                            "arguments": json.dumps({"key": "native-root-items-smoke", "value": "root items function_call native tool call translated"}),
                        },
                        {
                            "type": "function",
                            "id": "root_items_dry",
                            "function": {
                                ...
                                "arguments": json.dumps({
                                    "target": "app.example.test",
                                    "purpose": "root items native dry-run smoke",
                                    "command": f"printf native-root-items > {native_root_items_marker}",
                                    "execute": True,
                                }),
                            },
                        },
        ...
        def fake_native_root_items_urlopen(request, timeout=0):
            payload = json.loads(request.data.decode("utf-8"))
            native_root_items_captured["tool_count"] = len(payload.get("tools", [])) if isinstance(payload.get("tools"), list) else 0
            native_root_items_captured["tool_choice"] = payload.get("tool_choice")
            return NativeOpenAIRootItemsSmokeResponse()

        native_root_items_runtime = PhobosAgentRuntime(
            AgentRuntimeConfig(
                engagement_path=str(engagement_path),
                db_path=str(data / "native-provider-root-items.db"),
                session_name="native-provider-root-items-smoke",
                auto_model_planning=True,
            ),
            adapter=OpenAICompatibleAdapter(model="fake-native-root-items-smoke", base_url="http://127.0.0.1:9/v1"),
        )
        native_root_items_original_urlopen = model_adapters.urllib.request.urlopen
        try:
            model_adapters.urllib.request.urlopen = fake_native_root_items_urlopen
            native_root_items_plan = native_root_items_runtime.handle_message('/auto model=true prompt="native root items smoke token=native-root-items-secret"')
            native_root_items_plan_payload = json.loads(native_root_items_plan.split("\n", 1)[1])
            native_root_items_apply = native_root_items_runtime.handle_message('/auto apply=true model=true prompt="native root items smoke token=native-root-items-secret"')
            native_root_items_apply_payload = json.loads(native_root_items_apply.split("\n", 1)[1])
            native_root_items_recall = native_root_items_runtime.handle_message('/recall query=native-root-items-smoke')
            write("native-provider-root-items-function-calls.json", json.dumps({
                "plan": native_root_items_plan_payload,
                "apply": native_root_items_apply_payload,
                "captured": native_root_items_captured,
                "recall": native_root_items_recall,
                "marker_exists": native_root_items_marker.exists(),
            }, indent=2, sort_keys=True))
        finally:
            model_adapters.urllib.request.urlopen = native_root_items_original_urlopen
            native_root_items_runtime.close()
        native_root_items_calls = native_root_items_plan_payload.get("tool_calls", []) if isinstance(native_root_items_plan_payload.get("tool_calls"), list) else []
        native_root_items_call_metadata = [call.get("metadata", {}) if isinstance(call, dict) else {} for call in native_root_items_calls]
        native_root_items_ledger = native_root_items_apply_payload.get("execution_ledger", []) if isinstance(native_root_items_apply_payload.get("execution_ledger"), list) else []
        native_root_items_blob = native_root_items_plan + native_root_items_apply + native_root_items_recall + json.dumps(native_root_items_plan_payload) + json.dumps(native_root_items_apply_payload)
        checks["native_provider_root_items_wrapper_ok"] = (
            native_root_items_plan_payload.get("mode") == "plan_only"
            and [call.get("tool") for call in native_root_items_calls] == ["remember", "run_command"]
            and [item.get("provider_tool_call_id") for item in native_root_items_call_metadata] == ["root_items_memory", "root_items_dry"]
            and [item.get("native_tool_call_source") for item in native_root_items_call_metadata] == ["native provider root items function_call", "native provider root items function"]
            and native_root_items_calls[0].get("args", {}).get("key") == "native-root-items-smoke"
            and native_root_items_calls[1].get("args", {}).get("execute") is False
            and [item.get("result", {}).get("status") for item in native_root_items_apply_payload.get("results", [])] == ["ok", "dry_run"]
            and [item.get("provider_tool_call_id") for item in native_root_items_ledger] == ["root_items_memory", "root_items_dry"]
            and native_root_items_ledger[1].get("actual_command_or_process_activity") is False
            and native_status_milestone_contract.get("root_items_wrapper_translation") is True
            and "root_items_function_call" in native_status_data.get("provider_native_tool_call_variants", [])
            and "root_items_function" in native_status_data.get("provider_native_tool_call_variants", [])
            and "root items function_call native tool call translated" in native_root_items_recall
            and "old root items call should not dispatch" not in native_root_items_recall
            and native_root_items_captured.get("tool_choice") == "auto"
            and native_root_items_captured.get("tool_count", 0) > 0
            and not native_root_items_marker.exists()
            and native_root_items_result_marker not in native_root_items_blob
            and "native-root-items-secret" not in native_root_items_blob
        )

        native_root_item_singular_captured = {"memory": {}, "dry": {}}
        native_root_item_singular_marker = root / "native-root-item-singular-should-not-run.txt"

        class NativeOpenAIRootItemSingularSmokeResponse:
            ...
            def read(self) -> bytes:
                    ...
                    return json.dumps({
                        "item": {
                            ...
                            "function": {
                                ...
                                "arguments": json.dumps({
                                    "target": "app.example.test",
                                    "purpose": "root item singular native dry-run smoke",
                                    "command": f"printf native-root-item-singular > {native_root_item_singular_marker}",
                                    "execute": True,
                                }),
                            },
                        }
                ...
                return json.dumps({
                    "item": {
                        "type": "function_call",
                        "call_id": "root_item_memory",
                        "name": "remember",
                        "arguments": json.dumps({"key": "native-root-item-singular-smoke", "value": "root item singular function_call native tool call translated"}),
                    }
                }).encode("utf-8")

        def fake_native_root_item_singular_urlopen(request, timeout=0):
            payload = json.loads(request.data.decode("utf-8"))
            payload_blob = json.dumps(payload)
            kind = "dry" if "native root item singular dry" in payload_blob else "memory"
            native_root_item_singular_captured[kind] = {
                "tool_count": len(payload.get("tools", [])) if isinstance(payload.get("tools"), list) else 0,
                "tool_choice": payload.get("tool_choice"),
            }
            return NativeOpenAIRootItemSingularSmokeResponse(kind)

        native_root_item_singular_runtime = PhobosAgentRuntime(
            AgentRuntimeConfig(
                engagement_path=str(engagement_path),
                db_path=str(data / "native-provider-root-item-singular.db"),
                session_name="native-provider-root-item-singular-smoke",
                auto_model_planning=True,
            ),
            adapter=OpenAICompatibleAdapter(model="fake-native-root-item-singular-smoke", base_url="http://127.0.0.1:9/v1"),
        )
        native_root_item_singular_original_urlopen = model_adapters.urllib.request.urlopen
        try:
            model_adapters.urllib.request.urlopen = fake_native_root_item_singular_urlopen
            native_root_item_singular_plan = native_root_item_singular_runtime.handle_message('/auto model=true prompt="native root item singular smoke token=native-root-item-singular-secret"')
            native_root_item_singular_plan_payload = json.loads(native_root_item_singular_plan.split("\n", 1)[1])
            native_root_item_singular_apply = native_root_item_singular_runtime.handle_message('/auto apply=true model=true prompt="native root item singular smoke token=native-root-item-singular-secret"')
            native_root_item_singular_apply_payload = json.loads(native_root_item_singular_apply.split("\n", 1)[1])
            native_root_item_singular_dry_plan = native_root_item_singular_runtime.handle_message('/auto model=true prompt="native root item singular dry token=native-root-item-singular-secret"')
            native_root_item_singular_dry_plan_payload = json.loads(native_root_item_singular_dry_plan.split("\n", 1)[1])
            native_root_item_singular_dry_apply = native_root_item_singular_runtime.handle_message('/auto apply=true model=true prompt="native root item singular dry token=native-root-item-singular-secret"')
            native_root_item_singular_dry_apply_payload = json.loads(native_root_item_singular_dry_apply.split("\n", 1)[1])
            native_root_item_singular_recall = native_root_item_singular_runtime.handle_message('/recall query=native-root-item-singular-smoke')
            write("native-provider-root-item-singular-wrapper.json", json.dumps({
                "plan": native_root_item_singular_plan_payload,
                "apply": native_root_item_singular_apply_payload,
                "dry_plan": native_root_item_singular_dry_plan_payload,
                "dry_apply": native_root_item_singular_dry_apply_payload,
                "captured": native_root_item_singular_captured,
                "recall": native_root_item_singular_recall,
                "marker_exists": native_root_item_singular_marker.exists(),
            }, indent=2, sort_keys=True))
        finally:
            model_adapters.urllib.request.urlopen = native_root_item_singular_original_urlopen
            native_root_item_singular_runtime.close()
        native_root_item_singular_calls = native_root_item_singular_plan_payload.get("tool_calls", []) if isinstance(native_root_item_singular_plan_payload.get("tool_calls"), list) else []
        native_root_item_singular_dry_calls = native_root_item_singular_dry_plan_payload.get("tool_calls", []) if isinstance(native_root_item_singular_dry_plan_payload.get("tool_calls"), list) else []
        native_root_item_singular_call_metadata = [call.get("metadata", {}) if isinstance(call, dict) else {} for call in native_root_item_singular_calls]
        native_root_item_singular_dry_call_metadata = [call.get("metadata", {}) if isinstance(call, dict) else {} for call in native_root_item_singular_dry_calls]
        native_root_item_singular_ledger = native_root_item_singular_apply_payload.get("execution_ledger", []) if isinstance(native_root_item_singular_apply_payload.get("execution_ledger"), list) else []
        native_root_item_singular_dry_ledger = native_root_item_singular_dry_apply_payload.get("execution_ledger", []) if isinstance(native_root_item_singular_dry_apply_payload.get("execution_ledger"), list) else []
        native_root_item_singular_blob = native_root_item_singular_plan + native_root_item_singular_apply + native_root_item_singular_dry_plan + native_root_item_singular_dry_apply + native_root_item_singular_recall + json.dumps(native_root_item_singular_plan_payload) + json.dumps(native_root_item_singular_dry_plan_payload)
        checks["native_provider_root_item_singular_wrapper_ok"] = (
            native_root_item_singular_plan_payload.get("mode") == "plan_only"
            and native_root_item_singular_dry_plan_payload.get("mode") == "plan_only"
            and [call.get("tool") for call in native_root_item_singular_calls] == ["remember"]
            and [call.get("tool") for call in native_root_item_singular_dry_calls] == ["run_command"]
            and [item.get("provider_tool_call_id") for item in native_root_item_singular_call_metadata] == ["root_item_memory"]
            and [item.get("provider_tool_call_id") for item in native_root_item_singular_dry_call_metadata] == ["root_item_dry"]
            and [item.get("native_tool_call_source") for item in native_root_item_singular_call_metadata] == ["native provider root item function_call"]
            and [item.get("native_tool_call_source") for item in native_root_item_singular_dry_call_metadata] == ["native provider root item function"]
            and native_root_item_singular_calls[0].get("args", {}).get("key") == "native-root-item-singular-smoke"
            and native_root_item_singular_dry_calls[0].get("args", {}).get("execute") is False
            and [item.get("result", {}).get("status") for item in native_root_item_singular_apply_payload.get("results", [])] == ["ok"]
            and [item.get("result", {}).get("status") for item in native_root_item_singular_dry_apply_payload.get("results", [])] == ["dry_run"]
            and [item.get("provider_tool_call_id") for item in native_root_item_singular_ledger] == ["root_item_memory"]
            and [item.get("provider_tool_call_id") for item in native_root_item_singular_dry_ledger] == ["root_item_dry"]
            and native_root_item_singular_dry_ledger[0].get("actual_command_or_process_activity") is False
            and native_status_milestone_contract.get("root_item_singular_wrapper_translation") is True
            and "root_item_function_call" in native_status_data.get("provider_native_tool_call_variants", [])
            and "root_item_function" in native_status_data.get("provider_native_tool_call_variants", [])
            and "root item singular function_call native tool call translated" in native_root_item_singular_recall
            and native_root_item_singular_captured.get("memory", {}).get("tool_choice") == "auto"
            and native_root_item_singular_captured.get("dry", {}).get("tool_choice") == "auto"
            and native_root_item_singular_captured.get("memory", {}).get("tool_count", 0) > 0
            and native_root_item_singular_captured.get("dry", {}).get("tool_count", 0) > 0
            and not native_root_item_singular_marker.exists()
            and "native-root-item-singular-secret" not in native_root_item_singular_blob
        )
        ...
        native_root_tool_use_captured = {}
        native_root_tool_use_result_marker = "ROOT_TOOL_USE_RESPONSE_SHOULD_NOT_SURFACE_SMOKE"

        class NativeOpenAIRootToolUseSmokeResponse:
            def __enter__(self):
                return self
            ...
            def read(self) -> bytes:
                return json.dumps({
                    "content": "native root toolUse smoke token=native-root-tool-use-secret",
                    "tool_use": {
                        "tool_use_id": "root_tool_use_memory",
                        "name": "remember",
                        "input": {"key": "native-root-tool-use-smoke", "value": "root tool_use native tool call translated"},
                    },
                    "toolUse": {
                    ...
                    "tool": {
                        ...
                        "toolName": "remember",
                        "inputJson": {"key": "native-root-tool-smoke", "value": "root neutral tool native tool call translated"},
                    },
                    "tool_uses": [
                        {
                            "tool_call_id": "root_tool_uses_memory",
                            "name": "remember",
                            "input": {"key": "native-root-tool-uses-smoke", "value": "root tool_uses native tool call translated"},
                        }
                    ],
                    "toolUses": [
                        {
                    ...
                    "functionResponse": {
                        "name": "remember",
                        "response": {"content": native_root_tool_use_result_marker + " token=native-root-tool-use-secret"},
                    },
                }).encode("utf-8")

        def fake_native_root_tool_use_urlopen(request, timeout=0):
            payload = json.loads(request.data.decode("utf-8"))
            native_root_tool_use_captured["tool_count"] = len(payload.get("tools", [])) if isinstance(payload.get("tools"), list) else 0
            native_root_tool_use_captured["tool_choice"] = payload.get("tool_choice")
            return NativeOpenAIRootToolUseSmokeResponse()

        native_root_tool_use_runtime = PhobosAgentRuntime(
            AgentRuntimeConfig(
                engagement_path=str(engagement_path),
                db_path=str(data / "native-provider-root-tool-use-aliases.db"),
                session_name="native-provider-root-tool-use-aliases-smoke",
                auto_model_planning=True,
            ),
            adapter=OpenAICompatibleAdapter(model="fake-native-root-tool-use-smoke", base_url="http://127.0.0.1:9/v1"),
        )
        native_root_tool_use_original_urlopen = model_adapters.urllib.request.urlopen
        try:
            model_adapters.urllib.request.urlopen = fake_native_root_tool_use_urlopen
            native_root_tool_use_plan = native_root_tool_use_runtime.handle_message('/auto model=true prompt="native root toolUse smoke token=native-root-tool-use-secret"')
            native_root_tool_use_plan_payload = json.loads(native_root_tool_use_plan.split("\n", 1)[1])
            native_root_tool_use_apply = native_root_tool_use_runtime.handle_message('/auto apply=true model=true prompt="native root toolUse smoke token=native-root-tool-use-secret"')
            native_root_tool_use_apply_payload = json.loads(native_root_tool_use_apply.split("\n", 1)[1])
            native_root_tool_use_recall_one = native_root_tool_use_runtime.handle_message('/recall query=native-root-tool-use-smoke')
            native_root_tool_use_recall_two = native_root_tool_use_runtime.handle_message('/recall query=native-root-tool-uses-smoke')
            native_root_tool_use_recall_three = native_root_tool_use_runtime.handle_message('/recall query=native-root-tool-smoke')
            native_root_tool_use_recall = native_root_tool_use_recall_one + "\n" + native_root_tool_use_recall_two + "\n" + native_root_tool_use_recall_three
            write("native-provider-root-tool-use-aliases.json", json.dumps({
                "plan": native_root_tool_use_plan_payload,
                "apply": native_root_tool_use_apply_payload,
                "captured": native_root_tool_use_captured,
                "recall": native_root_tool_use_recall,
            }, indent=2, sort_keys=True))
        finally:
            model_adapters.urllib.request.urlopen = native_root_tool_use_original_urlopen
            native_root_tool_use_runtime.close()
        native_root_tool_use_calls = native_root_tool_use_plan_payload.get("tool_calls", []) if isinstance(native_root_tool_use_plan_payload.get("tool_calls"), list) else []
        native_root_tool_use_metadata = native_root_tool_use_plan_payload.get("metadata", {}) if isinstance(native_root_tool_use_plan_payload.get("metadata"), dict) else {}
        native_root_tool_use_call_metadata = [call.get("metadata", {}) if isinstance(call, dict) else {} for call in native_root_tool_use_calls]
        native_root_tool_use_sources = [item.get("native_tool_call_source") for item in native_root_tool_use_call_metadata]
        native_root_tool_use_ids = [item.get("provider_tool_call_id") for item in native_root_tool_use_call_metadata]
        native_root_tool_use_ledger = native_root_tool_use_apply_payload.get("execution_ledger", []) if isinstance(native_root_tool_use_apply_payload.get("execution_ledger"), list) else []
        native_root_tool_use_variants = native_status_data.get("provider_native_tool_call_variants", [])
        checks["native_provider_root_tool_use_aliases_ok"] = (
            native_root_tool_use_plan_payload.get("mode") == "plan_only"
            and [call.get("tool") for call in native_root_tool_use_calls] == ["remember", "list_tasks", "remember", "list_tasks", "remember"]
            and native_root_tool_use_metadata.get("native_tool_calls") is True
            and native_root_tool_use_metadata.get("native_tool_call_count") == 5
            and native_root_tool_use_sources == ["native provider root tool_use", "native provider root toolUse", "native provider root tool_uses", "native provider root toolUses", "native provider root tool"]
            and native_root_tool_use_ids == ["root_tool_use_memory", "root_toolUse_tasks", "root_tool_uses_memory", "root_toolUses_tasks", "root_tool_memory"]
            and [item.get("result", {}).get("status") for item in native_root_tool_use_apply_payload.get("results", [])] == ["ok"] * 5
            and [item.get("native_tool_call_source") for item in native_root_tool_use_ledger] == native_root_tool_use_sources
            and native_status_milestone_contract.get("root_tool_use_alias_translation") is True
            and native_status_milestone_contract.get("root_neutral_tool_alias_translation") is True
            and all(variant in native_root_tool_use_variants for variant in ["root_tool_use", "root_toolUse", "root_tool", "root_tool_uses", "root_toolUses"])
            and "tool_result" in json.dumps(native_root_tool_use_plan_payload.get("warnings", [])).lower()
            and "root tool_use native tool call translated" in native_root_tool_use_recall
            and "root neutral tool native tool call translated" in native_root_tool_use_recall
            and "root tool_uses native tool call translated" in native_root_tool_use_recall
            and native_root_tool_use_captured.get("tool_choice") == "auto"
            and native_root_tool_use_captured.get("tool_count", 0) > 0
            and native_root_tool_use_result_marker not in native_root_tool_use_plan + native_root_tool_use_apply + native_root_tool_use_recall + json.dumps(native_root_tool_use_plan_payload) + json.dumps(native_root_tool_use_apply_payload)
            and "native-root-tool-use-secret" not in native_root_tool_use_plan + native_root_tool_use_apply + native_root_tool_use_recall + json.dumps(native_root_tool_use_plan_payload) + json.dumps(native_root_tool_use_apply_payload)
        )

        native_message_tool_use_captured = {}
        native_message_tool_use_result_marker = "MESSAGE_TOOL_USE_RESPONSE_SHOULD_NOT_SURFACE_SMOKE"

        class NativeOpenAIMessageToolUseSmokeResponse:
            def __enter__(self):
                return self
            ...
            def read(self) -> bytes:
                return json.dumps({
                    "choices": [
                        {
                            "message": {
                                "content": "native message toolUse smoke token=native-message-tool-use-secret",
                                "tool_use": {
                                    "tool_use_id": "message_tool_use_memory",
                                    "name": "remember",
                                    "input": {"key": "native-message-tool-use-smoke", "value": "message tool_use native tool call translated"},
                                },
                                "toolUse": {
                                ...
                                "tool": {
                                    ...
                                    "toolName": "remember",
                                    "inputJson": {"key": "native-message-tool-smoke", "value": "message neutral tool native tool call translated"},
                                },
                                "tool_uses": [
                                    {
                                        "tool_call_id": "message_tool_uses_memory",
                                        "name": "remember",
                                        "input": {"key": "native-message-tool-uses-smoke", "value": "message tool_uses native tool call translated"},
                                    }
                                ],
                                "toolUses": [
                                    {
                                ...
                                "functionResponse": {
                                    "name": "remember",
                                    "response": {"content": native_message_tool_use_result_marker + " token=native-message-tool-use-secret"},
                                },
                            }
                        }
                    ]
        ...
        def fake_native_message_tool_use_urlopen(request, timeout=0):
            payload = json.loads(request.data.decode("utf-8"))
            native_message_tool_use_captured["tool_count"] = len(payload.get("tools", [])) if isinstance(payload.get("tools"), list) else 0
            native_message_tool_use_captured["tool_choice"] = payload.get("tool_choice")
            return NativeOpenAIMessageToolUseSmokeResponse()

        native_message_tool_use_runtime = PhobosAgentRuntime(
            AgentRuntimeConfig(
                engagement_path=str(engagement_path),
                db_path=str(data / "native-provider-message-tool-use-aliases.db"),
                session_name="native-provider-message-tool-use-aliases-smoke",
                auto_model_planning=True,
            ),
            adapter=OpenAICompatibleAdapter(model="fake-native-message-tool-use-smoke", base_url="http://127.0.0.1:9/v1"),
        )
        native_message_tool_use_original_urlopen = model_adapters.urllib.request.urlopen
        try:
            model_adapters.urllib.request.urlopen = fake_native_message_tool_use_urlopen
            native_message_tool_use_plan = native_message_tool_use_runtime.handle_message('/auto model=true prompt="native message toolUse smoke token=native-message-tool-use-secret"')
            native_message_tool_use_plan_payload = json.loads(native_message_tool_use_plan.split("\n", 1)[1])
            native_message_tool_use_apply = native_message_tool_use_runtime.handle_message('/auto apply=true model=true prompt="native message toolUse smoke token=native-message-tool-use-secret"')
            native_message_tool_use_apply_payload = json.loads(native_message_tool_use_apply.split("\n", 1)[1])
            native_message_tool_use_recall_one = native_message_tool_use_runtime.handle_message('/recall query=native-message-tool-use-smoke')
            native_message_tool_use_recall_two = native_message_tool_use_runtime.handle_message('/recall query=native-message-tool-uses-smoke')
            native_message_tool_use_recall_three = native_message_tool_use_runtime.handle_message('/recall query=native-message-tool-smoke')
            native_message_tool_use_recall = native_message_tool_use_recall_one + "\n" + native_message_tool_use_recall_two + "\n" + native_message_tool_use_recall_three
            write("native-provider-message-tool-use-aliases.json", json.dumps({
                "plan": native_message_tool_use_plan_payload,
                "apply": native_message_tool_use_apply_payload,
                "captured": native_message_tool_use_captured,
                "recall": native_message_tool_use_recall,
            }, indent=2, sort_keys=True))
        finally:
            model_adapters.urllib.request.urlopen = native_message_tool_use_original_urlopen
            native_message_tool_use_runtime.close()
        native_message_tool_use_calls = native_message_tool_use_plan_payload.get("tool_calls", []) if isinstance(native_message_tool_use_plan_payload.get("tool_calls"), list) else []
        native_message_tool_use_metadata = native_message_tool_use_plan_payload.get("metadata", {}) if isinstance(native_message_tool_use_plan_payload.get("metadata"), dict) else {}
        native_message_tool_use_call_metadata = [call.get("metadata", {}) if isinstance(call, dict) else {} for call in native_message_tool_use_calls]
        native_message_tool_use_sources = [item.get("native_tool_call_source") for item in native_message_tool_use_call_metadata]
        native_message_tool_use_ids = [item.get("provider_tool_call_id") for item in native_message_tool_use_call_metadata]
        native_message_tool_use_ledger = native_message_tool_use_apply_payload.get("execution_ledger", []) if isinstance(native_message_tool_use_apply_payload.get("execution_ledger"), list) else []
        native_message_tool_use_variants = native_status_data.get("provider_native_tool_call_variants", [])
        checks["native_provider_message_tool_use_aliases_ok"] = (
            native_message_tool_use_plan_payload.get("mode") == "plan_only"
            and [call.get("tool") for call in native_message_tool_use_calls] == ["remember", "list_tasks", "remember", "list_tasks", "remember"]
            and native_message_tool_use_metadata.get("native_tool_calls") is True
            and native_message_tool_use_metadata.get("native_tool_call_count") == 5
            and native_message_tool_use_sources == ["native provider message tool_use", "native provider message toolUse", "native provider message tool_uses", "native provider message toolUses", "native provider message tool"]
            and native_message_tool_use_ids == ["message_tool_use_memory", "message_toolUse_tasks", "message_tool_uses_memory", "message_toolUses_tasks", "message_tool_memory"]
            and [item.get("result", {}).get("status") for item in native_message_tool_use_apply_payload.get("results", [])] == ["ok"] * 5
            and [item.get("native_tool_call_source") for item in native_message_tool_use_ledger] == native_message_tool_use_sources
            and native_status_milestone_contract.get("message_tool_use_alias_translation") is True
            and native_status_milestone_contract.get("message_neutral_tool_alias_translation") is True
            and all(variant in native_message_tool_use_variants for variant in ["message_tool_use", "message_toolUse", "message_tool", "message_tool_uses", "message_toolUses"])
            and "message tool_use native tool call translated" in native_message_tool_use_recall
            and "message neutral tool native tool call translated" in native_message_tool_use_recall
            and "message tool_uses native tool call translated" in native_message_tool_use_recall
            and native_message_tool_use_captured.get("tool_choice") == "auto"
            and native_message_tool_use_captured.get("tool_count", 0) > 0
            and native_message_tool_use_result_marker not in native_message_tool_use_plan + native_message_tool_use_apply + native_message_tool_use_recall + json.dumps(native_message_tool_use_plan_payload) + json.dumps(native_message_tool_use_apply_payload)
            and "native-message-tool-use-secret" not in native_message_tool_use_plan + native_message_tool_use_apply + native_message_tool_use_recall + json.dumps(native_message_tool_use_plan_payload) + json.dumps(native_message_tool_use_apply_payload)
        )
        checks["native_provider_neutral_tool_aliases_ok"] = (
            checks.get("native_provider_root_tool_use_aliases_ok") is True
            and checks.get("native_provider_message_tool_use_aliases_ok") is True
            and native_root_tool_use_calls[-1].get("tool") == "remember"
            and native_root_tool_use_calls[-1].get("args", {}).get("key") == "native-root-tool-smoke"
            and native_root_tool_use_call_metadata[-1].get("native_tool_call_source") == "native provider root tool"
            and native_message_tool_use_calls[-1].get("tool") == "remember"
            and native_message_tool_use_calls[-1].get("args", {}).get("key") == "native-message-tool-smoke"
            and native_message_tool_use_call_metadata[-1].get("native_tool_call_source") == "native provider message tool"
        )
        ...
        class NativeAnthropicEndpointSmokeResponse:
            def __enter__(self):
                return self
            ...
            def read(self) -> bytes:
                return json.dumps({
                    "type": "message",
                    "content": [
                        {"type": "text", "text": "native Anthropic smoke token=native-anthropic-secret"},
                        {"type": "tool_use", "id": "anthropic_memory", "name": "remember", "input": {"key": "native-anthropic-smoke", "value": "Anthropic Messages native tool call translated"}},
                        {
                            "type": "tool_use",
                            "id": "anthropic_dry",
                            "name": "run_command",
                            "input": {
                                "target": "app.example.test",
                                "purpose": "Anthropic Messages dry-run smoke",
                                "command": f"printf native-anthropic > {native_anthropic_marker}",
                                "execute": True,
                            },
                        },
                        {"type": "tool_result", "tool_use_id": "anthropic_result", "content": native_anthropic_result_marker + " token=native-anthropic-secret"},
                    ],
                }).encode("utf-8")

        def fake_native_anthropic_urlopen(request, timeout=0):
            native_anthropic_captured.append({
                "url": request.full_url,
                "payload": json.loads(request.data.decode("utf-8")),
                "headers": dict(request.header_items()),
            })
            return NativeAnthropicEndpointSmokeResponse()

        native_anthropic_runtime = PhobosAgentRuntime(
            AgentRuntimeConfig(
                engagement_path=str(engagement_path),
                db_path=str(data / "native-anthropic-endpoint.db"),
                session_name="native-anthropic-endpoint-smoke",
                auto_model_planning=True,
            ),
            adapter=AnthropicMessagesAdapter(
                model="fake-anthropic-smoke-model",
                base_url="http://127.0.0.1:9/v1",
                key_env="PHOBOS_SMOKE_NO_SUCH_ANTHROPIC_KEY",
            ),
        )
        native_anthropic_original_urlopen = model_adapters.urllib.request.urlopen
        try:
            model_adapters.urllib.request.urlopen = fake_native_anthropic_urlopen
            native_anthropic_plan = native_anthropic_runtime.handle_message('/auto model=true prompt="native Anthropic smoke token=native-anthropic-secret"')
            native_anthropic_plan_payload = json.loads(native_anthropic_plan.split("\n", 1)[1])
            native_anthropic_apply = native_anthropic_runtime.handle_message('/auto apply=true model=true prompt="native Anthropic smoke token=native-anthropic-secret"')
            native_anthropic_apply_payload = json.loads(native_anthropic_apply.split("\n", 1)[1])
            native_anthropic_recall = native_anthropic_runtime.handle_message('/recall query=native-anthropic-smoke')
            write("native-anthropic-endpoint-tool-calls.json", json.dumps({
                "plan": native_anthropic_plan_payload,
                "apply": native_anthropic_apply_payload,
                "captured": native_anthropic_captured,
                "recall": native_anthropic_recall,
                "marker_exists": native_anthropic_marker.exists(),
            }, indent=2, sort_keys=True))
        finally:
            model_adapters.urllib.request.urlopen = native_anthropic_original_urlopen
            native_anthropic_runtime.close()
        native_anthropic_calls = native_anthropic_plan_payload.get("tool_calls", []) if isinstance(native_anthropic_plan_payload.get("tool_calls"), list) else []
        native_anthropic_metadata = native_anthropic_plan_payload.get("metadata", {}) if isinstance(native_anthropic_plan_payload.get("metadata"), dict) else {}
        native_anthropic_ledger = native_anthropic_apply_payload.get("execution_ledger", []) if isinstance(native_anthropic_apply_payload.get("execution_ledger"), list) else []
        native_anthropic_first = native_anthropic_captured[0] if native_anthropic_captured else {"url": "", "payload": {}, "headers": {}}
        native_anthropic_payload = native_anthropic_first.get("payload", {}) if isinstance(native_anthropic_first.get("payload"), dict) else {}
        native_anthropic_headers = {str(key).lower(): value for key, value in native_anthropic_first.get("headers", {}).items()} if isinstance(native_anthropic_first.get("headers"), dict) else {}
        native_anthropic_tools = native_anthropic_payload.get("tools", []) if isinstance(native_anthropic_payload.get("tools"), list) else []
        native_anthropic_tool_names = [item.get("name") for item in native_anthropic_tools if isinstance(item, dict)]
        native_anthropic_outputs = native_anthropic_plan + native_anthropic_apply + native_anthropic_recall + json.dumps(native_anthropic_plan_payload) + json.dumps(native_anthropic_apply_payload)
        checks["native_anthropic_adapter_ok"] = (
            native_anthropic_plan_payload.get("mode") == "plan_only"
            and [call.get("tool") for call in native_anthropic_calls] == ["remember", "run_command"]
            and native_anthropic_calls[1].get("args", {}).get("execute") is False
            and native_anthropic_metadata.get("provider") == "anthropic"
            and native_anthropic_metadata.get("native_tool_calls") is True
            and native_anthropic_metadata.get("native_tool_call_count") == 2
            and native_anthropic_first.get("url", "").endswith("/messages")
            and "messages" in native_anthropic_payload
            and "system" in native_anthropic_payload
            and "input" not in native_anthropic_payload
            and native_anthropic_payload.get("tool_choice") == {"type": "auto"}
            and "anthropic-version" in native_anthropic_headers
            and "x-api-key" not in native_anthropic_headers
            and "remember" in native_anthropic_tool_names
            and "approve" not in native_anthropic_tool_names
            and "deny" not in native_anthropic_tool_names
            and all("input_schema" in item for item in native_anthropic_tools if isinstance(item, dict))
            and all("parameters" not in item for item in native_anthropic_tools if isinstance(item, dict))
            and [item.get("native_tool_call_source") for item in native_anthropic_ledger] == ["native provider anthropic messages content tool_use", "native provider anthropic messages content tool_use"]
            and [item.get("provider_tool_call_id") for item in native_anthropic_ledger] == ["anthropic_memory", "anthropic_dry"]
            and [item.get("result", {}).get("status") for item in native_anthropic_apply_payload.get("results", [])] == ["ok", "dry_run"]
            and native_anthropic_ledger[1].get("execution_state") == "dry_run_not_executed"
            and native_anthropic_ledger[1].get("actual_command_or_process_activity") is False
            and native_status_milestone_contract.get("anthropic_messages_api_planning") is True
            and "anthropic_messages_api" in native_status_data.get("provider_native_tool_call_variants", [])
            and "anthropic_messages_tool_use" in native_status_data.get("provider_native_tool_call_variants", [])
            and "Anthropic Messages native tool call translated" in native_anthropic_recall
            and not native_anthropic_marker.exists()
            and native_anthropic_result_marker not in native_anthropic_outputs
        ...
        native_bedrock_converse_stream_marker = root / "native-bedrock-converse-stream-should-not-run.txt"
        native_bedrock_converse_stream_captured = {}
        native_bedrock_converse_stream_result_marker = "BEDROCK_CONVERSE_STREAM_RESULT_SHOULD_NOT_SURFACE_SMOKE"

        class NativeBedrockConverseStreamSmokeResponse:
            def __enter__(self):
                return self
            ...
            def read(self) -> bytes:
                return json.dumps({
                    "stream": [
                        {"messageStart": {"message": {"role": "assistant", "content": []}}},
                        {"contentBlockStart": {"contentBlockIndex": 0, "start": {"toolUse": {"toolUseId": "bedrock_converse_stream_smoke_memory", "name": "remember"}}}},
                        {"contentBlockDelta": {"contentBlockIndex": 0, "delta": {"toolUse": {"input": "{\"key\":\"native-bedrock-converse-stream-smoke\","}}}},
                        {"contentBlockDelta": {"contentBlockIndex": 0, "delta": {"toolUse": {"input": "\"value\":\"Bedrock ConverseStream toolUse wrappers translated\"}"}}}},
                        {"contentBlockStop": {"contentBlockIndex": 0}},
                        {"contentBlockStart": {"contentBlockIndex": 1, "start": {"toolUse": {"toolUseId": "bedrock_converse_stream_smoke_dry", "toolName": "run_command"}}}},
                        {"contentBlockDelta": {"contentBlockIndex": 1, "delta": {"toolUse": {"input": json.dumps({"target": "app.example.test", "purpose": "Bedrock ConverseStream dry-run smoke", "command": f"printf native-bedrock-converse-stream > {native_bedrock_converse_stream_marker}", "execute": True})}}}},
                        {"contentBlockStop": {"contentBlockIndex": 1}},
                        {"contentBlockStart": {"contentBlockIndex": 2, "start": {"toolResult": {"toolUseId": "bedrock_converse_stream_smoke_result", "content": native_bedrock_converse_stream_result_marker + " token=native-bedrock-converse-stream-secret"}}}},
                        {"contentBlockStop": {"contentBlockIndex": 2}},
                        {"messageStop": {"stopReason": "tool_use"}},
                    ],
                }).encode("utf-8")

        def fake_native_bedrock_converse_stream_urlopen(request, timeout=0):
            payload = json.loads(request.data.decode("utf-8"))
            native_bedrock_converse_stream_captured["tool_count"] = len(payload.get("tools", [])) if isinstance(payload.get("tools"), list) else 0
            native_bedrock_converse_stream_captured["tool_choice"] = payload.get("tool_choice")
            return NativeBedrockConverseStreamSmokeResponse()

        native_bedrock_converse_stream_runtime = PhobosAgentRuntime(
            AgentRuntimeConfig(
                engagement_path=str(engagement_path),
                db_path=str(root / "native-bedrock-converse-stream-agent.db"),
                session_name="native-bedrock-converse-stream-smoke-runtime",
                auto_model_planning=True,
            ),
            adapter=AnthropicMessagesAdapter(
                model="fake-bedrock-converse-stream-smoke-model",
                base_url="http://127.0.0.1:9/v1",
                key_env="PHOBOS_TEST_NO_SUCH_ANTHROPIC_KEY",
            ),
        )
        native_bedrock_converse_stream_original_urlopen = model_adapters.urllib.request.urlopen
        try:
            model_adapters.urllib.request.urlopen = fake_native_bedrock_converse_stream_urlopen
            native_bedrock_converse_stream_plan = native_bedrock_converse_stream_runtime.handle_message('/auto model=true prompt="native Bedrock ConverseStream smoke token=native-bedrock-converse-stream-secret"')
            native_bedrock_converse_stream_plan_payload = json.loads(native_bedrock_converse_stream_plan.split("\n", 1)[1])
            native_bedrock_converse_stream_apply = native_bedrock_converse_stream_runtime.handle_message('/auto apply=true model=true prompt="native Bedrock ConverseStream smoke token=native-bedrock-converse-stream-secret"')
            native_bedrock_converse_stream_apply_payload = json.loads(native_bedrock_converse_stream_apply.split("\n", 1)[1])
            native_bedrock_converse_stream_recall = native_bedrock_converse_stream_runtime.handle_message('/recall query=native-bedrock-converse-stream-smoke')
            write("native-bedrock-converse-stream.json", redact_secrets(json.dumps({
                "plan": native_bedrock_converse_stream_plan_payload,
                "apply": native_bedrock_converse_stream_apply_payload,
                "captured": native_bedrock_converse_stream_captured,
                "recall": native_bedrock_converse_stream_recall,
                "marker_exists": native_bedrock_converse_stream_marker.exists(),
            }, indent=2, sort_keys=True)) or "{}")
        finally:
            model_adapters.urllib.request.urlopen = native_bedrock_converse_stream_original_urlopen
            native_bedrock_converse_stream_runtime.close()
        native_bedrock_converse_stream_calls = native_bedrock_converse_stream_plan_payload.get("tool_calls", []) if isinstance(native_bedrock_converse_stream_plan_payload.get("tool_calls"), list) else []
        native_bedrock_converse_stream_call_metadata = [call.get("metadata", {}) if isinstance(call, dict) else {} for call in native_bedrock_converse_stream_calls]
        native_bedrock_converse_stream_ledger = native_bedrock_converse_stream_apply_payload.get("execution_ledger", []) if isinstance(native_bedrock_converse_stream_apply_payload.get("execution_ledger"), list) else []
        native_bedrock_converse_stream_outputs = native_bedrock_converse_stream_plan + native_bedrock_converse_stream_apply + native_bedrock_converse_stream_recall + json.dumps(native_bedrock_converse_stream_plan_payload) + json.dumps(native_bedrock_converse_stream_apply_payload)
        checks["native_bedrock_converse_stream_tool_use_ok"] = (
            native_bedrock_converse_stream_plan_payload.get("mode") == "plan_only"
            and [call.get("tool") for call in native_bedrock_converse_stream_calls] == ["remember", "run_command"]
            and native_bedrock_converse_stream_calls[0].get("args", {}).get("value") == "Bedrock ConverseStream toolUse wrappers translated"
            and native_bedrock_converse_stream_calls[1].get("args", {}).get("execute") is False
            and [item.get("provider_tool_call_id") for item in native_bedrock_converse_stream_call_metadata] == ["bedrock_converse_stream_smoke_memory", "bedrock_converse_stream_smoke_dry"]
            and [item.get("native_tool_call_source") for item in native_bedrock_converse_stream_call_metadata] == ["native provider bedrock converse stream content toolUse", "native provider bedrock converse stream content toolUse"]
            and [item.get("provider_tool_call_id") for item in native_bedrock_converse_stream_ledger] == ["bedrock_converse_stream_smoke_memory", "bedrock_converse_stream_smoke_dry"]
            and [item.get("native_tool_call_source") for item in native_bedrock_converse_stream_ledger] == ["native provider bedrock converse stream content toolUse", "native provider bedrock converse stream content toolUse"]
            and [item.get("result", {}).get("status") for item in native_bedrock_converse_stream_apply_payload.get("results", [])] == ["ok", "dry_run"]
            and native_status_milestone_contract.get("bedrock_converse_stream_tool_use_translation") is True
            and "bedrock_converse_stream_toolUse" in native_status_data.get("provider_native_tool_call_variants", [])
            and native_bedrock_converse_stream_captured.get("tool_choice") == {"type": "auto"}
            and native_bedrock_converse_stream_captured.get("tool_count", 0) > 0
            and "Bedrock ConverseStream toolUse wrappers translated" in native_bedrock_converse_stream_recall
            and not native_bedrock_converse_stream_marker.exists()
            and native_bedrock_converse_stream_result_marker not in native_bedrock_converse_stream_outputs
            and "native-bedrock-converse-stream-secret" not in native_bedrock_converse_stream_outputs
        )
        ...
        native_direct_candidate_captured = {"count": 0, "tool_choices": []}
        native_direct_candidate_result_marker = "NATIVE_DIRECT_CANDIDATE_RESULT_SHOULD_NOT_SURFACE"

        class NativeProviderDirectCandidateSmokeResponse:
            def __init__(self, payload):
                self.payload = payload
        ...
        def fake_native_direct_candidate_urlopen(request, timeout=0):
            payload = json.loads(request.data.decode("utf-8"))
            native_direct_candidate_captured["count"] += 1
            native_direct_candidate_captured["tool_count"] = len(payload.get("tools", [])) if isinstance(payload.get("tools"), list) else 0
            native_direct_candidate_captured["tool_choices"].append(payload.get("tool_choice"))
            if native_direct_candidate_captured["count"] == 1:
                return NativeProviderDirectCandidateSmokeResponse({
                    "candidates": [
                        {
                            "content": {
                                "text": "native direct candidate content smoke token=native-direct-candidate-secret",
                                "functionCall": {
                                    "toolCallId": "direct_candidate_content_memory",
                                    "name": "remember",
                                    "args": {"key": "native-candidate-direct-content-smoke", "value": "candidate direct content native tool call translated"},
                                },
                                "functionResponse": {"response": {"content": native_direct_candidate_result_marker + " token=native-direct-candidate-secret"}},
                            }
                        }
                    ]
                })
            return NativeProviderDirectCandidateSmokeResponse({
                "candidate": {
                    "text": "native direct candidate object smoke token=native-direct-candidate-secret",
                    "functionCall": {
                        "functionCallId": "direct_candidate_object_memory",
                        "name": "remember",
                        "parameters": {"key": "native-candidate-direct-object-smoke", "value": "candidate direct object native tool call translated"},
                    },
                }
            })

        native_direct_candidate_runtime = PhobosAgentRuntime(
            AgentRuntimeConfig(
                engagement_path=str(engagement_path),
                db_path=str(data / "native-provider-direct-candidate.db"),
                session_name="native-provider-direct-candidate-smoke",
                auto_model_planning=True,
            ),
            adapter=OpenAICompatibleAdapter(model="fake-native-direct-candidate-smoke", base_url="http://127.0.0.1:9/v1"),
        )
        native_direct_candidate_original_urlopen = model_adapters.urllib.request.urlopen
        try:
            model_adapters.urllib.request.urlopen = fake_native_direct_candidate_urlopen
            native_direct_candidate_plan = native_direct_candidate_runtime.handle_message('/auto model=true prompt="native direct candidate smoke token=native-direct-candidate-secret"')
            native_direct_candidate_plan_payload = json.loads(native_direct_candidate_plan.split("\n", 1)[1])
            native_direct_candidate_apply = native_direct_candidate_runtime.handle_message('/auto apply=true model=true prompt="native direct candidate object smoke token=native-direct-candidate-secret"')
            native_direct_candidate_apply_payload = json.loads(native_direct_candidate_apply.split("\n", 1)[1])
            native_direct_candidate_recall = native_direct_candidate_runtime.handle_message('/recall query=native-candidate-direct-object-smoke')
            write("native-provider-direct-candidate-function-call.json", json.dumps({
                "plan": native_direct_candidate_plan_payload,
                "apply": native_direct_candidate_apply_payload,
                "captured": native_direct_candidate_captured,
                "recall": native_direct_candidate_recall,
            }, indent=2, sort_keys=True))
        finally:
            model_adapters.urllib.request.urlopen = native_direct_candidate_original_urlopen
            native_direct_candidate_runtime.close()
        native_direct_candidate_calls = native_direct_candidate_plan_payload.get("tool_calls", []) if isinstance(native_direct_candidate_plan_payload.get("tool_calls"), list) else []
        native_direct_candidate_metadata = native_direct_candidate_plan_payload.get("metadata", {}) if isinstance(native_direct_candidate_plan_payload.get("metadata"), dict) else {}
        native_direct_candidate_ledger = native_direct_candidate_apply_payload.get("execution_ledger", []) if isinstance(native_direct_candidate_apply_payload.get("execution_ledger"), list) else []
        native_direct_candidate_outputs = native_direct_candidate_plan + native_direct_candidate_apply + native_direct_candidate_recall + json.dumps(native_direct_candidate_plan_payload) + json.dumps(native_direct_candidate_apply_payload)
        checks["native_provider_candidate_direct_function_call_ok"] = (
            native_direct_candidate_plan_payload.get("mode") == "plan_only"
            and [call.get("tool") for call in native_direct_candidate_calls] == ["remember"]
            and native_direct_candidate_calls[0].get("args", {}).get("key") == "native-candidate-direct-content-smoke"
            and "native provider candidate functionCall" in native_direct_candidate_calls[0].get("reason", "")
            and native_direct_candidate_calls[0].get("metadata", {}).get("provider_tool_call_id") == "direct_candidate_content_memory"
            and native_direct_candidate_metadata.get("native_tool_call_count") == 1
            and "tool_result" in json.dumps(native_direct_candidate_plan_payload.get("warnings", [])).lower()
            and [item.get("result", {}).get("status") for item in native_direct_candidate_apply_payload.get("results", [])] == ["ok"]
            and [item.get("provider_tool_call_id") for item in native_direct_candidate_ledger] == ["direct_candidate_object_memory"]
            and native_direct_candidate_ledger[0].get("native_tool_call_source") == "native provider candidate functionCall"
            and native_status_milestone_contract.get("candidate_direct_function_call_translation") is True
            and "candidate_direct_function_call" in native_status_data.get("provider_native_tool_call_variants", [])
            and "candidate direct object native tool call translated" in native_direct_candidate_recall
            and native_direct_candidate_captured.get("tool_choices") == ["auto", "auto"]
            and native_direct_candidate_captured.get("tool_count", 0) > 0
            and native_direct_candidate_result_marker not in native_direct_candidate_outputs
            and "native-direct-candidate-secret" not in native_direct_candidate_outputs
        )
        ...
        native_milestone_required_checks = [
            ...
            "native_provider_tool_call_nested_tool_object_ok",
            ...
            "native_provider_candidate_direct_function_call_ok",
            ...
            "native_provider_root_tool_use_aliases_ok",
            "native_provider_message_tool_use_aliases_ok",
            "native_provider_neutral_tool_aliases_ok",
