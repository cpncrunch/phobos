...
Current verification covers guardrails, CLI/profile/auth-status/preflight/closeout/finding-bundle, strict config/bridge scalar parsing, DB seal/unseal backup round-trips, Burp MCP client/artifacts, BloodHound path/ADCS import, CVE advisor, model adapter, model tool-call plan validation plus redacted plan-preview transcripts with one-shot planner traces, bounded redacted native planner runtime-context handoff, native tool-call provider fallback chains, natural-message native auto-execute provenance, explicit allowed native tool-call command execution, native scanner-wrapper execute=false coercion unless `/auto execute=true` is supplied, applied native `/auto` transcript artifacts/chat summaries, native `/auto`/`/auto-loop` slash flag safety for `on`/`off` and ambiguous booleans/steps, OpenAI-compatible native `tool_calls`/content-block `tool_use`/`function_call`/legacy `function_call` translation plus malformed native-call rejection and provider `tool_result` ignore handling, guardrail/approval preview boundaries, native loop approval/block stop boundaries, native operator approval replay for confirm-gated model plans, native model approval-action guardrails, native runtime-policy confirm/block boundaries, invalid-plan stops for all-rejected model feedback, cumulative result-feedback loop transcripts, terminal no-tool model-loop stops after feedback without deterministic re-planning, metadata-only native transcript index/detail views, redacted native execution ledgers and execution-summary claim counts, native tool-loop gateway/chat surfaces, and a native tool-calling milestone contract status/smoke gate, finding exporter, deterministic finding QA review, redacted per-finding evidence bundles, safety preflight readiness reports, synthetic guardrail self-tests, artifact output containment, lifecycle records, structured nmap/httpx/nuclei/ffuf wrappers, schema-declared string/integer/number/integer-bounds/number-bounds/string-and-collection-size-bounds/boolean/required/blank-required/enum/closed-object-extra tool argument validation, tool-run/finding storage redaction, expanded Authorization/Cookie/cloud-OAuth/private-key redaction, session message/memory/context/media storage redaction, SQLite FTS recall, guarded auto-planning and bounded auto-loop, Hindsight aliases, session-scoped LCM-style context node describe/expand, isolated local delegation child sessions, local/remote-metadata bridge media handling, media artifacts, redacted audit storage/display, redacted evidence timelines, read-only evidence manifests, evidence secret scans, closeout readiness reviews with local drill-down links, sealed portable snapshots, local skills, task boards, tool policy approvals/blocks, foreground/background process management, scheduled jobs, bridge dispatch safety, gateway routes, and export-pack generation. Demo outputs under `demo-phobos-parity/` are generated by the smoke script and are intentionally not committed.
...
- guarded deterministic `/auto` planning plus optional model-assisted planning that accepts either JSON content plans or OpenAI-compatible provider-native `tool_calls` (including nested function calls and flattened top-level `name`/`arguments` variants), content-block `tool_use`/`function_call`, or legacy `function_call` payloads through the configured provider fallback chain while ignoring provider-side `tool_result` echoes, plus bounded `/auto-loop`; native `apply`/`model`/`execute` slash flags parse explicit boolean values safely and reject ambiguous booleans or malformed step counts before planning/dispatch; if `auto_execute_natural` is explicitly enabled, natural-message native plans are tagged `trigger=natural_auto`, keep command/process execution dry-run unless an explicit slash `execute=true` path is used, and expose that provenance in audit/transcript detail views; model-proposed tool calls receive bounded redacted runtime context, are registry-name and JSON-schema validated before plan display or dispatch, include runtime-policy annotations for `confirm_tools`/`blocked_tools`, and use read-only guardrail previews for target-affecting calls before apply; approval-control tools are hidden from model specs and rejected if returned; confirm-gated native plans remain non-executed until direct `/approve` replay; one-shot plans and loops write redacted JSON/Markdown transcripts under `agent/auto-plans` and `agent/auto-loops`, one-shot plans include a bounded redacted planner/provider trace, loops feed redacted non-terminal results back between bounded steps, record per-step execution-ledger deltas plus redacted planner/provider trace entries so each loop step has a local no-overclaim proof, stop on repeated or same-step duplicate model plans before dispatch/re-dispatch of the same call, respect a terminal no-tool model response after feedback instead of re-planning deterministically from the original prompt, stop with an explicit `model_error` state if provider planning fails after tool-result feedback, stop with `invalid_plan` if every model-proposed follow-up call is rejected before dispatch, stop immediately on approval/block terminal results, surface explicit max-step budget exhaustion, and render chat/gateway summaries from the execution ledger instead of inferring that a tool or command ran;
- `/status` includes a read-only native tool-calling safety contract and milestone acceptance matrix: model-planning flags, the bounded step budget, terminal no-tool/no-dispatch markers, and max-step/model-error/invalid-plan/duplicate/same-step-duplicate stop enforcement, plan-only/execute-required defaults, per-step execution-ledger delta, one-shot planner-trace, and per-step planner-trace support, supported provider-native tool-call variants, provider `tool_result`/`function_result` echo suppression, approval-control tools hidden from model specs, execution-capable/target-affecting tool classes, native-loop contract completion flags, and local transcript counts without reading target systems or raw transcript contents;
