...
- **Model adapter layer** — supports a deterministic offline heuristic adapter plus OpenAI-compatible/local/Hermes-CLI adapters for role-specific drafting; OpenAI-compatible endpoints can also return provider-native `tool_calls` for guarded `/auto` and `/auto-loop` planning, with Phobos translating them into the same validated plan boundary before any dispatch while keeping approval-control actions under explicit direct operator commands.
