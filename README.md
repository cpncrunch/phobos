...
- **Model adapter layer** — supports a deterministic offline heuristic adapter plus OpenAI-compatible/local/Hermes-CLI adapters for role-specific drafting; OpenAI-compatible endpoints can also return provider-native `tool_calls` for guarded `/auto` and `/auto-loop` planning, with Phobos translating them into the same validated plan boundary before any dispatch, keeping approval-control actions under explicit direct operator commands, and emitting redacted plan-preview, one-shot apply, and loop transcripts plus execution ledgers so dry-runs/approvals/blocks are never described as executed commands while explicitly allowed execution is claimable only when the registry returns an executed/started result.
...
- local HTTP gateway with JSON endpoints, route discovery, local dashboard, dedicated `/auto` and `/auto-loop` native tool-loop POST routes, granular guardrail editor, memory hygiene views, bounded JSON POST bodies, session-bound finding/tool-run/delegation/media/job/task/process detail, finding-bundle packaging, timeline/manifest/secret-scan/preflight/guardrail-test/closeout/local-ref views, and routes for status/tools/schemas/sessions/context/memories/memory/preflight/guardrail-test/timeline/manifest/manifest-verify/secret-scan/closeout/ref/detail/resolve-ref/LCM/tasks/task-detail/findings/finding-detail/finding-bundle/tool-runs/tool-run-detail/jobs/job-detail/processes/process-detail/approvals/redacted approval detail/delegations/delegation-detail/media/media-detail/auth/bridges/guardrails/audit;
