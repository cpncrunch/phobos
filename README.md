...
- `/status` includes a read-only native tool-calling safety contract and milestone acceptance matrix: model-planning flags, the bounded step budget, terminal no-tool/no-dispatch markers, and max-step/model-error/invalid-plan/duplicate/same-step-duplicate stop enforcement, plan-only/execute-required defaults, per-step execution-ledger delta, one-shot planner-trace, and per-step planner-trace support, supported provider-native tool-call variants, provider tool-call ID provenance in plan/result ledgers, provider `tool_result`/`function_result` echo suppression, approval-control tools hidden from model specs, execution-capable/target-affecting tool classes, native-loop contract completion flags, and local transcript counts without reading target systems or raw transcript contents;
