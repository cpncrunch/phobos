...
- `/status` includes a read-only native tool-calling safety contract: model-planning flags, the bounded step budget, terminal no-tool/no-dispatch markers, and max-step/model-error/invalid-plan/duplicate/same-step-duplicate stop enforcement, plan-only/execute-required defaults, per-step execution-ledger delta, one-shot planner-trace, and per-step planner-trace support, supported provider-native tool-call variants, provider `tool_result`/`function_result` echo suppression, approval-control tools hidden from model specs, execution-capable/target-affecting tool classes, and local transcript counts without reading target systems or raw transcript contents;
