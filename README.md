...
- guarded deterministic `/auto` planning plus optional model-assisted planning that accepts either JSON content plans or OpenAI-compatible provider-native `tool_calls`/legacy `function_call` payloads through the configured provider fallback chain, plus bounded `/auto-loop`; native `apply`/`model`/`execute` slash flags parse explicit `on`/`off`/`true`/`false` values safely and reject ambiguous booleans or malformed step counts before planning/dispatch; model-proposed tool calls receive a bounded, redacted runtime context containing recent messages, relevant memories, task/approval state, loaded-skill metadata, and ROE/policy status before the adapter plans, and are registry-name and JSON-schema validated before plan display, dispatch, runtime-policy approval queueing, or guardrail execution, target-affecting calls include read-only guardrail preview metadata (`allow`/`confirm`/`block`) before apply, `approve`/`deny` approval-control tools are omitted from model tool specs and rejected if returned so queued confirmations require explicit direct operator commands, confirm-gated native plans remain non-executed until direct `/approve` replay re-runs guardrails and executes the stored approval, rejected calls are shown in a redacted `rejected_tool_calls` transcript field, plan-only `/auto` previews write redacted JSON/Markdown transcripts under `agent/auto-plans` with `no_tools_executed=true`, applied one-shot `/auto` plans use the same transcript directory and chat summaries report ledger counts instead of inferring execution, model loops feed redacted tool results back between bounded steps with explicit `stop_reason` values plus JSON/Markdown transcripts under `agent/auto-loops`, but stop immediately with `approval_required`, `blocked_result`, or `approval_or_blocked_result` when a registry result queues approval or blocks so terminal guardrail outcomes are not fed into further autonomous planning, `/auto-transcripts`/`/auto-transcript` and `auto-transcript:` refs expose metadata-only transcript index/detail views without raw file-content emission, applied plans/loops include an `execution_ledger` that separates actual command/process activity from dry-runs, approval queues, blocks, handler errors, and local-only tool completions, and the ledger now positively marks explicitly allowed executions as claimable only when the registry result proves command/process activity; dedicated gateway/chat surfaces expose the same runtime path without bypassing policy or ROE;
