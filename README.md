...
- **Model adapter layer** — supports a deterministic offline heuristic adapter plus OpenAI-compatible/local/Hermes-CLI adapters for role-specific drafting; OpenAI-compatible endpoints can also return provider-native `tool_calls` (including nested function calls, single-object top-level `tool_calls`, singular `tool_call` aliases, camelCase `toolCalls`/`toolCall` aliases, flattened top-level `name`/`arguments` variants, provider tool-name aliases such as `toolName`/`functionName`, and argument aliases such as `arguments_json` or `inputJson`), content-block `tool_use`/`function_call`/`functionCall`, `content.parts` `functionCall` objects, Responses-style top-level `output`/`function_call` including single-object `output` shims, nested `function`/`functionCall` output items, typed or typeless direct or nested `output[].message` `tool_calls`/`toolCalls`/`tool_call`/`toolCall`/`functionCall`/`functionCalls`/`function_calls` aliases, and `output[].message.content` `function_call`/`tool_use`/`functionCall` blocks (including nested Gemini-style `parts` `functionCall` blocks), candidate list-or-single `parts` `functionCall`, root-level `message` wrappers carrying `tool_calls`/aliases plus content `functionCall` and nested `parts` `functionCall` blocks, root-level `toolUse`/`toolUses`/`tool_use`/`tool_uses` aliases, root or `choices[].message` `functionCall`/`functionCalls`/`function_calls` aliases (including flat, nested `function`, and nested `functionCall` items), or legacy `function_call` payloads for guarded `/auto`, `/auto-loop`, and the public `phobos-agent auto`/`phobos-agent auto-loop` CLI entrypoints; provider `tool_result`/`functionResponse` echoes are ignored rather than summarized or dispatched, custom/freeform/provider-hosted native tool calls such as `custom_tool_call`, `server_tool_use`, and `mcp_tool_use` are rejected without surfacing their input, and provider tool-call IDs (`id`, `call_id`, `tool_call_id`, `tool_use_id`, `callId`, `toolCallId`, or `toolUseId`) are preserved as bounded provenance in plan/result ledgers. Phobos provides bounded redacted runtime context, translates model-requested calls into the same validated plan boundary before any dispatch, keeps approval-control actions under explicit direct operator commands, stops loops at explicit max-step budgets, and emits redacted plan-preview, one-shot apply, and loop transcripts plus metadata-only transcript index/detail views and execution ledgers so dry-runs, approvals, blocks, and handler errors are never claimed as executed commands.
...
- guarded deterministic `/auto` planning plus optional model-assisted planning accepts JSON plans even when providers wrap them in fenced/prose output, OpenAI-compatible provider-native `tool_calls` plus singular `tool_call` and camelCase `toolCalls`/`toolCall` aliases, provider tool-name aliases such as `toolName`/`functionName`, content-block `tool_use`/`function_call`/`functionCall`, Responses-style top-level `output`/`function_call` including single-object `output` shims, nested `function`/`functionCall` output items, typed or typeless direct or nested `output[].message` `tool_calls`/`toolCalls`/`tool_call`/`toolCall`/`functionCall`/`functionCalls`/`function_calls` aliases, and `output[].message.content` `function_call`/`tool_use`/`functionCall` blocks (including nested Gemini-style `parts` `functionCall` blocks), candidate list-or-single `parts` `functionCall`, root-level `message` wrappers carrying `tool_calls`/aliases plus content `functionCall` and nested `parts` `functionCall` blocks, root-level `toolUse`/`toolUses`/`tool_use`/`tool_uses` aliases, root or `choices[].message` `functionCall`/`functionCalls`/`function_calls` aliases (including flat, nested `function`, and nested `functionCall` items), and legacy `function_call` payloads through the configured provider fallback chain. The native boundary supplies bounded redacted runtime context, validates registered tool names and JSON-schema arguments before dispatch, applies runtime policy and ROE guardrail previews for target-affecting calls, hides approval-control tools from model specs, requires direct `/approve` replay for confirm-gated plans, coerces command/process/scanner execution to dry-run unless the operator explicitly supplies `execute=true`, writes redacted one-shot and loop transcripts under `agent/auto-plans` and `agent/auto-loops`, feeds redacted non-terminal results through bounded `/auto-loop` steps, stops on terminal no-tool/model-error/invalid/duplicate/approval/block conditions, and renders chat/gateway summaries from execution ledgers so dry-runs, approvals, blocks, and handler errors are never claimed as executed commands;
- `/status` includes a read-only native tool-calling safety contract and milestone acceptance matrix: model-planning flags, wrapped/fenced JSON plan extraction support, the bounded step budget, terminal no-tool/no-dispatch markers, and max-step/model-error/invalid-plan/duplicate/same-step-duplicate stop enforcement, plan-only/execute-required defaults, per-step execution-ledger delta, one-shot planner-trace, and per-step planner-trace support, supported provider-native tool-call variants including root-level `toolUse`/`toolUses` aliases, provider tool-name aliases such as `toolName`/`functionName`, provider argument aliases such as `arguments_json`/`inputJson`, provider tool-call ID aliases such as `call_id`/`tool_use_id`/`callId`/`toolCallId`, provider tool-call ID provenance in plan/result ledgers and transcript review summaries, provider `tool_result`/`function_result`/`function_call_output`/`functionResponse` echo suppression, provider-hosted/custom/freeform tool-call rejection, approval-control tools hidden from model specs, execution-capable/target-affecting tool classes, native-loop contract completion flags, and local transcript counts without reading target systems or raw transcript contents;
...
`native_tool_call_milestone_contract_ok=True` is an aggregate gate over every native tool-call safety/translation smoke above, including single/top-level content-block calls, flat/nested/message-level/message-content/parts Responses output calls, typed and typeless nested `output[].message` wrappers, root-level `message` wrapper alias matrix, root-level `toolUse`/`toolUses` aliases, root/message `functionCall` / `functionCalls` / `function_calls` aliases, provider tool-name alias normalization, provider call-ID provenance, result-echo suppression, provider-hosted/freeform tool-call rejection, runtime policy, ROE preview, approval stops, and ledger claim semantics.
