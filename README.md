...
- **Model adapter layer** — supports a deterministic offline heuristic adapter plus OpenAI-compatible/Responses/Gemini/Anthropic/local/Hermes-CLI adapters for role-specific drafting; provider-native planning translates JSON-content and native tool-call shapes from OpenAI-compatible/Responses/Gemini/Anthropic/Bedrock-style gateways, including object-map batches, streaming fragments, nested function/tool aliases, content/candidate/root wrappers, list-valued `response`/`result`/`data` envelopes, and provider tool-result echo suppression. Every proposed call still enters Phobos' schema, runtime-policy, ROE, approval, explicit-execute, ledger, and redaction boundary before any action can run.
