...
class AgentRuntimeTests(unittest.TestCase):
    ...
    def test_openai_chat_completions_sse_legacy_function_call_chunks_merge(self):
        with tempfile.TemporaryDirectory() as tmp:
            tmp_path = Path(tmp)
            engagement = tmp_path / "engagement.json"
            EngagementROE(
                name="Native OpenAI Chat SSE Legacy Function Call",
                authorized=True,
                in_scope_targets=["app.example.test"],
                evidence_dir=str(tmp_path / "evidence"),
            ).save(engagement)
            captured_payloads = []
            dry_run_marker = tmp_path / "native-openai-chat-sse-legacy-should-not-execute.txt"
            run_args = json.dumps({
                "target": "app.example.test",
                "purpose": "Chat Completions SSE legacy function_call dry-run validation",
                "command": f"printf native-openai-chat-sse-legacy > {dry_run_marker}",
                "execute": True,
            })

            class FakeChatSSELegacyHTTPResponse:
                def __enter__(self):
                    return self
            ...
            def fake_urlopen(request, timeout=0):
                captured_payloads.append(json.loads(request.data.decode("utf-8")))
                return FakeChatSSELegacyHTTPResponse()

            runtime = OffSecAgentRuntime(
                AgentRuntimeConfig(
                    engagement_path=str(engagement),
                    db_path=str(tmp_path / "agent.db"),
                    session_name="native-openai-chat-sse-legacy-runtime",
                    auto_model_planning=True,
                ),
                adapter=OpenAICompatibleAdapter(model="fake-chat-sse-legacy-model", base_url="http://127.0.0.1:9/v1"),
            )
            try:
                with mock.patch("offsec_agent_harness.model_adapters.urllib.request.urlopen", side_effect=fake_urlopen):
                    planned = runtime.handle_message('/auto model=true prompt="native chat SSE legacy token=chat-sse-legacy-secret"')
                    plan_payload = json.loads(planned.split("\n", 1)[1])
                    self.assertEqual(plan_payload["mode"], "plan_only")
                    self.assertEqual([call["tool"] for call in plan_payload["tool_calls"]], ["run_command"])
                    self.assertFalse(plan_payload["tool_calls"][0]["args"]["execute"])
                    self.assertEqual(plan_payload["tool_calls"][0]["args"]["purpose"], "Chat Completions SSE legacy function_call dry-run validation")
                    call_metadata = plan_payload["tool_calls"][0].get("metadata", {})
                    self.assertEqual(call_metadata.get("native_tool_call_source"), "native provider chat completions sse delta function_call")
                    self.assertEqual(call_metadata.get("native_tool_call_index"), 1)

                    applied = runtime.handle_message('/auto apply=true model=true prompt="native chat SSE legacy token=chat-sse-legacy-secret"')
                    applied_payload = json.loads(applied.split("\n", 1)[1])
                self.assertEqual([item["result"]["status"] for item in applied_payload["results"]], ["dry_run"])
                self.assertFalse(dry_run_marker.exists())
                status = runtime.registry.run("runtime_status", {}).data.get("native_tool_calling", {})
                self.assertTrue(status.get("milestone_contract", {}).get("chat_completions_sse_legacy_function_call_translation"), status)
                self.assertIn("openai_chat_completions_sse_legacy_function_call", status.get("provider_native_tool_call_variants", []))
                self.assertEqual(captured_payloads[0].get("tool_choice"), "auto")
                combined = planned + applied + json.dumps(plan_payload) + json.dumps(applied_payload) + json.dumps(status)
                self.assertNotIn("chat-sse-legacy-secret", combined)
            finally:
                runtime.close()
    ...
    def test_openai_responses_raw_sse_call_id_only_argument_deltas_merge_with_added_items(self):
        with tempfile.TemporaryDirectory() as tmp:
            tmp_path = Path(tmp)
            engagement = tmp_path / "engagement.json"
            EngagementROE(
                name="Native OpenAI Responses Raw SSE Call ID Alias",
                authorized=True,
                in_scope_targets=["app.example.test"],
                evidence_dir=str(tmp_path / "evidence"),
            ).save(engagement)
            captured_requests = []
            dry_run_marker = tmp_path / "native-openai-responses-sse-call-id-should-not-execute.txt"

            class FakeResponsesCallIdOnlyDeltaHTTPResponse:
                def __enter__(self):
                    return self
                ...
                def read(self) -> bytes:
                    memory_args = json.dumps({"key": "native-openai-responses-sse-call-id", "value": "Responses SSE call_id-only argument deltas assembled"})
                    run_args = json.dumps({
                        "target": "app.example.test",
                        "purpose": "Responses SSE call_id-only dry-run validation",
                        "command": f"printf native-openai-responses-sse-call-id > {dry_run_marker}",
                        "execute": True,
                    })
                    frames = [
                        ("response.output_text.delta", {"type": "response.output_text.delta", "delta": "native Responses call-id SSE plan token=responses-sse-call-id-secret"}),
                        (
                            "response.output_item.added",
                            {
                                "type": "response.output_item.added",
                                "item": {
                                    "id": "sse_alias_memory_item",
                                    "type": "function_call",
                                    "call_id": "responses_sse_alias_memory",
                                    "name": "remember",
                                    "arguments": memory_args[:41],
                                },
                            },
                        ),
                        {
                    ...
                    for index, frame in enumerate(frames, start=1):
                        if isinstance(frame, tuple):
                            ...
                            event_name, data = str(frame.get("type") or "message"), frame
                        body_parts.append(f"event: {event_name}\nid: call-id-sse-{index}\ndata: {json.dumps(data)}\n\n")
                    return ("".join(body_parts) + "data: [DONE]\n\n").encode("utf-8")

            def fake_urlopen(request, timeout=0):
                captured_requests.append(json.loads(request.data.decode("utf-8")))
                return FakeResponsesCallIdOnlyDeltaHTTPResponse()

            runtime = OffSecAgentRuntime(
                AgentRuntimeConfig(
                    engagement_path=str(engagement),
                    db_path=str(tmp_path / "agent.db"),
                    session_name="native-openai-responses-sse-call-id-runtime",
                    auto_model_planning=True,
                ),
                adapter=OpenAIResponsesAdapter(model="fake-native-responses-sse-call-id-model", base_url="http://127.0.0.1:9/v1"),
            )
            try:
                with mock.patch("offsec_agent_harness.model_adapters.urllib.request.urlopen", side_effect=fake_urlopen):
                    planned = runtime.handle_message('/auto model=true prompt="native Responses raw SSE call-id token=responses-sse-call-id-secret"')
                    plan_payload = json.loads(planned.split("\n", 1)[1])
                    self.assertEqual(plan_payload["mode"], "plan_only")
                    self.assertEqual([call["tool"] for call in plan_payload["tool_calls"]], ["remember", "run_command"])
                    self.assertEqual(plan_payload["tool_calls"][0]["args"]["value"], "Responses SSE call_id-only argument deltas assembled")
                    self.assertFalse(plan_payload["tool_calls"][1]["args"]["execute"])

                    applied = runtime.handle_message('/auto apply=true model=true prompt="native Responses raw SSE call-id token=responses-sse-call-id-secret"')
                    apply_payload = json.loads(applied.split("\n", 1)[1])
                    self.assertEqual([item["result"]["status"] for item in apply_payload["results"]], ["ok", "dry_run"])
                    apply_ledger = apply_payload.get("execution_ledger", [])
                recall = runtime.handle_message('/recall query=native-openai-responses-sse-call-id')
                status = runtime.registry.run("runtime_status", {}).data.get("native_tool_calling", {})
                self.assertIn("Responses SSE call_id-only argument deltas assembled", recall)
                self.assertEqual([item.get("provider_tool_call_id") for item in apply_ledger], ["responses_sse_alias_memory", "responses_sse_alias_dry"])
                self.assertEqual([item.get("native_tool_call_source") for item in apply_ledger], ["native provider responses stream function_call", "native provider responses stream function_call"])
                self.assertTrue(status.get("milestone_contract", {}).get("responses_stream_call_id_delta_alias_translation"), status)
                self.assertIn("responses_stream_call_id_delta_alias", status.get("provider_native_tool_call_variants", []))
                self.assertTrue(captured_requests)
                self.assertEqual(captured_requests[0].get("tool_choice"), "auto")
                self.assertFalse(dry_run_marker.exists())
                self.assertNotIn("responses-sse-call-id-secret", planned + applied + recall + json.dumps(plan_payload) + json.dumps(apply_payload) + json.dumps(status))
            finally:
                runtime.close()
    ...
    def test_anthropic_converse_style_stream_wrappers_are_translated(self):
        with tempfile.TemporaryDirectory() as tmp:
            tmp_path = Path(tmp)
            engagement = tmp_path / "engagement.json"
            EngagementROE(
                name="Native Anthropic Converse Stream ToolUse",
                authorized=True,
                in_scope_targets=["app.example.test"],
                evidence_dir=str(tmp_path / "evidence"),
            ).save(engagement)
            dry_run_marker = tmp_path / "native-anthropic-converse-should-not-execute.txt"
            result_marker = "ANTHROPIC_CONVERSE_RESULT_SHOULD_NOT_SURFACE"

            class FakeAnthropicConverseStreamHTTPResponse:
                def __enter__(self):
                    return self
                ...
                def read(self) -> bytes:
                    return json.dumps({
                        "events": [
                            ...
                            {"contentBlockStart": {"contentBlockIndex": 0, "start": {"toolUse": {"toolUseId": "anthropic_converse_memory", "name": "remember"}}}},
                            {"contentBlockDelta": {"contentBlockIndex": 0, "delta": {"toolUse": {"input": "{\"key\":\"native-anthropic-converse\","}}}},
                            {"contentBlockDelta": {"contentBlockIndex": 0, "delta": {"toolUse": {"input": "\"value\":\"Converse-style stream toolUse wrappers translated\"}"}}}},
                            {"contentBlockStop": {"contentBlockIndex": 0}},
                            {"contentBlockStart": {"contentBlockIndex": 1, "start": {"toolUse": {"toolUseId": "anthropic_converse_dry", "toolName": "run_command"}}}},
                            {"contentBlockDelta": {"contentBlockIndex": 1, "delta": {"toolUse": {"input": json.dumps({"target": "app.example.test", "purpose": "Converse-style stream dry-run validation", "command": f"printf native-anthropic-converse > {dry_run_marker}", "execute": True})}}}},
                            {"contentBlockStop": {"contentBlockIndex": 1}},
                            {"contentBlockStart": {"contentBlockIndex": 2, "start": {"toolResult": {"toolUseId": "anthropic_converse_result", "content": result_marker + " token=anthropic-converse-secret"}}}},
            ...
            def fake_urlopen(request, timeout=0):
                return FakeAnthropicConverseStreamHTTPResponse()

            runtime = OffSecAgentRuntime(
                AgentRuntimeConfig(
                    engagement_path=str(engagement),
                    db_path=str(tmp_path / "agent.db"),
                    session_name="native-anthropic-converse-stream-runtime",
                    auto_model_planning=True,
                ),
                adapter=AnthropicMessagesAdapter(
                    model="fake-anthropic-converse-stream-model",
                    base_url="http://127.0.0.1:9/v1",
                    key_env="PHOBOS_TEST_NO_SUCH_ANTHROPIC_KEY",
                ),
            )
            try:
                with mock.patch("offsec_agent_harness.model_adapters.urllib.request.urlopen", side_effect=fake_urlopen):
                    planned = runtime.handle_message('/auto model=true prompt="native Anthropic Converse stream token=anthropic-converse-secret"')
                    plan_payload = json.loads(planned.split("\n", 1)[1])
                    applied = runtime.handle_message('/auto apply=true model=true prompt="native Anthropic Converse stream token=anthropic-converse-secret"')
                    apply_payload = json.loads(applied.split("\n", 1)[1])
                recall = runtime.handle_message('/recall query=native-anthropic-converse')
                calls = plan_payload.get("tool_calls", [])
                call_metadata = [call.get("metadata", {}) for call in calls]
                apply_ledger = apply_payload.get("execution_ledger", [])
                self.assertEqual(plan_payload["mode"], "plan_only")
                self.assertEqual([call.get("tool") for call in calls], ["remember", "run_command"])
                self.assertEqual(calls[0].get("args", {}).get("value"), "Converse-style stream toolUse wrappers translated")
                self.assertFalse(calls[1].get("args", {}).get("execute"))
                self.assertEqual([item.get("provider_tool_call_id") for item in call_metadata], ["anthropic_converse_memory", "anthropic_converse_dry"])
                self.assertEqual([item.get("native_tool_call_source") for item in call_metadata], ["native provider anthropic messages stream content toolUse", "native provider anthropic messages stream content toolUse"])
                self.assertEqual([item.get("provider_tool_call_id") for item in apply_ledger], ["anthropic_converse_memory", "anthropic_converse_dry"])
                self.assertEqual([item.get("native_tool_call_source") for item in apply_ledger], ["native provider anthropic messages stream content toolUse", "native provider anthropic messages stream content toolUse"])
                self.assertEqual([item.get("result", {}).get("status") for item in apply_payload.get("results", [])], ["ok", "dry_run"])
                status = runtime.registry.run("runtime_status", {}).data.get("native_tool_calling", {})
                self.assertTrue(status.get("milestone_contract", {}).get("anthropic_converse_stream_tool_use_translation"), status)
                self.assertIn("anthropic_converse_stream_toolUse", status.get("provider_native_tool_call_variants", []))
                self.assertIn("Converse-style stream toolUse wrappers translated", recall)
                self.assertFalse(dry_run_marker.exists())
                self.assertNotIn(result_marker, planned + applied + recall + json.dumps(plan_payload) + json.dumps(apply_payload))
                self.assertNotIn("anthropic-converse-secret", planned + applied + recall + json.dumps(plan_payload) + json.dumps(apply_payload))
            finally:
                runtime.close()
    ...
    def test_openai_response_envelope_wrapper_tool_calls_are_translated(self):
        with tempfile.TemporaryDirectory() as tmp:
            tmp_path = Path(tmp)
            engagement = tmp_path / "engagement.json"
            EngagementROE(
                name="Native Response Envelope Wrapper",
                authorized=True,
                in_scope_targets=["app.example.test"],
                evidence_dir=str(tmp_path / "evidence"),
            ).save(engagement)
            captured_payloads = []
            dry_run_marker = tmp_path / "native-response-envelope-should-not-run.txt"
            provider_result_marker = "RESPONSE_ENVELOPE_TOOL_RESULT_SHOULD_NOT_SURFACE"

            class FakeResponseEnvelopeHTTPResponse:
                def __enter__(self):
                    return self
                    ...
                    return json.dumps({
                        "response": {
                            "message": {
                                "role": "assistant",
                                "content": [
                                    {"type": "text", "text": "response envelope wrapper token=response-envelope-secret"},
                                    {"type": "tool_result", "content": provider_result_marker + " token=response-envelope-secret"},
                                ],
                                "tool_calls": [
                                    {
                                        ...
                                        "function": {
                                            "name": "remember",
                                            "arguments": json.dumps({"key": "native-response-envelope", "value": "response envelope wrapper accepted"}),
                                        },
                                    },
                                    {
                                        ...
                                        "function": {
                                            ...
                                            "arguments": json.dumps({
                                                "target": "app.example.test",
                                                "purpose": "response envelope native dry-run boundary",
                                                "command": f"printf native-response-envelope > {dry_run_marker}",
                                                "execute": True,
                                            }),
                                        },
                                    },
                        ...
                        "metadata": {"note": "outer envelope token=response-envelope-secret should not surface"},
                    }).encode("utf-8")

            def fake_urlopen(request, timeout=0):
                captured_payloads.append(json.loads(request.data.decode("utf-8")))
                return FakeResponseEnvelopeHTTPResponse()

            runtime = OffSecAgentRuntime(
                AgentRuntimeConfig(
                    engagement_path=str(engagement),
                    db_path=str(tmp_path / "agent.db"),
                    session_name="native-response-envelope-wrapper",
                    auto_model_planning=True,
                ),
                adapter=OpenAICompatibleAdapter(model="fake-native-response-envelope", base_url="http://127.0.0.1:9/v1"),
            )
            try:
                with mock.patch("offsec_agent_harness.model_adapters.urllib.request.urlopen", side_effect=fake_urlopen):
                    planned = runtime.handle_message('/auto model=true prompt="native response envelope wrapper token=response-envelope-secret"')
                    payload = json.loads(planned.split("\n", 1)[1])
                    self.assertEqual(payload["mode"], "plan_only")
                    self.assertEqual([call["tool"] for call in payload["tool_calls"]], ["remember", "run_command"])
                    self.assertTrue(all("native provider root message tool_calls" in call.get("reason", "") for call in payload["tool_calls"]))
                    self.assertEqual(payload["tool_calls"][0]["args"]["key"], "native-response-envelope")
                    self.assertFalse(payload["tool_calls"][1]["args"].get("execute"))
                    call_metadata = [call.get("metadata", {}) for call in payload["tool_calls"]]
                    self.assertEqual([item.get("provider_tool_call_id") for item in call_metadata], ["response_envelope_memory", "response_envelope_dry"])
                    self.assertEqual([item.get("native_tool_call_source") for item in call_metadata], ["native provider root message tool_calls", "native provider root message tool_calls"])
                    self.assertNotIn("response-envelope-secret", planned + json.dumps(payload))
                    self.assertNotIn(provider_result_marker, planned + json.dumps(payload))

                    applied = runtime.handle_message('/auto apply=true model=true prompt="native response envelope wrapper token=response-envelope-secret"')
                    applied_payload = json.loads(applied.split("\n", 1)[1])
                    self.assertEqual([item["result"]["status"] for item in applied_payload["results"]], ["ok", "dry_run"])
                    ledger = applied_payload.get("execution_ledger", [])
                    self.assertEqual([item.get("provider_tool_call_id") for item in ledger], ["response_envelope_memory", "response_envelope_dry"])
                    self.assertFalse(ledger[1].get("actual_command_or_process_activity"))
                recall = runtime.handle_message('/recall query=native-response-envelope')
                status = runtime.registry.run("runtime_status", {}).data.get("native_tool_calling", {})
                self.assertIn("response envelope wrapper accepted", recall)
                self.assertIn("provider_response_envelope_wrapper", status.get("provider_native_tool_call_variants", []))
                self.assertTrue(status.get("milestone_contract", {}).get("provider_response_envelope_wrapper_translation"), status)
                self.assertFalse(dry_run_marker.exists())
                self.assertTrue(captured_payloads)
                self.assertEqual(captured_payloads[0].get("tool_choice"), "auto")
                self.assertNotIn("response-envelope-secret", applied + recall + json.dumps(status))
                self.assertNotIn(provider_result_marker, applied + recall)
            finally:
                runtime.close()

    def test_openai_result_envelope_wrapper_tool_calls_are_translated(self):
        with tempfile.TemporaryDirectory() as tmp:
            tmp_path = Path(tmp)
            engagement = tmp_path / "engagement.json"
            EngagementROE(
                name="Native Result Envelope Wrapper",
                authorized=True,
                in_scope_targets=["app.example.test"],
                evidence_dir=str(tmp_path / "evidence"),
            ).save(engagement)
            captured_payloads = []
            dry_run_marker = tmp_path / "native-result-envelope-should-not-run.txt"
            provider_result_marker = "RESULT_ENVELOPE_TOOL_RESULT_SHOULD_NOT_SURFACE"

            class FakeResultEnvelopeHTTPResponse:
                def __enter__(self):
                    return self
                    ...
                    return json.dumps({
                        "result": {
                            "message": {
                                "role": "assistant",
                                "content": [
                                    {"type": "text", "text": "result envelope wrapper token=result-envelope-secret"},
                                    {"type": "tool_result", "content": provider_result_marker + " token=result-envelope-secret"},
                                ],
                                "tool_calls": [
                                    {
                                        ...
                                        "function": {
                                            "name": "remember",
                                            "arguments": json.dumps({"key": "native-result-envelope", "value": "result envelope wrapper accepted"}),
                                        },
                                    },
                                    {
                                        ...
                                        "function": {
                                            ...
                                            "arguments": json.dumps({
                                                "target": "app.example.test",
                                                "purpose": "result envelope native dry-run boundary",
                                                "command": f"printf native-result-envelope > {dry_run_marker}",
                                                "execute": True,
                                            }),
                                        },
                                    },
                        ...
                        "metadata": {"note": "outer envelope token=result-envelope-secret should not surface"},
                    }).encode("utf-8")

            def fake_urlopen(request, timeout=0):
                captured_payloads.append(json.loads(request.data.decode("utf-8")))
                return FakeResultEnvelopeHTTPResponse()

            runtime = OffSecAgentRuntime(
                AgentRuntimeConfig(
                    engagement_path=str(engagement),
                    db_path=str(tmp_path / "agent.db"),
                    session_name="native-result-envelope-wrapper",
                    auto_model_planning=True,
                ),
                adapter=OpenAICompatibleAdapter(model="fake-native-result-envelope", base_url="http://127.0.0.1:9/v1"),
            )
            try:
                with mock.patch("offsec_agent_harness.model_adapters.urllib.request.urlopen", side_effect=fake_urlopen):
                    planned = runtime.handle_message('/auto model=true prompt="native result envelope wrapper token=result-envelope-secret"')
                    payload = json.loads(planned.split("\n", 1)[1])
                    self.assertEqual(payload["mode"], "plan_only")
                    self.assertEqual([call["tool"] for call in payload["tool_calls"]], ["remember", "run_command"])
                    self.assertTrue(all("native provider root message tool_calls" in call.get("reason", "") for call in payload["tool_calls"]))
                    self.assertEqual(payload["tool_calls"][0]["args"]["key"], "native-result-envelope")
                    self.assertFalse(payload["tool_calls"][1]["args"].get("execute"))
                    call_metadata = [call.get("metadata", {}) for call in payload["tool_calls"]]
                    self.assertEqual([item.get("provider_tool_call_id") for item in call_metadata], ["result_envelope_memory", "result_envelope_dry"])
                    self.assertEqual([item.get("native_tool_call_source") for item in call_metadata], ["native provider root message tool_calls", "native provider root message tool_calls"])
                    self.assertNotIn("result-envelope-secret", planned + json.dumps(payload))
                    self.assertNotIn(provider_result_marker, planned + json.dumps(payload))

                    applied = runtime.handle_message('/auto apply=true model=true prompt="native result envelope wrapper token=result-envelope-secret"')
                    applied_payload = json.loads(applied.split("\n", 1)[1])
                    self.assertEqual([item["result"]["status"] for item in applied_payload["results"]], ["ok", "dry_run"])
                    ledger = applied_payload.get("execution_ledger", [])
                    self.assertEqual([item.get("provider_tool_call_id") for item in ledger], ["result_envelope_memory", "result_envelope_dry"])
                    self.assertFalse(ledger[1].get("actual_command_or_process_activity"))
                recall = runtime.handle_message('/recall query=native-result-envelope')
                status = runtime.registry.run("runtime_status", {}).data.get("native_tool_calling", {})
                self.assertIn("result envelope wrapper accepted", recall)
                self.assertIn("provider_result_envelope_wrapper", status.get("provider_native_tool_call_variants", []))
                self.assertTrue(status.get("milestone_contract", {}).get("provider_result_envelope_wrapper_translation"), status)
                self.assertFalse(dry_run_marker.exists())
                self.assertTrue(captured_payloads)
                self.assertEqual(captured_payloads[0].get("tool_choice"), "auto")
                self.assertNotIn("result-envelope-secret", applied + recall + json.dumps(status))
                self.assertNotIn(provider_result_marker, applied + recall)
            finally:
                runtime.close()

    def test_openai_data_envelope_wrapper_tool_calls_are_translated(self):
        with tempfile.TemporaryDirectory() as tmp:
            tmp_path = Path(tmp)
            engagement = tmp_path / "engagement.json"
            EngagementROE(
                name="Native Data Envelope Wrapper",
                authorized=True,
                in_scope_targets=["app.example.test"],
                evidence_dir=str(tmp_path / "evidence"),
            ).save(engagement)
            captured_payloads = []
            dry_run_marker = tmp_path / "native-data-envelope-should-not-run.txt"
            provider_result_marker = "DATA_ENVELOPE_TOOL_RESULT_SHOULD_NOT_SURFACE"

            class FakeDataEnvelopeHTTPResponse:
                def __enter__(self):
                    return self
                    ...
                    return json.dumps({
                        "data": {
                            "message": {
                                "role": "assistant",
                                "content": [
                                    {"type": "text", "text": "data envelope wrapper token=data-envelope-secret"},
                                    {"type": "tool_result", "content": provider_result_marker + " token=data-envelope-secret"},
                                ],
                                "tool_calls": [
                                    {
                                        ...
                                        "function": {
                                            "name": "remember",
                                            "arguments": json.dumps({"key": "native-data-envelope", "value": "data envelope wrapper accepted"}),
                                        },
                                    },
                                    {
                                        ...
                                        "function": {
                                            ...
                                            "arguments": json.dumps({
                                                "target": "app.example.test",
                                                "purpose": "data envelope native dry-run boundary",
                                                "command": f"printf native-data-envelope > {dry_run_marker}",
                                                "execute": True,
                                            }),
                                        },
                                    },
                        ...
                        "metadata": {"note": "outer envelope token=data-envelope-secret should not surface"},
                    }).encode("utf-8")

            def fake_urlopen(request, timeout=0):
                captured_payloads.append(json.loads(request.data.decode("utf-8")))
                return FakeDataEnvelopeHTTPResponse()

            runtime = OffSecAgentRuntime(
                AgentRuntimeConfig(
                    engagement_path=str(engagement),
                    db_path=str(tmp_path / "agent.db"),
                    session_name="native-data-envelope-wrapper",
                    auto_model_planning=True,
                ),
                adapter=OpenAICompatibleAdapter(model="fake-native-data-envelope", base_url="http://127.0.0.1:9/v1"),
            )
            try:
                with mock.patch("offsec_agent_harness.model_adapters.urllib.request.urlopen", side_effect=fake_urlopen):
                    planned = runtime.handle_message('/auto model=true prompt="native data envelope wrapper token=data-envelope-secret"')
                    payload = json.loads(planned.split("\n", 1)[1])
                    self.assertEqual(payload["mode"], "plan_only")
                    self.assertEqual([call["tool"] for call in payload["tool_calls"]], ["remember", "run_command"])
                    self.assertTrue(all("native provider root message tool_calls" in call.get("reason", "") for call in payload["tool_calls"]))
                    self.assertEqual(payload["tool_calls"][0]["args"]["key"], "native-data-envelope")
                    self.assertFalse(payload["tool_calls"][1]["args"].get("execute"))
                    call_metadata = [call.get("metadata", {}) for call in payload["tool_calls"]]
                    self.assertEqual([item.get("provider_tool_call_id") for item in call_metadata], ["data_envelope_memory", "data_envelope_dry"])
                    self.assertEqual([item.get("native_tool_call_source") for item in call_metadata], ["native provider root message tool_calls", "native provider root message tool_calls"])
                    self.assertNotIn("data-envelope-secret", planned + json.dumps(payload))
                    self.assertNotIn(provider_result_marker, planned + json.dumps(payload))

                    applied = runtime.handle_message('/auto apply=true model=true prompt="native data envelope wrapper token=data-envelope-secret"')
                    applied_payload = json.loads(applied.split("\n", 1)[1])
                    self.assertEqual([item["result"]["status"] for item in applied_payload["results"]], ["ok", "dry_run"])
                    ledger = applied_payload.get("execution_ledger", [])
                    self.assertEqual([item.get("provider_tool_call_id") for item in ledger], ["data_envelope_memory", "data_envelope_dry"])
                    self.assertFalse(ledger[1].get("actual_command_or_process_activity"))
                recall = runtime.handle_message('/recall query=native-data-envelope')
                status = runtime.registry.run("runtime_status", {}).data.get("native_tool_calling", {})
                self.assertIn("data envelope wrapper accepted", recall)
                self.assertIn("provider_data_envelope_wrapper", status.get("provider_native_tool_call_variants", []))
                self.assertTrue(status.get("milestone_contract", {}).get("provider_data_envelope_wrapper_translation"), status)
                self.assertFalse(dry_run_marker.exists())
                self.assertTrue(captured_payloads)
                self.assertEqual(captured_payloads[0].get("tool_choice"), "auto")
                self.assertNotIn("data-envelope-secret", applied + recall + json.dumps(status))
                self.assertNotIn(provider_result_marker, applied + recall)
            finally:
                runtime.close()
    ...
    def test_openai_plural_envelope_wrapper_tool_calls_are_translated(self):
        with tempfile.TemporaryDirectory() as tmp:
            tmp_path = Path(tmp)
            engagement = tmp_path / "engagement.json"
            EngagementROE(
                name="Native Plural Envelope Wrapper",
                authorized=True,
                in_scope_targets=["app.example.test"],
                evidence_dir=str(tmp_path / "evidence"),
            ).save(engagement)
            captured_payloads = []
            dry_run_marker = tmp_path / "native-plural-envelope-should-not-run.txt"
            provider_result_marker = "PLURAL_ENVELOPE_TOOL_RESULT_SHOULD_NOT_SURFACE"

            def plural_envelope_payload(wrapper_key: str) -> dict:
                return {
                    wrapper_key: [
                        {
                            "message": {
                                "role": "assistant",
                                "content": f"stale {wrapper_key} assistant should not be selected token=plural-envelope-secret",
                                "tool_calls": [
                                    {
                                        ...
                                        "function": {
                                            "name": "remember",
                                            "arguments": json.dumps({"key": f"native-{wrapper_key}-old", "value": f"old {wrapper_key} call should not dispatch"}),
                                        },
                                    }
                                ],
                            }
                        },
                        {
                            "message": {
                                "role": "assistant",
                                "content": [
                                    {"type": "text", "text": f"{wrapper_key} plural envelope wrapper token=plural-envelope-secret"},
                                    {"type": "tool_result", "content": provider_result_marker + " token=plural-envelope-secret"},
                                ],
                                "tool_calls": [
                                    {
                                        ...
                                        "function": {
                                            "name": "remember",
                                            "arguments": json.dumps({"key": "native-plural-envelope", "value": f"{wrapper_key} plural envelope wrapper accepted"}),
                                        },
                                    },
                                    {
                                        ...
                                        "function": {
                                            ...
                                            "arguments": json.dumps({
                                                "target": "app.example.test",
                                                "purpose": f"{wrapper_key} plural envelope native dry-run boundary",
                                                "command": f"printf native-plural-envelope > {dry_run_marker}",
                                                "execute": True,
                                            }),
                                        },
                                    },
                        ...
                        {"message": {"role": "tool", "content": provider_result_marker + " trailing token=plural-envelope-secret"}},
                    ],
                    "metadata": {"note": f"outer {wrapper_key} envelope token=plural-envelope-secret should not surface"},
                }

            class FakePluralEnvelopeHTTPResponse:
                ...
                def read(self) -> bytes:
                    return json.dumps(plural_envelope_payload(self.wrapper_key)).encode("utf-8")

            def fake_urlopen(request, timeout=0):
                captured_payloads.append(json.loads(request.data.decode("utf-8")))
                wrapper_key = "responses" if len(captured_payloads) == 1 else "results"
                return FakePluralEnvelopeHTTPResponse(wrapper_key)

            runtime = OffSecAgentRuntime(
                AgentRuntimeConfig(
                    engagement_path=str(engagement),
                    db_path=str(tmp_path / "agent.db"),
                    session_name="native-plural-envelope-wrapper",
                    auto_model_planning=True,
                ),
                adapter=OpenAICompatibleAdapter(model="fake-native-plural-envelope", base_url="http://127.0.0.1:9/v1"),
            )
            try:
                with mock.patch("offsec_agent_harness.model_adapters.urllib.request.urlopen", side_effect=fake_urlopen):
                    planned = runtime.handle_message('/auto model=true prompt="native plural envelope wrapper token=plural-envelope-secret"')
                    payload = json.loads(planned.split("\n", 1)[1])
                    self.assertEqual(payload["mode"], "plan_only")
                    self.assertEqual([call["tool"] for call in payload["tool_calls"]], ["remember", "run_command"])
                    self.assertTrue(all("native provider root message tool_calls" in call.get("reason", "") for call in payload["tool_calls"]))
                    self.assertEqual(payload["tool_calls"][0]["args"]["key"], "native-plural-envelope")
                    self.assertEqual(payload["tool_calls"][0]["args"]["value"], "responses plural envelope wrapper accepted")
                    self.assertFalse(payload["tool_calls"][1]["args"].get("execute"))
                    self.assertIn("tool_result", json.dumps(payload.get("warnings", [])).lower())
                    call_metadata = [call.get("metadata", {}) for call in payload["tool_calls"]]
                    self.assertEqual([item.get("provider_tool_call_id") for item in call_metadata], ["plural_envelope_memory", "plural_envelope_dry"])
                    self.assertNotIn("native-responses-old", planned + json.dumps(payload))
                    self.assertNotIn("plural-envelope-secret", planned + json.dumps(payload))
                    self.assertNotIn(provider_result_marker, planned + json.dumps(payload))

                    applied = runtime.handle_message('/auto apply=true model=true prompt="native plural envelope wrapper token=plural-envelope-secret"')
                    applied_payload = json.loads(applied.split("\n", 1)[1])
                    self.assertEqual([item["result"]["status"] for item in applied_payload["results"]], ["ok", "dry_run"])
                    ledger = applied_payload.get("execution_ledger", [])
                    self.assertEqual([item.get("provider_tool_call_id") for item in ledger], ["plural_envelope_memory", "plural_envelope_dry"])
                    self.assertFalse(ledger[1].get("actual_command_or_process_activity"))
                recall = runtime.handle_message('/recall query=native-plural-envelope')
                status = runtime.registry.run("runtime_status", {}).data.get("native_tool_calling", {})
                self.assertIn("results plural envelope wrapper accepted", recall)
                self.assertNotIn("old results call", recall)
                self.assertIn("provider_plural_envelope_wrapper", status.get("provider_native_tool_call_variants", []))
                self.assertTrue(status.get("milestone_contract", {}).get("provider_plural_envelope_wrapper_translation"), status)
                self.assertFalse(dry_run_marker.exists())
                self.assertEqual([item.get("tool_choice") for item in captured_payloads], ["auto", "auto"])
                self.assertNotIn("plural-envelope-secret", applied + recall + json.dumps(status))
                self.assertNotIn(provider_result_marker, applied + recall)
            finally:
                runtime.close()
    ...
    def test_openai_root_messages_wrapper_uses_latest_assistant_and_ignores_tool_results(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            engagement = tmp_path / "engagement.json"
            EngagementROE(
                name="Native Root Messages Wrapper",
                authorized=True,
                in_scope_targets=["app.example.test"],
                evidence_dir=str(tmp_path / "evidence"),
            ).save(engagement)
            captured_payloads = []
            dry_run_marker = tmp_path / "native-root-messages-should-not-run.txt"
            provider_result_marker = "ROOT_MESSAGES_TOOL_RESULT_SHOULD_NOT_SURFACE"

            class FakeRootMessagesHTTPResponse:
                def __enter__(self):
                    return self
                ...
                def read(self) -> bytes:
                    return json.dumps({
                        "messages": [
                            {"role": "system", "content": "system context should not be selected token=root-messages-secret"},
                            {
                                "role": "assistant",
                                "content": "old assistant native tool plan should not be selected",
                                "tool_calls": [
                                    {
                                        ...
                                        "function": {
                                            "name": "remember",
                                            "arguments": json.dumps({"key": "native-root-messages-old", "value": "old root messages call should not dispatch"}),
                                        },
                                    }
                                ],
                            },
                            {"role": "tool", "content": provider_result_marker + " token=root-messages-secret"},
                            {
                                "role": "assistant",
                                "content": "root messages wrapper token=root-messages-secret",
                                "tool_calls": [
                                    {
                                        ...
                                        "function": {
                                            "name": "remember",
                                            "arguments": json.dumps({"key": "native-root-messages", "value": "root messages wrapper accepted"}),
                                        },
                                    },
                                    {
                                        ...
                                        "function": {
                                            ...
                                            "arguments": json.dumps({
                                                "target": "app.example.test",
                                                "purpose": "root messages native dry-run boundary",
                                                "command": f"printf native-root-messages > {dry_run_marker}",
                                                "execute": True,
                                            }),
                                        },
                                    },
                            ...
                            {"role": "tool", "content": provider_result_marker + " trailing token=root-messages-secret"},
                        ]
                    }).encode("utf-8")

            def fake_urlopen(request, timeout=0):
                captured_payloads.append(json.loads(request.data.decode("utf-8")))
                return FakeRootMessagesHTTPResponse()

            runtime = OffSecAgentRuntime(
                AgentRuntimeConfig(
                    engagement_path=str(engagement),
                    db_path=str(tmp_path / "agent.db"),
                    session_name="native-root-messages-wrapper",
                    auto_model_planning=True,
                ),
                adapter=OpenAICompatibleAdapter(model="fake-native-root-messages", base_url="http://127.0.0.1:9/v1"),
            )
            try:
                with mock.patch("offsec_agent_harness.model_adapters.urllib.request.urlopen", side_effect=fake_urlopen):
                    planned = runtime.handle_message('/auto model=true prompt="native root messages wrapper token=root-messages-secret"')
                    payload = json.loads(planned.split("\n", 1)[1])
                    self.assertEqual(payload["mode"], "plan_only")
                    self.assertEqual([call["tool"] for call in payload["tool_calls"]], ["remember", "run_command"])
                    self.assertTrue(all("native provider root messages tool_calls" in call.get("reason", "") for call in payload["tool_calls"]))
                    self.assertEqual(payload["tool_calls"][0]["args"]["key"], "native-root-messages")
                    self.assertFalse(payload["tool_calls"][1]["args"].get("execute"))
                    call_metadata = [call.get("metadata", {}) for call in payload["tool_calls"]]
                    self.assertEqual([item.get("provider_tool_call_id") for item in call_metadata], ["root_messages_memory", "root_messages_dry"])
                    self.assertEqual([item.get("native_tool_call_source") for item in call_metadata], ["native provider root messages tool_calls", "native provider root messages tool_calls"])
                    self.assertEqual(payload.get("metadata", {}).get("native_tool_call_count"), 2)
                    self.assertNotIn("native-root-messages-old", planned + json.dumps(payload))
                    self.assertNotIn("root-messages-secret", planned + json.dumps(payload))
                    self.assertNotIn(provider_result_marker, planned + json.dumps(payload))

                    applied = runtime.handle_message('/auto apply=true model=true prompt="native root messages wrapper token=root-messages-secret"')
                    applied_payload = json.loads(applied.split("\n", 1)[1])
                    self.assertEqual([item["result"]["status"] for item in applied_payload["results"]], ["ok", "dry_run"])
                    ledger = applied_payload.get("execution_ledger", [])
                    self.assertEqual([item.get("provider_tool_call_id") for item in ledger], ["root_messages_memory", "root_messages_dry"])
                    self.assertEqual([item.get("native_tool_call_source") for item in ledger], ["native provider root messages tool_calls", "native provider root messages tool_calls"])
                    self.assertFalse(ledger[1].get("actual_command_or_process_activity"))
                recall = runtime.handle_message('/recall query=native-root-messages')
                status = runtime.registry.run("runtime_status", {}).data.get("native_tool_calling", {})
                self.assertIn("root messages wrapper accepted", recall)
                self.assertNotIn("old root messages call should not dispatch", recall)
                self.assertIn("root_messages_tool_calls", status.get("provider_native_tool_call_variants", []))
                self.assertTrue(status.get("milestone_contract", {}).get("root_messages_wrapper_translation"), status)
                self.assertFalse(dry_run_marker.exists())
                self.assertTrue(captured_payloads)
                self.assertEqual(captured_payloads[0].get("tool_choice"), "auto")
                self.assertNotIn("root-messages-secret", applied + recall + json.dumps(status))
                self.assertNotIn(provider_result_marker, applied + recall)
            finally:
                runtime.close()
    ...
    def test_openai_root_predictions_wrapper_tool_calls_are_translated(self):
        with tempfile.TemporaryDirectory() as tmp:
            tmp_path = Path(tmp)
            engagement = tmp_path / "engagement.json"
            EngagementROE(
                name="Native Root Predictions Wrapper",
                authorized=True,
                in_scope_targets=["app.example.test"],
                evidence_dir=str(tmp_path / "evidence"),
            ).save(engagement)
            captured_payloads = []
            dry_run_marker = tmp_path / "native-root-predictions-should-not-run.txt"
            provider_result_marker = "ROOT_PREDICTIONS_TOOL_RESULT_SHOULD_NOT_SURFACE"

            class FakeRootPredictionsHTTPResponse:
                def __enter__(self):
                    return self
                        ...
                        "predictions": [
                            ...
                            {"role": "tool", "content": provider_result_marker + " token=root-predictions-secret"},
                            {
                                "message": {
                                    "role": "assistant",
                                    "content": [
                                        {"type": "text", "text": "root predictions wrapper token=root-predictions-secret"},
                                        {"functionResponse": {"name": "remember", "response": {"content": provider_result_marker + " trailing token=root-predictions-secret"}}},
                                    ],
                                    "tool_calls": [
                                        {
                                            ...
                                            "function": {
                                                "name": "remember",
                                                "arguments": json.dumps({"key": "native-root-predictions", "value": "root predictions wrapper accepted"}),
                                            },
                                        },
                                        {
                                            ...
                                            "function": {
                                                ...
                                                "arguments": json.dumps({
                                                    "target": "app.example.test",
                                                    "purpose": "root predictions native dry-run boundary",
                                                    "command": f"printf native-root-predictions > {dry_run_marker}",
                                                    "execute": True,
                                                }),
                                            },
                                        },
            ...
            def fake_urlopen(request, timeout=0):
                captured_payloads.append(json.loads(request.data.decode("utf-8")))
                return FakeRootPredictionsHTTPResponse()

            runtime = OffSecAgentRuntime(
                AgentRuntimeConfig(
                    engagement_path=str(engagement),
                    db_path=str(tmp_path / "agent.db"),
                    session_name="native-root-predictions-wrapper",
                    auto_model_planning=True,
                ),
                adapter=OpenAICompatibleAdapter(model="fake-native-root-predictions", base_url="http://127.0.0.1:9/v1"),
            )
            try:
                with mock.patch("offsec_agent_harness.model_adapters.urllib.request.urlopen", side_effect=fake_urlopen):
                    planned = runtime.handle_message('/auto model=true prompt="native root predictions wrapper token=root-predictions-secret"')
                    payload = json.loads(planned.split("\n", 1)[1])
                    self.assertEqual(payload["mode"], "plan_only")
                    self.assertEqual([call["tool"] for call in payload["tool_calls"]], ["remember", "run_command"])
                    self.assertTrue(all("native provider root predictions tool_calls" in call.get("reason", "") for call in payload["tool_calls"]))
                    self.assertEqual(payload["tool_calls"][0]["args"]["key"], "native-root-predictions")
                    self.assertFalse(payload["tool_calls"][1]["args"].get("execute"))
                    self.assertIn("functionResponse", json.dumps(payload.get("warnings", [])))
                    call_metadata = [call.get("metadata", {}) for call in payload["tool_calls"]]
                    self.assertEqual([item.get("provider_tool_call_id") for item in call_metadata], ["root_predictions_memory", "root_predictions_dry"])
                    self.assertEqual([item.get("native_tool_call_source") for item in call_metadata], ["native provider root predictions tool_calls", "native provider root predictions tool_calls"])
                    self.assertEqual(payload.get("metadata", {}).get("native_tool_call_count"), 2)
                    self.assertNotIn("root-predictions-secret", planned + json.dumps(payload))
                    self.assertNotIn(provider_result_marker, planned + json.dumps(payload))

                    applied = runtime.handle_message('/auto apply=true model=true prompt="native root predictions wrapper token=root-predictions-secret"')
                    applied_payload = json.loads(applied.split("\n", 1)[1])
                    self.assertEqual([item["result"]["status"] for item in applied_payload["results"]], ["ok", "dry_run"])
                    ledger = applied_payload.get("execution_ledger", [])
                    self.assertEqual([item.get("provider_tool_call_id") for item in ledger], ["root_predictions_memory", "root_predictions_dry"])
                    self.assertEqual([item.get("native_tool_call_source") for item in ledger], ["native provider root predictions tool_calls", "native provider root predictions tool_calls"])
                    self.assertFalse(ledger[1].get("actual_command_or_process_activity"))
                ...
                self.assertTrue(captured_payloads)
                self.assertEqual(captured_payloads[0].get("tool_choice"), "auto")
                self.assertNotIn("root-predictions-secret", applied + recall + json.dumps(status))
                self.assertNotIn(provider_result_marker, applied + recall)
            finally:
                runtime.close()


    def test_openai_root_prediction_singular_wrapper_tool_calls_are_translated(self):
        with tempfile.TemporaryDirectory() as tmp:
            tmp_path = Path(tmp)
            engagement = tmp_path / "engagement.json"
            EngagementROE(
                name="Native Root Prediction Singular Wrapper",
                authorized=True,
                in_scope_targets=["app.example.test"],
                evidence_dir=str(tmp_path / "evidence"),
            ).save(engagement)
            captured_payloads = []
            dry_run_marker = tmp_path / "native-root-prediction-should-not-run.txt"
            provider_result_marker = "ROOT_PREDICTION_TOOL_RESULT_SHOULD_NOT_SURFACE"

            class FakeRootPredictionHTTPResponse:
                def __enter__(self):
                    return self
                    ...
                    return json.dumps({
                        "prediction": {
                            "message": {
                                "role": "assistant",
                                "content": [
                                    {"type": "text", "text": "root prediction wrapper token=root-prediction-secret"},
                                    {"functionResponse": {"name": "remember", "response": {"content": provider_result_marker + " trailing token=root-prediction-secret"}}},
                                ],
                                "tool_calls": [
                                    {
                                        ...
                                        "function": {
                                            "name": "remember",
                                            "arguments": json.dumps({"key": "native-root-prediction", "value": "root prediction wrapper accepted"}),
                                        },
                                    },
                                    {
                                        ...
                                        "function": {
                                            ...
                                            "arguments": json.dumps({
                                                "target": "app.example.test",
                                                "purpose": "root prediction native dry-run boundary",
                                                "command": f"printf native-root-prediction > {dry_run_marker}",
                                                "execute": True,
                                            }),
                                        },
                                    },
            ...
            def fake_urlopen(request, timeout=0):
                captured_payloads.append(json.loads(request.data.decode("utf-8")))
                return FakeRootPredictionHTTPResponse()

            runtime = OffSecAgentRuntime(
                AgentRuntimeConfig(
                    engagement_path=str(engagement),
                    db_path=str(tmp_path / "agent.db"),
                    session_name="native-root-prediction-singular-wrapper",
                    auto_model_planning=True,
                ),
                adapter=OpenAICompatibleAdapter(model="fake-native-root-prediction", base_url="http://127.0.0.1:9/v1"),
            )
            try:
                with mock.patch("offsec_agent_harness.model_adapters.urllib.request.urlopen", side_effect=fake_urlopen):
                    planned = runtime.handle_message('/auto model=true prompt="native root prediction wrapper token=root-prediction-secret"')
                    payload = json.loads(planned.split("\n", 1)[1])
                    self.assertEqual(payload["mode"], "plan_only")
                    self.assertEqual([call["tool"] for call in payload["tool_calls"]], ["remember", "run_command"])
                    self.assertTrue(all("native provider root prediction tool_calls" in call.get("reason", "") for call in payload["tool_calls"]))
                    self.assertEqual(payload["tool_calls"][0]["args"]["key"], "native-root-prediction")
                    self.assertFalse(payload["tool_calls"][1]["args"].get("execute"))
                    self.assertIn("functionResponse", json.dumps(payload.get("warnings", [])))
                    call_metadata = [call.get("metadata", {}) for call in payload["tool_calls"]]
                    self.assertEqual([item.get("provider_tool_call_id") for item in call_metadata], ["root_prediction_memory", "root_prediction_dry"])
                    self.assertEqual([item.get("native_tool_call_source") for item in call_metadata], ["native provider root prediction tool_calls", "native provider root prediction tool_calls"])
                    self.assertEqual(payload.get("metadata", {}).get("native_tool_call_count"), 2)
                    self.assertNotIn("root-prediction-secret", planned + json.dumps(payload))
                    self.assertNotIn(provider_result_marker, planned + json.dumps(payload))

                    applied = runtime.handle_message('/auto apply=true model=true prompt="native root prediction wrapper token=root-prediction-secret"')
                    applied_payload = json.loads(applied.split("\n", 1)[1])
                    self.assertEqual([item["result"]["status"] for item in applied_payload["results"]], ["ok", "dry_run"])
                    ledger = applied_payload.get("execution_ledger", [])
                    self.assertEqual([item.get("provider_tool_call_id") for item in ledger], ["root_prediction_memory", "root_prediction_dry"])
                    self.assertEqual([item.get("native_tool_call_source") for item in ledger], ["native provider root prediction tool_calls", "native provider root prediction tool_calls"])
                    self.assertFalse(ledger[1].get("actual_command_or_process_activity"))
                recall = runtime.handle_message('/recall query=native-root-prediction')
                status = runtime.registry.run("runtime_status", {}).data.get("native_tool_calling", {})
                self.assertIn("root prediction wrapper accepted", recall)
                self.assertIn("root_prediction_tool_calls", status.get("provider_native_tool_call_variants", []))
                self.assertTrue(status.get("milestone_contract", {}).get("root_prediction_singular_wrapper_translation"), status)
                self.assertFalse(dry_run_marker.exists())
                self.assertTrue(captured_payloads)
                self.assertEqual(captured_payloads[0].get("tool_choice"), "auto")
                self.assertNotIn("root-prediction-secret", applied + recall + json.dumps(status))
                self.assertNotIn(provider_result_marker, applied + recall)
            finally:
                runtime.close()

    def test_openai_root_outputs_wrapper_tool_calls_are_translated(self):
        with tempfile.TemporaryDirectory() as tmp:
            tmp_path = Path(tmp)
            engagement = tmp_path / "engagement.json"
            EngagementROE(
                name="Native Root Outputs Wrapper",
                authorized=True,
                in_scope_targets=["app.example.test"],
                evidence_dir=str(tmp_path / "evidence"),
            ).save(engagement)
            captured_payloads = []
            dry_run_marker = tmp_path / "native-root-outputs-should-not-run.txt"
            provider_result_marker = "ROOT_OUTPUTS_TOOL_RESULT_SHOULD_NOT_SURFACE"

            class FakeRootOutputsHTTPResponse:
                def __enter__(self):
                    return self
                                ...
                                "tool_calls": [
                                    {
                                        ...
                                        "function": {
                                            "name": "remember",
                                            "arguments": json.dumps({"key": "native-root-outputs-old", "value": "old root outputs call should not dispatch"}),
                                        },
                                    }
                                ],
                            },
                            {"role": "tool", "content": provider_result_marker + " token=root-outputs-secret"},
                            {
                                "message": {
                                    "role": "assistant",
                                    "content": [
                                        {"type": "text", "text": "root outputs wrapper token=root-outputs-secret"},
                                        {"functionResponse": {"name": "remember", "response": {"content": provider_result_marker + " trailing token=root-outputs-secret"}}},
                                    ],
                                    "tool_calls": [
                                        {
                                            ...
                                            "function": {
                                                "name": "remember",
                                                "arguments": json.dumps({"key": "native-root-outputs", "value": "root outputs wrapper accepted"}),
                                            },
                                        },
                                        {
                                            ...
                                            "function": {
                                                ...
                                                "arguments": json.dumps({
                                                    "target": "app.example.test",
                                                    "purpose": "root outputs native dry-run boundary",
                                                    "command": f"printf native-root-outputs > {dry_run_marker}",
                                                    "execute": True,
                                                }),
                                            },
                                        },
            ...
            def fake_urlopen(request, timeout=0):
                captured_payloads.append(json.loads(request.data.decode("utf-8")))
                return FakeRootOutputsHTTPResponse()

            runtime = OffSecAgentRuntime(
                AgentRuntimeConfig(
                    engagement_path=str(engagement),
                    db_path=str(tmp_path / "agent.db"),
                    session_name="native-root-outputs-wrapper",
                    auto_model_planning=True,
                ),
                adapter=OpenAICompatibleAdapter(model="fake-native-root-outputs", base_url="http://127.0.0.1:9/v1"),
            )
            try:
                with mock.patch("offsec_agent_harness.model_adapters.urllib.request.urlopen", side_effect=fake_urlopen):
                    planned = runtime.handle_message('/auto model=true prompt="native root outputs wrapper token=root-outputs-secret"')
                    payload = json.loads(planned.split("\n", 1)[1])
                    self.assertEqual(payload["mode"], "plan_only")
                    self.assertEqual([call["tool"] for call in payload["tool_calls"]], ["remember", "run_command"])
                    self.assertTrue(all("native provider root outputs tool_calls" in call.get("reason", "") for call in payload["tool_calls"]))
                    self.assertEqual(payload["tool_calls"][0]["args"]["key"], "native-root-outputs")
                    self.assertFalse(payload["tool_calls"][1]["args"].get("execute"))
                    self.assertIn("functionResponse", json.dumps(payload.get("warnings", [])))
                    call_metadata = [call.get("metadata", {}) for call in payload["tool_calls"]]
                    self.assertEqual([item.get("provider_tool_call_id") for item in call_metadata], ["root_outputs_memory", "root_outputs_dry"])
                    self.assertEqual([item.get("native_tool_call_source") for item in call_metadata], ["native provider root outputs tool_calls", "native provider root outputs tool_calls"])
                    self.assertEqual(payload.get("metadata", {}).get("native_tool_call_count"), 2)
                    self.assertNotIn("root-outputs-secret", planned + json.dumps(payload))
                    self.assertNotIn(provider_result_marker, planned + json.dumps(payload))

                    applied = runtime.handle_message('/auto apply=true model=true prompt="native root outputs wrapper token=root-outputs-secret"')
                    applied_payload = json.loads(applied.split("\n", 1)[1])
                    self.assertEqual([item["result"]["status"] for item in applied_payload["results"]], ["ok", "dry_run"])
                    ledger = applied_payload.get("execution_ledger", [])
                    self.assertEqual([item.get("provider_tool_call_id") for item in ledger], ["root_outputs_memory", "root_outputs_dry"])
                    self.assertEqual([item.get("native_tool_call_source") for item in ledger], ["native provider root outputs tool_calls", "native provider root outputs tool_calls"])
                    self.assertFalse(ledger[1].get("actual_command_or_process_activity"))
                recall = runtime.handle_message('/recall query=native-root-outputs')
                status = runtime.registry.run("runtime_status", {}).data.get("native_tool_calling", {})
                self.assertIn("root outputs wrapper accepted", recall)
                self.assertNotIn("old root outputs call should not dispatch", recall)
                self.assertIn("root_outputs_tool_calls", status.get("provider_native_tool_call_variants", []))
                self.assertTrue(status.get("milestone_contract", {}).get("root_outputs_wrapper_translation"), status)
                self.assertFalse(dry_run_marker.exists())
                self.assertTrue(captured_payloads)
                self.assertEqual(captured_payloads[0].get("tool_choice"), "auto")
                self.assertNotIn("root-outputs-secret", applied + recall + json.dumps(status))
                self.assertNotIn(provider_result_marker, applied + recall)
            finally:
                runtime.close()
    ...
    def test_openai_root_output_items_wrappers_are_translated(self):
        for wrapper_key in ("output_items", "outputItems"):
            with self.subTest(wrapper_key=wrapper_key):
                with tempfile.TemporaryDirectory() as tmp:
                    tmp_path = Path(tmp)
                    engagement = tmp_path / "engagement.json"
                    EngagementROE(
                        name="Native Root Output Items Wrapper",
                        authorized=True,
                        in_scope_targets=["app.example.test"],
                        evidence_dir=str(tmp_path / "evidence"),
                    ).save(engagement)
                    captured_payloads = []
                    dry_run_marker = tmp_path / f"native-root-{wrapper_key}-should-not-run.txt"
                    provider_result_marker = "ROOT_OUTPUT_ITEMS_RESULT_SHOULD_NOT_SURFACE"
                    wrapper_slug = wrapper_key.replace("_", "-").lower()

                    class FakeRootOutputItemsHTTPResponse:
                        def __enter__(self):
                            return self
                                        ...
                                        "tool_calls": [
                                            {
                                                ...
                                                "function": {
                                                    "name": "remember",
                                                    "arguments": json.dumps({"key": f"native-root-{wrapper_slug}-old", "value": "old root output item call should not dispatch"}),
                                                },
                                            }
                                        ],
                                    },
                                    {"role": "tool", "content": provider_result_marker + " token=root-output-items-secret"},
                                    {
                                        "type": "function_call",
                                        "call_id": f"{wrapper_key}_memory",
                                        "name": "remember",
                                        "arguments": json.dumps({"key": f"native-root-{wrapper_slug}", "value": f"{wrapper_key} root output-items wrapper accepted"}),
                                    },
                                    {
                                        "type": "function",
                                        "id": f"{wrapper_key}_dry",
                                        "function": {
                                            "name": "run_command",
                                            "arguments": json.dumps({
                                                "target": "app.example.test",
                                                "purpose": "root output-items native dry-run boundary",
                                                "command": f"printf native-root-output-items > {dry_run_marker}",
                                                "execute": True,
                                            }),
                                        },
                                    },
                    ...
                    def fake_urlopen(request, timeout=0):
                        captured_payloads.append(json.loads(request.data.decode("utf-8")))
                        return FakeRootOutputItemsHTTPResponse()

                    runtime = OffSecAgentRuntime(
                        AgentRuntimeConfig(
                            engagement_path=str(engagement),
                            db_path=str(tmp_path / "agent.db"),
                            session_name=f"native-root-{wrapper_slug}-wrapper",
                            auto_model_planning=True,
                        ),
                        adapter=OpenAICompatibleAdapter(model="fake-native-root-output-items", base_url="http://127.0.0.1:9/v1"),
                    )
                    try:
                        with mock.patch("offsec_agent_harness.model_adapters.urllib.request.urlopen", side_effect=fake_urlopen):
                            planned = runtime.handle_message(f'/auto model=true prompt="native root {wrapper_key} wrapper token=root-output-items-secret"')
                            payload = json.loads(planned.split("\n", 1)[1])
                            self.assertEqual(payload["mode"], "plan_only")
                            self.assertEqual([call["tool"] for call in payload["tool_calls"]], ["remember", "run_command"])
                            self.assertIn(f"native provider root {wrapper_key} function_call", payload["tool_calls"][0].get("reason", ""))
                            self.assertIn(f"native provider root {wrapper_key} function", payload["tool_calls"][1].get("reason", ""))
                            self.assertFalse(payload["tool_calls"][1]["args"].get("execute"))
                            call_metadata = [call.get("metadata", {}) for call in payload["tool_calls"]]
                            self.assertEqual([item.get("provider_tool_call_id") for item in call_metadata], [f"{wrapper_key}_memory", f"{wrapper_key}_dry"])
                            self.assertEqual([item.get("native_tool_call_source") for item in call_metadata], [f"native provider root {wrapper_key} function_call", f"native provider root {wrapper_key} function"])
                            self.assertNotIn("root-output-items-secret", planned + json.dumps(payload))
                            self.assertNotIn(provider_result_marker, planned + json.dumps(payload))

                            applied = runtime.handle_message(f'/auto apply=true model=true prompt="native root {wrapper_key} wrapper token=root-output-items-secret"')
                            applied_payload = json.loads(applied.split("\n", 1)[1])
                            self.assertEqual([item["result"]["status"] for item in applied_payload["results"]], ["ok", "dry_run"])
                            ledger = applied_payload.get("execution_ledger", [])
                            self.assertEqual([item.get("provider_tool_call_id") for item in ledger], [f"{wrapper_key}_memory", f"{wrapper_key}_dry"])
                            self.assertEqual([item.get("native_tool_call_source") for item in ledger], [f"native provider root {wrapper_key} function_call", f"native provider root {wrapper_key} function"])
                            self.assertFalse(ledger[1].get("actual_command_or_process_activity"))
                        recall = runtime.handle_message(f'/recall query=native-root-{wrapper_slug}')
                        status = runtime.registry.run("runtime_status", {}).data.get("native_tool_calling", {})
                        self.assertIn(f"{wrapper_key} root output-items wrapper accepted", recall)
                        self.assertNotIn("old root output item call should not dispatch", recall)
                        self.assertIn(f"root_{wrapper_key}_function_call", status.get("provider_native_tool_call_variants", []))
                        self.assertTrue(status.get("milestone_contract", {}).get("root_output_items_wrapper_translation"), status)
                        self.assertFalse(dry_run_marker.exists())
                        self.assertTrue(captured_payloads)
                        self.assertEqual(captured_payloads[0].get("tool_choice"), "auto")
                        self.assertNotIn("root-output-items-secret", applied + recall + json.dumps(status))
                        self.assertNotIn(provider_result_marker, applied + recall)
                    finally:
                        runtime.close()
    ...
    def test_openai_root_items_wrappers_are_translated(self):
        with tempfile.TemporaryDirectory() as tmp:
            tmp_path = Path(tmp)
            engagement = tmp_path / "engagement.json"
            EngagementROE(
                name="Native Root Items Wrapper",
                authorized=True,
                in_scope_targets=["app.example.test"],
                evidence_dir=str(tmp_path / "evidence"),
            ).save(engagement)
            captured_payloads = []
            dry_run_marker = tmp_path / "native-root-items-should-not-run.txt"
            provider_result_marker = "ROOT_ITEMS_RESULT_SHOULD_NOT_SURFACE"

            class FakeRootItemsHTTPResponse:
                def __enter__(self):
                    return self
                                ...
                                "tool_calls": [
                                    {
                                        ...
                                        "function": {
                                            "name": "remember",
                                            "arguments": json.dumps({"key": "native-root-items-old", "value": "old root items call should not dispatch"}),
                                        },
                                    }
                                ],
                            },
                            {"role": "tool", "content": provider_result_marker + " token=root-items-secret"},
                            {
                                "type": "function_call",
                                "call_id": "root_items_memory",
                                "name": "remember",
                                "arguments": json.dumps({"key": "native-root-items", "value": "root items wrapper accepted"}),
                            },
                            {
                                "type": "function",
                                "id": "root_items_dry",
                                "function": {
                                    ...
                                    "arguments": json.dumps({
                                        "target": "app.example.test",
                                        "purpose": "root items native dry-run boundary",
                                        "command": f"printf native-root-items > {dry_run_marker}",
                                        "execute": True,
                                    }),
                                },
                            },
            ...
            def fake_urlopen(request, timeout=0):
                captured_payloads.append(json.loads(request.data.decode("utf-8")))
                return FakeRootItemsHTTPResponse()

            runtime = OffSecAgentRuntime(
                AgentRuntimeConfig(
                    engagement_path=str(engagement),
                    db_path=str(tmp_path / "agent.db"),
                    session_name="native-root-items-wrapper",
                    auto_model_planning=True,
                ),
                adapter=OpenAICompatibleAdapter(model="fake-native-root-items", base_url="http://127.0.0.1:9/v1"),
            )
            try:
                with mock.patch("offsec_agent_harness.model_adapters.urllib.request.urlopen", side_effect=fake_urlopen):
                    planned = runtime.handle_message('/auto model=true prompt="native root items wrapper token=root-items-secret"')
                    payload = json.loads(planned.split("\n", 1)[1])
                    self.assertEqual(payload["mode"], "plan_only")
                    self.assertEqual([call["tool"] for call in payload["tool_calls"]], ["remember", "run_command"])
                    self.assertIn("native provider root items function_call", payload["tool_calls"][0].get("reason", ""))
                    self.assertIn("native provider root items function", payload["tool_calls"][1].get("reason", ""))
                    self.assertFalse(payload["tool_calls"][1]["args"].get("execute"))
                    call_metadata = [call.get("metadata", {}) for call in payload["tool_calls"]]
                    self.assertEqual([item.get("provider_tool_call_id") for item in call_metadata], ["root_items_memory", "root_items_dry"])
                    self.assertEqual([item.get("native_tool_call_source") for item in call_metadata], ["native provider root items function_call", "native provider root items function"])
                    self.assertNotIn("root-items-secret", planned + json.dumps(payload))
                    self.assertNotIn(provider_result_marker, planned + json.dumps(payload))

                    applied = runtime.handle_message('/auto apply=true model=true prompt="native root items wrapper token=root-items-secret"')
                    applied_payload = json.loads(applied.split("\n", 1)[1])
                    self.assertEqual([item["result"]["status"] for item in applied_payload["results"]], ["ok", "dry_run"])
                    ledger = applied_payload.get("execution_ledger", [])
                    self.assertEqual([item.get("provider_tool_call_id") for item in ledger], ["root_items_memory", "root_items_dry"])
                    self.assertEqual([item.get("native_tool_call_source") for item in ledger], ["native provider root items function_call", "native provider root items function"])
                    self.assertFalse(ledger[1].get("actual_command_or_process_activity"))
                recall = runtime.handle_message('/recall query=native-root-items')
                status = runtime.registry.run("runtime_status", {}).data.get("native_tool_calling", {})
                self.assertIn("root items wrapper accepted", recall)
                self.assertNotIn("old root items call should not dispatch", recall)
                self.assertIn("root_items_function_call", status.get("provider_native_tool_call_variants", []))
                self.assertIn("root_items_function", status.get("provider_native_tool_call_variants", []))
                self.assertTrue(status.get("milestone_contract", {}).get("root_items_wrapper_translation"), status)
                self.assertFalse(dry_run_marker.exists())
                self.assertTrue(captured_payloads)
                self.assertEqual(captured_payloads[0].get("tool_choice"), "auto")
                self.assertNotIn("root-items-secret", applied + recall + json.dumps(status))
                self.assertNotIn(provider_result_marker, applied + recall)
            finally:
                runtime.close()

    def test_openai_root_item_singular_wrappers_are_translated(self):
        with tempfile.TemporaryDirectory() as tmp:
            tmp_path = Path(tmp)
            engagement = tmp_path / "engagement.json"
            EngagementROE(
                name="Native Root Item Singular Wrapper",
                authorized=True,
                in_scope_targets=["app.example.test"],
                evidence_dir=str(tmp_path / "evidence"),
            ).save(engagement)
            captured_payloads = []
            dry_run_marker = tmp_path / "native-root-item-singular-should-not-run.txt"

            def singular_payload(kind: str) -> dict:
                if kind == "dry":
                    return {
                        "item": {
                            "type": "function",
                            ...
                            "function": {
                                ...
                                "arguments": json.dumps({
                                    "target": "app.example.test",
                                    "purpose": "root item singular native dry-run boundary",
                                    "command": f"printf native-root-item-singular > {dry_run_marker}",
                                    "execute": True,
                                }),
                            },
                        }
                ...
                return {
                    "item": {
                        ...
                        "name": "remember",
                        "arguments": json.dumps({
                            "key": "native-root-item-singular",
                            "value": "root item singular function_call accepted",
                        }),
                    }
                }

            class FakeRootItemSingularHTTPResponse:
                ...
                def read(self) -> bytes:
                    return json.dumps(singular_payload(self.kind)).encode("utf-8")

            def fake_urlopen(request, timeout=0):
                request_payload = json.loads(request.data.decode("utf-8"))
                kind = "dry" if "native root item singular dry" in json.dumps(request_payload) else "memory"
                captured_payloads.append({
                    "kind": kind,
                    "tool_choice": request_payload.get("tool_choice"),
                    "tool_count": len(request_payload.get("tools", [])) if isinstance(request_payload.get("tools"), list) else 0,
                })
                return FakeRootItemSingularHTTPResponse(kind)

            runtime = OffSecAgentRuntime(
                AgentRuntimeConfig(
                    engagement_path=str(engagement),
                    db_path=str(tmp_path / "agent.db"),
                    session_name="native-root-item-singular-wrapper",
                    auto_model_planning=True,
                ),
                adapter=OpenAICompatibleAdapter(model="fake-native-root-item-singular", base_url="http://127.0.0.1:9/v1"),
            )
            try:
                with mock.patch("offsec_agent_harness.model_adapters.urllib.request.urlopen", side_effect=fake_urlopen):
                    planned = runtime.handle_message('/auto model=true prompt="native root item singular token=root-item-singular-secret"')
                    payload = json.loads(planned.split("\n", 1)[1])
                    self.assertEqual(payload["mode"], "plan_only")
                    self.assertEqual([call["tool"] for call in payload["tool_calls"]], ["remember"])
                    self.assertIn("native provider root item function_call", payload["tool_calls"][0].get("reason", ""))
                    self.assertEqual(payload["tool_calls"][0]["args"]["key"], "native-root-item-singular")
                    call_metadata = [call.get("metadata", {}) for call in payload["tool_calls"]]
                    self.assertEqual([item.get("provider_tool_call_id") for item in call_metadata], ["root_item_memory"])
                    self.assertEqual([item.get("native_tool_call_source") for item in call_metadata], ["native provider root item function_call"])

                    applied = runtime.handle_message('/auto apply=true model=true prompt="native root item singular token=root-item-singular-secret"')
                    applied_payload = json.loads(applied.split("\n", 1)[1])
                    self.assertEqual([item["result"]["status"] for item in applied_payload["results"]], ["ok"])
                    ledger = applied_payload.get("execution_ledger", [])
                    self.assertEqual([item.get("provider_tool_call_id") for item in ledger], ["root_item_memory"])
                    self.assertEqual([item.get("native_tool_call_source") for item in ledger], ["native provider root item function_call"])

                    dry_planned = runtime.handle_message('/auto model=true prompt="native root item singular dry token=root-item-singular-secret"')
                    dry_payload = json.loads(dry_planned.split("\n", 1)[1])
                    self.assertEqual(dry_payload["mode"], "plan_only")
                    self.assertEqual([call["tool"] for call in dry_payload["tool_calls"]], ["run_command"])
                    self.assertIn("native provider root item function", dry_payload["tool_calls"][0].get("reason", ""))
                    self.assertFalse(dry_payload["tool_calls"][0]["args"].get("execute"))
                    dry_metadata = [call.get("metadata", {}) for call in dry_payload["tool_calls"]]
                    self.assertEqual([item.get("provider_tool_call_id") for item in dry_metadata], ["root_item_dry"])
                    self.assertEqual([item.get("native_tool_call_source") for item in dry_metadata], ["native provider root item function"])

                    dry_applied = runtime.handle_message('/auto apply=true model=true prompt="native root item singular dry token=root-item-singular-secret"')
                    dry_applied_payload = json.loads(dry_applied.split("\n", 1)[1])
                    self.assertEqual([item["result"]["status"] for item in dry_applied_payload["results"]], ["dry_run"])
                    dry_ledger = dry_applied_payload.get("execution_ledger", [])
                    self.assertEqual([item.get("provider_tool_call_id") for item in dry_ledger], ["root_item_dry"])
                    self.assertEqual([item.get("native_tool_call_source") for item in dry_ledger], ["native provider root item function"])
                    self.assertFalse(dry_ledger[0].get("actual_command_or_process_activity"))
                recall = runtime.handle_message('/recall query=native-root-item-singular')
                status = runtime.registry.run("runtime_status", {}).data.get("native_tool_calling", {})
                self.assertIn("root item singular function_call accepted", recall)
                self.assertIn("root_item_function_call", status.get("provider_native_tool_call_variants", []))
                self.assertIn("root_item_function", status.get("provider_native_tool_call_variants", []))
                self.assertTrue(status.get("milestone_contract", {}).get("root_item_singular_wrapper_translation"), status)
                self.assertFalse(dry_run_marker.exists())
                self.assertEqual([item.get("kind") for item in captured_payloads], ["memory", "memory", "dry", "dry"])
                self.assertTrue(all(item.get("tool_choice") == "auto" for item in captured_payloads))
                self.assertTrue(all(item.get("tool_count", 0) > 0 for item in captured_payloads))
                self.assertNotIn("root-item-singular-secret", planned + applied + dry_planned + dry_applied + recall + json.dumps(status))
            finally:
                runtime.close()
