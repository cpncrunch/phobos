...
class OffSecToolRegistry:
    ...
    def _register_builtins(self) -> None:
        ...
        self.register_tool("run_command", self.run_command, _spec("run_command", "Run a short shell command through ROE guardrails; confirm-level actions are queued for approval.", {
            ...
            "timeout": _integer("Foreground timeout in seconds.", minimum=1),
        }, ["target", "purpose", "command"]))
        ...
        self.register_tool("wait_process", self.wait_process, _spec("wait_process", "Wait up to timeout seconds for a background process to complete, then return status and log tails.", {"id": _integer("Current-session process id.", minimum=1), "timeout": _integer("Seconds to wait; 0 performs an immediate status check.", minimum=0), "limit": _integer("Maximum log-tail bytes to return.", minimum=1)}, ["id"]))
        self.register_tool("process_log", self.process_log, _spec("process_log", "Read redacted stdout/stderr tails for a background process.", {"id": _integer("Current-session process id.", minimum=1), "limit": _integer("Maximum log-tail bytes to return.", minimum=1)}, ["id"]))
        ...
        self.register_tool("list_processes", self.list_processes, _spec("list_processes", "List tracked background processes for the current session.", {"limit": _integer("Maximum rows to return.", minimum=1)}))
        ...
        self.register_tool("nmap_scan", self.nmap_scan, _spec("nmap_scan", "ROE-gated nmap-style service enumeration wrapper with structured parsing and evidence artifacts.", {"target": _string("In-scope host/IP/CIDR."), "ports": _string("Optional comma/range ports, e.g. 80,443,8000-8010."), "profile": _string_enum("safe|version|quick; default version.", _NMAP_PROFILE_VALUES), "stdout": _string("Optional captured output to parse without executing."), "input_file": _string("Optional output file to parse without executing."), "execute": {"type": "boolean"}, "timeout": _integer("Execution timeout in seconds.", minimum=1)}, ["target"]))
        self.register_tool("httpx_probe", self.httpx_probe, _spec("httpx_probe", "ROE-gated httpx-style HTTP probing wrapper with JSON/plaintext parsing and evidence artifacts.", {"url": _string("In-scope URL or host."), "target": _string("Alias for url."), "stdout": _string("Optional captured output to parse without executing."), "input_file": _string("Optional output file to parse without executing."), "execute": {"type": "boolean"}, "timeout": _integer("Execution timeout in seconds.", minimum=1)}, []))
        self.register_tool("nuclei_scan", self.nuclei_scan, _spec("nuclei_scan", "ROE-gated nuclei wrapper. Real execution requires an explicit safe template path; parser/dry-run paths remain available without nuclei installed.", {"url": _string("In-scope URL or host."), "target": _string("Alias for url."), "templates": _string("Template file/directory for execution; required when execute=true."), "template": _string("Alias for templates."), "rate_limit": _integer("Maximum requests per second for execution.", minimum=1), "stdout": _string("Optional captured JSONL/plain output to parse without executing."), "input_file": _string("Optional output file to parse without executing."), "execute": {"type": "boolean"}, "timeout": _integer("Execution timeout in seconds.", minimum=1)}, []))
        self.register_tool("ffuf_scan", self.ffuf_scan, _spec("ffuf_scan", "ROE-gated ffuf-style content discovery wrapper with conservative rate limits and structured evidence.", {"url": _string("In-scope URL containing FUZZ or base URL where /FUZZ is appended."), "wordlist": _string("Wordlist path required for execution."), "rate": _integer("Maximum requests per second for execution.", minimum=1), "stdout": _string("Optional captured JSON output to parse without executing."), "input_file": _string("Optional output file to parse without executing."), "execute": {"type": "boolean"}, "timeout": _integer("Execution timeout in seconds.", minimum=1)}, ["url"]))
        self.register_tool("list_tool_runs", self.list_tool_runs, _spec("list_tool_runs", "List structured wrapper runs and their parsed evidence artifacts.", {"limit": _integer("Maximum rows to return.", minimum=1), "tool_name": _string("Optional wrapper tool name filter.")}, []))
        ...
        self.register_tool("list_findings", self.list_findings, _spec("list_findings", "List finding lifecycle records.", {"status": _string_enum("draft/needs-evidence/confirmed/resolved/accepted-risk/false-positive/all; default all.", _FINDING_LIST_STATUS_VALUES), "limit": _integer("Maximum rows to return.", minimum=1)}, []))
        ...
        self.register_tool("recall", self.recall, _spec("recall", "Search local agent memory.", {"query": _string("Memory search query."), "limit": _integer("Maximum rows to return.", minimum=1)}, ["query"]))
        self.register_tool("list_memories", self.list_memories, _spec("list_memories", "List redacted local agent memory keys/values for hygiene review.", {"query": _string("Optional memory search query."), "limit": _integer("Maximum rows to return.", minimum=1)}, []))
        ...
        self.register_tool("search_session", self.search_session, _spec("search_session", "Search current-session messages.", {"query": _string("Message search query."), "limit": _integer("Maximum rows to return.", minimum=1)}, ["query"]))
        self.register_tool("search_all_sessions", self.search_all_sessions, _spec("search_all_sessions", "Search messages across all local Phobos sessions in this DB.", {"query": _string("Message search query."), "limit": _integer("Maximum rows to return.", minimum=1)}, ["query"]))
        self.register_tool("context_snapshot", self.context_snapshot, _spec("context_snapshot", "Return latest compact summary, recent messages, and relevant memory.", {"query": _string("Optional relevance query."), "limit": _integer("Maximum rows to return.", minimum=1)}))
        self.register_tool("compact_context", self.compact_context, _spec("compact_context", "Summarize recent session messages into durable local context.", {"limit": _integer("Maximum recent messages to summarize.", minimum=1)}))
        self.register_tool("context_compact_node", self.context_compact_node, _spec("context_compact_node", "Create an LCM-style context node from recent messages and optionally roll child nodes into a parent.", {"limit": _integer("Maximum recent messages to summarize.", minimum=1), "title": _string("Optional node title."), "parent": {"type": "boolean"}}, []))
        self.register_tool("context_describe", self.context_describe, _spec("context_describe", "Describe local LCM-style context nodes without expanding full sources.", {"id": _integer("Current-session context node id.", minimum=1), "limit": _integer("Maximum nodes to return.", minimum=1)}, []))
        self.register_tool("context_expand", self.context_expand, _spec("context_expand", "Expand a local context node and recover its source messages/child summaries.", {"id": _integer("Current-session context node id.", minimum=1), "source_limit": _integer("Maximum source rows to include.", minimum=1)}, ["id"]))
        self.register_tool("lcm_compact", self.context_compact_node, _spec("lcm_compact", "Alias for context_compact_node.", {"limit": _integer("Maximum recent messages to summarize.", minimum=1), "title": _string("Optional node title."), "parent": {"type": "boolean"}}, []))
        self.register_tool("lcm_describe", self.context_describe, _spec("lcm_describe", "Alias for context_describe.", {"id": _integer("Current-session context node id.", minimum=1), "limit": _integer("Maximum nodes to return.", minimum=1)}, []))
        self.register_tool("lcm_expand", self.context_expand, _spec("lcm_expand", "Alias for context_expand.", {"id": _integer("Current-session context node id.", minimum=1), "source_limit": _integer("Maximum source rows to include.", minimum=1)}, ["id"]))
        self.register_tool("context_query", self.context_query, _spec("context_query", "Search memories, session history, and LCM-style context nodes, then synthesize an answer.", {"query": _string("Question or recall query."), "limit": _integer("Maximum rows to return.", minimum=1)}, ["query"]))
        self.register_tool("reflect_memory", self.reflect_memory, _spec("reflect_memory", "Synthesize an answer from local memories and session/context recall without executing tools.", {"query": _string("Question to answer from memory/context."), "limit": _integer("Maximum rows to return.", minimum=1)}, ["query"]))
        ...
        self.register_tool("hindsight_recall", self.hindsight_recall, _spec("hindsight_recall", "Recall Hindsight-style memory plus related session/context matches.", {"query": _string("Recall query."), "limit": _integer("Maximum rows to return.", minimum=1)}, ["query"]))
        self.register_tool("hindsight_reflect", self.hindsight_reflect, _spec("hindsight_reflect", "Synthesize an answer across retained memory, messages, and local LCM-style context nodes.", {"query": _string("Question to reflect on."), "limit": _integer("Maximum rows to return.", minimum=1)}, ["query"]))
        self.register_tool("workspace_read", self.workspace_read, _spec("workspace_read", "Read a text file inside the engagement workspace.", {"path": _string("Workspace-relative path."), "limit": _integer("Maximum bytes/characters to return.", minimum=1)}, ["path"]))
        ...
        self.register_tool("workspace_search", self.workspace_search, _spec("workspace_search", "Search text files inside the engagement workspace.", {"query": _string("Substring/regex query."), "glob": _string("Glob like **/*.md."), "limit": _integer("Maximum matches to return.", minimum=1)}, ["query"]))
        ...
        self.register_tool("list_delegations", self.list_delegations, _spec("list_delegations", "List durable local delegation batches.", {"limit": _integer("Maximum rows to return.", minimum=1)}, []))
        ...
        self.register_tool("media_list", self.media_list, _spec("media_list", "List imported media/artifact files for this session.", {"limit": _integer("Maximum rows to return.", minimum=1)}, []))
        ...
        self.register_tool("list_approvals", self.list_approvals, _spec("list_approvals", "List approvals for the current session with redacted arguments/decisions.", {"status": _string("Approval status; default pending; use all for resolved approvals too."), "limit": _integer("Maximum rows to return.", minimum=1)}))
        ...
        self.register_tool("audit_log", self.audit_log, _spec("audit_log", "List recent redacted audit log entries.", {"limit": _integer("Maximum rows to return.", minimum=1)}))
        ...
        self.register_tool("export_session", self.export_session, _spec("export_session", "Export a redacted portable session handoff JSON bundle.", {"out": _string("Optional JSON output path; relative paths are written under agent/session-exports."), "message_limit": _integer("Maximum recent messages to include.", minimum=1)}))
        ...
        self.register_tool("list_tasks", self.list_tasks, _spec("list_tasks", "List the current session task board.", {"status": _string_enum("Filter by pending/in_progress/completed/cancelled/all.", _TASK_LIST_STATUS_VALUES), "limit": _integer("Maximum rows to return.", minimum=1)}))
